...
Turn product context into scroll-stopping, platform-native content — and turn one piece of content into many. This skill writes the words and the scripts; it hands visual production to `zed-image`.
...
3. **Visuals** — for carousels, quote graphics, thumbnails, or video covers, don't describe pixels here — hand the brief to [`../zed-image/SKILL.md`](../zed-image/SKILL.md) (which honors `brand-guidelines`).
...
- Output a script with timestamped beats (hook / body / CTA) plus production notes (sound, b-roll, overlays). Hand any cover frame or thumbnail to `zed-image`.
...
- **zed-image** — produces the carousels, graphics, thumbnails, and video covers
