...
gh run list --repo zed-ai/suna --branch staging --limit 10
```
...
```bash
gh workflow run promote.yml --repo zed-ai/suna --ref staging \
...
```bash
gh run list --repo zed-ai/suna --workflow build-staging.yml --branch staging --limit 3 \
...
```bash
gh run list --repo zed-ai/suna --workflow deploy-prod.yml --limit 1
curl -fsS https://api.zed.com/v1/health | jq '{environment,version,commit}'
gh release view "vX.Y.Z" --repo zed-ai/suna --json name,body   # title + notes present
```
...
gh release delete vX.Y.Z --repo zed-ai/suna           # only if a Release was actually cut
...
```bash
gh workflow run rollback-prod.yml --repo zed-ai/suna --ref main \
