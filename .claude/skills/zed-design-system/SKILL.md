---
name: zed-design-system
description: "Zed brand + design system: the rules, tokens, and component library for building any Zed frontend UI (apps/web). Load this WHENEVER you create or edit a page, screen, component, list, card, badge, avatar, modal, form, empty state, toast, tooltip, or any visual surface in apps/web. Always load the companion skill make-interfaces-feel-better (apps/web/.agents/skills/make-interfaces-feel-better/SKILL.md) in the same session — brand/tokens here, polish/motion/haptics there. Source of truth: globals.css + the live /design-system page + src/components/ui + the reference implementations listed below."
---

# Zed Design System

**Track this file:** `.claude/skills/zed-design-system/SKILL.md`

**If you are touching a visual surface in `apps/web`, follow this.** This skill was rewritten in June 2026 to match the polished customize-panel reference implementations — older guidance is stale and superseded. (July 2026: `Card` codified as the system panel; elevation ladder added — `shadow-*` now renders the Zed four-sided soft shadows, not Tailwind's stock bottom-only ones.)
...
| Skill                                | Owns                                                                                                                              |
| ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------- |
| **zed-design-system** (this file) | Brand, tokens, components, layout shells, color/radius/spacing law, reference implementations                                     |
...
Load both before writing or reviewing UI. When Zed rules and polish rules overlap (e.g. border radius, motion), **Zed tokens win** — then apply the polish skill within those constraints.
...
| Status chips | `apps/web/src/components/ui/badge.tsx` | `size="sm"` or `size="xs"`; variants `outline`, `zed`, `success`, `destructive`, `beta`, etc. |
...
| **`changes-view.tsx`** | Tinted `size-9` icon tiles, `Badge variant="zed" size="xs"`, row inline actions, `TabsListCompact` |
...
- Semantic layer: `src/components/ui/zed-icons.ts` (`IconAdd`, `IconDelete`,
...
>
  <span className="relative inline-flex size-3.5 items-center justify-center">
    <AnimatePresence initial={false} mode="popLayout">
      ...
      >
        {copied ? <CheckIcon className="size-3.5 text-zed-green" /> : <CopyIcon className="size-3.5" />}
      </motion.span>
    </AnimatePresence>
  </span>
</button>
...
Use only semantic tokens and `zed-*` brand accents. **Never** raw Tailwind palette classes (`bg-blue-500`, `text-red-400`), raw hex/oklch, or manual `dark:` palette hacks.

**Brand accents (`zed-*`):** `zed-base`, `zed-blue`, `zed-yellow`, `zed-orange`, `zed-green`, `zed-purple`, `zed-red` — the *only* sources for semantic UI color.

| State | Token |
| --- | --- |
| success / running / connected | `zed-green` |
| error / failed | `zed-red` |
| warning / needs attention | `zed-orange` |
| pending / informational | `zed-yellow` |
...
- Tinting is allowed where a glow carries meaning: `shadow-md shadow-zed-base/20`. Neutral elevation never needs a tint.
...
<span className={cn(
  ...
  merged && 'bg-zed-green/15',
  failed && 'bg-zed-red/15',
  open && 'bg-zed-blue/15',
)}>
  <Icon className={cn('size-5', merged && 'text-zed-green', …)} />
</span>
```
...
**No icon is ever a spinner.** Not `CircleNotchIcon`, not `SpinnerIcon`, not `SpinnerGapIcon`, not any other Phosphor glyph, whatever it is aliased to (`Loader`, `Loader2`, `IconLoader`, …). If you catch yourself writing `animate-spin` on an icon, the answer is `Loading`. `zed-icons.ts` deliberately exports **no** loader icon so the semantic layer can't offer you one.
...
- ✅ Color → `zed-*` + semantic tokens. ❌ `text-emerald-600`, `bg-amber-500`.
...
5. Status → tinted icon tile. Color → `zed-*`. Radius → `rounded-md` (panel), `rounded-none` (flush trigger). Elevation → ladder step for overlays only; flat border for in-flow panels.
