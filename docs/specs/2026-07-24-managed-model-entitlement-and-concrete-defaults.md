...
That exemption lets a free-tier account use Zed-managed LLM credentials.
The account wallet can then receive LLM debits.

The synthetic `auto` model hides the selected upstream model.
The SDK sends `zed/auto` when a user does not select a model.
The gateway resolves that value to a provider-funded or Zed-managed model.

## Required invariants

1. Tier controls Zed-managed model entitlement.
2. `free`, `none`, and unknown tiers cannot use Zed-managed models.
3. The rule applies in `dev`, `preview`, `staging`, and `prod`.
4. Wallet balance does not grant Zed-managed model entitlement.
5. Free-tier wallet credits fund sandbox compute only.
6. Paid tiers retain Zed-managed model access.
...
11. Stale `auto` and `zed/auto` requests return a typed `400` client error.
...
Zed-managed resolution rejects the request before dispatch.
...
- Test SDK sends without `zed/auto`.
- Test runtime defaults without `zed/auto`.
