...
indirection. Zed no longer exposes or resolves that model.
...
5. `auto` and `zed/auto` are stale inputs. The gateway returns
   `400 model_not_found`.
6. Free and `none` tiers cannot use Zed-managed models.
