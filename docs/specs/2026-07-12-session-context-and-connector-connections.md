...
**Audience:** Zed API/SDK/runtime owners and wrapper-backend authors such as VEYRIS.
...
`ZED_TOKEN` and `OPENCODE_CONFIG_CONTENT` impossible. More importantly,
individual keys are never expanded into environment variables.

### Persistence and runtime materialization

`zed.project_session_runtime_contexts` stores one validated JSON object per
...
```text
ZED_SESSION_CONTEXT={"workspace_id":"org_123","wrapper.locale":"de","licensed":true}
```
...
| Field | Meaning |
|---|---|
| `session_id` | durable Zed session |
...
Zed does not inject an arbitrary remote MCP server into OpenCode per session.
...
`zed-connectors` meta-tool server. It exposes catalog/discover/describe/call
...
One Zed project holds the VEYRIS agent configuration. Each VEYRIS Better
...
4. VEYRIS creates a Zed session with the two connection ids plus optional
   non-secret `runtime_context` (`workspace_id`, locale, display hints).
5. Zed atomically persists the session bindings before runtime provision.
...
workspace is ignored or rejected. The Zed project never receives direct
Neon credentials.
