...
`ZED_ENFORCE_SESSION_AGENT_LOCK`, default **off** — so in-session agent
...
switched agent can't inherit the boot agent's connector / Zed-CLI grant. The
...
the prompt reaches tool execution — then flip `ZED_ENFORCE_SESSION_AGENT_LOCK`
