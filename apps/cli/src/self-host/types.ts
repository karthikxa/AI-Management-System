export interface SelfHostCommandFlags {
  ...
  updateTz?: string;
  /** Operator holds a Zed Enterprise license: unlocks SSO/SCIM/RBAC/audit
   ...
   *  (ZED_RESTRICT_ACCOUNT_CREATION + the frontend-mirroring
   *  ZED_PUBLIC_RESTRICT_ACCOUNT_CREATION). Signups, existing teams, and
   ...
   *  `https://<domain>` (and `https://api.<domain>`) — sets ZED_DOMAIN,
   *  turning on the bundled Caddy reverse proxy/ACME TLS, same as
   *  `env set ZED_DOMAIN=...` (`--domain <domain>`). Pass an empty string
  ...
  tunnel?: 'cloudflare';
  /** Operator admin email(s), comma-separated. Sets ZED_PLATFORM_ADMIN_EMAILS
   ...
   *  any registry. Sets ZED_IMAGE_PULL=never (the updater/`update` skip
