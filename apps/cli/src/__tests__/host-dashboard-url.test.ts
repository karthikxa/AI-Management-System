...
// Regression coverage for the self-host login bug: `zed login` used to
...
// dev port pairing (`:8008` → `:3000`). Neither holds for `zed self-host`,
...
// `zed self-host` from its own `PUBLIC_URL`, or settable via
// `zed hosts add --dashboard-url`) that `webDashboardUrl` prefers over
// any guess.
...
beforeEach(() => {
  ...
  tmp = mkdtempSync(join(tmpdir(), 'zed-cli-host-dashboard-'));
  process.env.ZED_CONFIG_FILE = join(tmp, 'config.json');
});
...
describe('webDashboardUrl — explicit dashboard_url override', () => {
  test('self-host laptop default: API :13738 / dashboard :13737 — derivation would guess :3000 and be wrong', () => {
    ...
    // With the authoritative value (what `zed self-host` now stamps on
  ...
  test('explicit override wins over ZED_FRONTEND_URL / ZED_DASHBOARD_URL', () => {
    process.env.ZED_FRONTEND_URL = 'https://wrong.example.com';
    expect(webDashboardUrl('http://localhost:13738', 'http://localhost:13737')).toBe(
      'http://localhost:13737',
    );
  });
  ...
  test('domain self-host deployment still resolves without an explicit override (api.<domain> → <domain>)', () => {
    ...
    // derive from ZED_DOMAIN in self-host.ts's normalizeFullSupabaseEnv,
...
describe('Host.dashboard_url — persists through the config file round trip', () => {
  test('upsertHost → loadConfig → getHost carries dashboard_url through normalizeConfig', () => {
    ...
    // pass — the same path a real ~/.config/zed/config.json goes through).
    const config = loadConfig();
    expect(config.hosts.selfhost?.dashboard_url).toBe('http://localhost:13737');
  });

  test('saveAuthForHost (what `zed login` calls to persist a fresh token) does NOT wipe dashboard_url', () => {
    ...
    // first successful `zed login` against a `zed self-host`-registered
    // host silently erased the authoritative frontend URL, and the NEXT
    // `zed login` (e.g. after `zed logout` + `zed login` again) was
    ...
    expect(host?.token).toBe('zed_pat_fresh');
