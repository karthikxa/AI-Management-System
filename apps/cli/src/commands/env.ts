...
const HELP = help`Usage: zed env <subcommand> [options]
...
async function envPull(outArg: string | undefined, force: boolean, opts: CtxOpts): Promise<number> {
  ...
  const lines: string[] = [
    '# Generated by `zed env pull` — fill in values locally.',
  ...
  process.stdout.write(`${C.dim}  Values are not exported — see \`zed env push --from .env\` to upload.${C.reset}\n`);
  return 0;
}
