/**
 * Resolve the Zed FRONTEND (dashboard) base URL — never the API host.
 *
 * The CLI (and the in-sandbox agent that shells out to it) is connected to
 * Zed over the API, so the only base URL it is handed is `ZED_API_URL`
 * (e.g. https://api-prod.zed.com). User-facing links must point at the
 * frontend (e.g. https://zed.com). Resolving them by string-munging the API
 * host is fragile — it silently produced `api-prod.zed.com/projects/…`
 ...
 *      authoritatively at the source: `zed self-host` knows its own
 ...
 *   2. `ZED_FRONTEND_URL` — injected into every sandbox right next to
 *      `ZED_API_URL` (the server's `config.FRONTEND_URL`, verbatim). Also
 *      settable locally.
 *   3. `ZED_DASHBOARD_URL` — legacy local override, kept for back-compat.
...
export function webDashboardUrl(apiBase: string, dashboardUrl?: string | null): string {
  ...
  const explicit = sandboxEnvValue('ZED_FRONTEND_URL') || process.env.ZED_DASHBOARD_URL;
...
function deriveFrontendFromApiBase(apiBase: string): string {
  try {
    ...
    // api-prod.zed.com → zed.com ; api-dev.zed.com → dev.zed.com
    ...
    // dev-api.zed.com → dev.zed.com (any <env>-api.<domain>)
