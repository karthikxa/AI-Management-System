...
func TestDaemonizeReplacesAStalePidOwnedByAnotherProcess(t *testing.T) {
	...
	// prove that the process is zed-appd.
...
func TestChildEnvironmentRemovesControlSecrets(t *testing.T) {
	env := childEnvironment([]string{
		"PATH=/bin",
		"ZED_APPD_TOKEN=secret",
		"ZED_APP_SPEC={}",
		"ZED_APP_SPEC_PATH=/zed/config/app.json",
		"ZED_APPD_CADDY_BIN=/secret/path",
		"PUBLIC_VALUE=yes",
	})
	joined := strings.Join(env, "\n")
	if strings.Contains(joined, "ZED_APPD_TOKEN") || strings.Contains(joined, "ZED_APP_SPEC") {
