/**
 * The HTTP boundary between this worker and the Zed API.
 *
 ...
 * `ask_zed`, `run_command`, `post_turn` — the mirror of the old
 * `ask_zed` → `continueSession()`, sandbox exec, and `appendTurn()` paths.
 *
 ...
 * which is worse than it saying "I couldn't reach Zed."
 */
 ...
 * call announce "I could not reach Zed" when Zed had in fact answered
...
function mcpEndpoint(ctx: CallContext): string {
  const base = ctx.zedApiUrl.replace(/\/+$/, '');
  ...
  try {
    res = await fetch(url, {
      ...
      headers: {
        ...
        authorization: `Bearer ${ctx.zedApiToken}`,
      },
...
/**
 * Fire-and-forget hand-off to the Zed agent session — the mirror of the
 * old `ask_zed` → `continueSession()` call. Resolves the moment the
 * request is queued, never when Zed finishes thinking: that answer, if
 * any, arrives later out-of-band (see inbound-replies.ts) and gets spoken
 * then, not returned from here.
 */
export async function sendPromptToZed(
  ctx: CallContext,
  text: string,
): Promise<{ ok: true } | { ok: false; kind: PostFailureKind; error: string }> {
  const result = await callVoiceMcpTool(ctx, 'ask_zed', { request: text }, PROMPT_TIMEOUT_MS);
 ...
 * result — unlike `sendPromptToZed`, this DOES wait, because the whole
