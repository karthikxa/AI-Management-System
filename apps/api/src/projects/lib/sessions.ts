...
export async function buildSessionSandboxEnvVars(input: {
  ...
  if (input.defaultBranch && !input.platformMetaAgent) {
    ...
    // Wrap with a timeout so local provider sessions don't fail when the
    // git mirror is unavailable (large repo clone/fetch). Falls back to 'all'
    // (unrestricted) on timeout — safe for local dev.
    agentGrantEnv = await Promise.race([
...
export async function createProjectSession(input: {
  ...
  const loadedAgents = await loadProjectAgents(project, {
    forceRefresh: requestedProvider !== 'local',
  ...
  // Skip the loopback check for the local provider — it runs on the same machine
  // and can reach localhost just fine.
  ...
  // Skip for local provider — it doesn't use git mirrors so resolveTemplate
  // would hang trying to fetch from the remote.
  if (providerName !== 'local' && !platformMetaAgent && sandboxSlug && sandboxSlug !== DEFAULT_SANDBOX_SLUG) {
    try {
