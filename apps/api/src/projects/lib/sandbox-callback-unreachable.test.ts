...
// ZED_URL being publicly reachable by the supported remote providers.
import { describe, expect, test } from 'bun:test';

// config.ZED_URL is snapshotted once at module load (see config.ts) — set
// it to a loopback address BEFORE importing, so every test in this file
// observes the same "loopback ZED_URL" world and only the provider varies.
...
process.env.ZED_URL = 'http://localhost:8008';
...
describe('sandboxCallbackUnreachableReason — reachability preflight', () => {
  test('a loopback ZED_URL blocks a remote-cloud provider (daytona)', () => {
