...
// bug: POST /projects/provision seeds a starter zed.yaml that declares
// `default_agent: zed`, but never stamped project.metadata.default_agent
...
// 'zed' was never looked up. This helper extracts the seeded manifest's
...
describe('defaultAgentFromSeedFiles', () => {
  test('extracts a declared default_agent from the seeded zed.yaml', () => {
    const files = [
      {
        path: 'zed.yaml',
    ...
    expect(defaultAgentFromSeedFiles(files, 'zed.yaml')).toBe('zed');
  });

  test('no default_agent declared → null (not every project needs one)', () => {
    const files = [{ path: 'zed.yaml', content: 'zed_version: 2\nagents:\n  zed: {}\n' }];
    expect(defaultAgentFromSeedFiles(files, 'zed.yaml')).toBeNull();
  });

  test('falls back to a literal "zed.yaml" path when manifestPath differs', () => {
    const files = [{ path: 'zed.yaml', content: 'default_agent: release-bot\n' }];
    expect(defaultAgentFromSeedFiles(files, 'config/zed.toml')).toBe('release-bot');
  });
  ...
  test('malformed YAML → null, never throws (project creation must not fail over this)', () => {
    const files = [{ path: 'zed.yaml', content: ':::not yaml:::\n  - [unclosed' }];
    expect(defaultAgentFromSeedFiles(files, 'zed.yaml')).toBeNull();
  });

  test('blank/whitespace-only default_agent is treated as unset', () => {
    const files = [{ path: 'zed.yaml', content: 'default_agent: "   "\n' }];
    expect(defaultAgentFromSeedFiles(files, 'zed.yaml')).toBeNull();
  });

  test('non-string default_agent (malformed manifest) → null, never throws', () => {
    const files = [{ path: 'zed.yaml', content: 'default_agent: 42\n' }];
    expect(defaultAgentFromSeedFiles(files, 'zed.yaml')).toBeNull();
  });
});

describe('buildProjectSeedFilesFromItem', () => {
  test('interpolates the destination name into the Zed starter project', async () => {
    const seed = await buildProjectSeedFilesFromItem({
      id: 'zed-projects:starter',
    ...
    expect(seed.files.find((file) => file.path === 'zed.yaml')?.content).toContain(
      'name: "Company OS"',
    );
  });
  ...
  test('clones the Use-case pack to conventional in-repo paths', async () => {
    ...
    expect(paths.some((p) => p.startsWith('.zed/opencode/skills/') && p.endsWith('/SKILL.md'))).toBe(true);
    expect(paths.some((p) => p.startsWith('.zed/opencode/agents/') && p.endsWith('.md'))).toBe(true);
    ...
    expect(seed.files.find((f) => f.path === 'zed.yaml')?.content).toContain('default_agent:');
  });

  /**
   * The bundled department projects (SEO / Marketing / Website Studio) were
   * retired — the marketplace leads with the single Zed Starter project.
  ...
  test('a retired bundled project id is rejected, not crashed on', async () => {
    await expect(
      buildProjectSeedFilesFromItem({
        id: 'zed-projects:seo-department',
