...
projectsApp.openapi(
  ...
  // Merge DB-stored agents into config when manifest has none (local dev).
  try {
    ...
    if (rows.length > 0) {
      // Get existing agent names from manifest to avoid duplicates
      const existingNames = new Set<string>();
      if (Array.isArray(rawConfig.agents)) {
        rawConfig.agents.forEach((a: any) => existingNames.add(a.name));
      } else if (rawConfig.agents && typeof rawConfig.agents === 'object') {
        Object.keys(rawConfig.agents).forEach((k) => existingNames.add(k));
      }
      // Merge DB agents that aren't already in the manifest
      const dbOnly = rows
      ...
      if (dbOnly.length > 0) {
        const manifestAgents = Array.isArray(rawConfig.agents) ? rawConfig.agents : [];
        (rawConfig as any).agents = [...manifestAgents, ...dbOnly];
      }
    }
  } catch (e) {
    // DB agent merge skipped — config comes from manifest only
  }
