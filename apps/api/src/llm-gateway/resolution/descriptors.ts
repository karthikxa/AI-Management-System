import type { UpstreamDescriptor } from '@zed/llm-gateway';
...
// path (Zed's own AWS account/region choice); a BYOK project's default is
 ...
 * (Zed's own AWS_BEDROCK_REGION config). Takes the region as a parameter —
...
// geographies Zed has validated are mapped; an unrecognized region returns
...
function bedrockManagedDescriptor(managed: ManagedModel): UpstreamDescriptor | null {
  ...
  // NOTE — this is the MANAGED (Zed-credits) Bedrock path, reached only when
  // ZED_MANAGED_PROVIDER_ENABLED is on: it uses ZED'S OWN shared AWS
  // credentials and bills the user's Zed credits. It is NOT "how Bedrock
...
export function managedCandidates(managed: ManagedModel): UpstreamDescriptor[] {
  ...
  // on a deployment with ZED_MANAGED_PROVIDER_ENABLED off (managed-models.ts),
  ...
  if (!config.ZED_MANAGED_PROVIDER_ENABLED) return [];
  const d = {
