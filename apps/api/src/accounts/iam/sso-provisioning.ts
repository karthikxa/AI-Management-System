...
// flow: instead of a Zed operator running `supabase sso add` out of band, we
 ...
 * logs; a failure never blocks saving the (already-persisted) Zed config.
 */
 ...
 * Zed row is removed but the Supabase provider is orphaned, stranding the
