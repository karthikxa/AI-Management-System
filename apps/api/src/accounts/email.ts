...
const BRAND_WORDMARK = 'Zed';
const BRAND_FOOTER = 'Zed — The Autonomous Company Operating System';
...
export async function sendAccountInviteEmail(opts: {
  ...
  const body = `
    <p style="${S.p}">
      ${inviterLine} to join ${target} on Zed.
    </p>
    ${roleChip}
    <a href="${escapeHtml(url)}" style="${S.btn}">Review invite</a>
    <p style="${S.smallNote}">
      Don't have a Zed account yet? You'll be prompted to sign up first —
      ${signupTail}
    </p>
  `;
  ...
  const html = renderEmail({
    ...
    title: opts.projectName
      ? `Join ${opts.projectName} on Zed`
      : `Join ${opts.accountName} on Zed`,
    body,
  });

  return send({
    to: opts.email,
    subject: `You're invited to ${subjectTarget} on Zed`,
    html,
...
export async function sendProjectAccessRequestEmail(opts: {
  ...
  const projectName = opts.projectName?.trim() || 'a Zed project';
