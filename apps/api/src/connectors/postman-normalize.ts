...
export function normalizePostmanCollection(doc: any): PostmanNormalizationResult {
  ...
  if (doc.auth) warnings.push('collection authentication was ignored; configure connector auth in Zed');
  ...
  const walk = (items: unknown[], folders: string[], inherited: Map<string, string>) => {
    for (const candidate of items) {
      ...
      if (request.auth) warnings.push(`${[...folders, name].join(' / ')} authentication was ignored; configure connector auth in Zed`);
      ...
      if (sanitizedUrl.removed) {
        warnings.push(`${[...folders, name].join(' / ')} credential-like query parameter was ignored; configure connector auth in Zed`);
      }
