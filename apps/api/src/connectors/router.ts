...
export function createConnectorRouter(deps: ConnectorRouterDeps): OpenAPIHono {
  ...
  app.openapi(
    ...
    async (c: any) => {
      ...
      // When the feature flag is disabled, return empty list instead of error
      // so the UI can gracefully fall back to the Pipedream catalogue
      if (!(await deps.featureFlagEnabled(projectId, 'connectors_api_discover'))) {
        return c.json({ items: [], total: 0, hasMore: false, nextCursor: undefined });
      }
      if (!deps.listDiscoverConnectors) return c.json({ items: [], total: 0, hasMore: false, nextCursor: undefined });
      try {
        return c.json(
  ...
  app.openapi(
    ...
    async (c: any) => {
      ...
      const admin = await deps.resolveAdmin(c, projectId);
      if (!admin) return c.json({ error: 'forbidden' }, 403);
      // When the feature flag is disabled, return empty object instead of error
      if (!(await deps.featureFlagEnabled(projectId, 'connectors_api_discover'))) {
        return c.json({});
      }
      if (!deps.getDiscoverConnector) return c.json({});
      try {
  ...
  app.openapi(
    ...
    async (c: any) => {
      ...
      const admin = await deps.resolveAdmin(c, projectId);
      if (!admin) return c.json({ error: 'forbidden' }, 403);
      // Return empty list when Pipedream isn't configured (local dev)
      if (!deps.listPipedreamApps) {
        return c.json({ apps: [], has_more: false, error: null });
      }
      try {
        ...
        // Graceful fallback when Pipedream auth fails (e.g. invalid credentials in local dev)
        console.warn('[connectors] pipedream/apps failed, returning empty list:', err instanceof Error ? err.message : String(err));
        return c.json({ apps: [], has_more: false, error: null });
      }
    },
  );
