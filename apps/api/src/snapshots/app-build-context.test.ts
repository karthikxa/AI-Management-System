...
afterEach(async () => {
  delete process.env.ZED_APPD_BIN_PATH;
  delete process.env.ZED_APP_CADDY_BIN_PATH;
...
describe('stageAppBuildContext', () => {
  test('preserves the source context and adds only the immutable App runtime layer', async () => {
    const fixture = await mkdtemp(join(tmpdir(), 'zed-app-context-test-'));
    ...
    process.env.ZED_APPD_BIN_PATH = join(fixture, 'appd');
    process.env.ZED_APP_CADDY_BIN_PATH = join(fixture, 'caddy');
    ...
    const runtimeSpecPath = join(staged.contextDir, '.zed-app-runtime', 'app.json');
    ...
    expect(dockerfile).toContain('ENTRYPOINT ["/zed/bin/zed-appd"]');
    expect(dockerfile).toContain('EXPOSE 7331 8080');
    expect(dockerfile).not.toContain('ZED_APPD_TOKEN');
  });
  ...
  test('rejects a source path that can replace runtime files', async () => {
    ...
    cleanup.push(fixture);
    const source = join(fixture, 'source');
    await mkdir(join(source, '.zed-app-runtime'), { recursive: true });
    for (const name of ['appd', 'caddy']) {
      await writeFile(join(fixture, name), name);
      await chmod(join(fixture, name), 0o755);
    }
    ...
    await expect(
      stageAppBuildContext('zed-app-test', 'FROM scratch', {
        sourceDir: source,
