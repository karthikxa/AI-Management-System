...
describe('waitForActive — PHASE 2 poll error classification', () => {
  test('a 401 during polling fails immediately (does not burn the deadline)', async () => {
    ...
    await expect(waitForActive('zed-default-abc', undefined, 'tpl_abc')).rejects.toThrow(/401/);
  ...
  test('a TLS cert failure fails immediately', async () => {
    ...
    await expect(waitForActive('zed-default-abc', undefined, 'tpl_abc')).rejects.toThrow();
  }, 10_000);

  test('an id that resolves to a different name is rejected (adopt mismatch)', async () => {
    globalThis.fetch = (async () =>
      jsonResponse({ id: 'tpl_abc', name: 'zed-default-OTHER', state: 'ready' })) as unknown as typeof fetch;

    await expect(waitForActive('zed-default-abc', undefined, 'tpl_abc')).rejects.toThrow(/mismatched template/);
  }, 10_000);

  test('resolves when the exact id reports ready', async () => {
    globalThis.fetch = (async () =>
      jsonResponse({ id: 'tpl_abc', name: 'zed-default-abc', state: 'ready' })) as unknown as typeof fetch;

    await expect(waitForActive('zed-default-abc', undefined, 'tpl_abc')).resolves.toBeUndefined();
  }, 10_000);
  ...
  test('a transient 503 does NOT fail the wait — it retries to ready', async () => {
    ...
    globalThis.fetch = (async () => {
      ...
      return jsonResponse({ id: 'tpl_abc', name: 'zed-default-abc', state: 'ready' });
    }) as unknown as typeof fetch;
  ...
  test('an explicit provider "failed" state is terminal', async () => {
    globalThis.fetch = (async () =>
      jsonResponse({ id: 'tpl_abc', name: 'zed-default-abc', state: 'failed' })) as unknown as typeof fetch;

    await expect(waitForActive('zed-default-abc', undefined, 'tpl_abc')).rejects.toThrow(/build failed/);
  }, 10_000);
});
