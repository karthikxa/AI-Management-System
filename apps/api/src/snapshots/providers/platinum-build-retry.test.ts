...
describe('Platinum build-error retry classifier', () => {
  test.each([
    ...
    [
      ...
      new Error('Platinum template zed-default-3e3906a27df1 did not become ready (last state: missing)'),
    ],
    [
  ...
  test.each([
    ...
    ['explicit build failure', new Error('Platinum template zed-default-abc123 build failed')],
    ...
    [
      ...
      new Error('Platinum template zed-default-abc123 did not become ready (last state: building)'),
    ],
    ...
    // Nothing frees a template row on its own and Zed has no org-wide GC for
