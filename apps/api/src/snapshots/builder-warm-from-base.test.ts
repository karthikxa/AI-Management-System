  ...
  const ref = await resolveWarmBaseImageRef(
    {
      ...
      getSnapshotImageRef: async () => 'registry.example/zed-default-abc:latest',
    },
    'zed-default-abc',
  );
  expect(ref).toBe('registry.example/zed-default-abc:latest');
});
  ...
  const ref = await resolveWarmBaseImageRef(
    {
      ...
      getSnapshotImageRef: async () => {
        imageRefCalls += 1;
        return 'registry.example/zed-default-abc:latest';
      },
    },
    'zed-default-abc',
  );
