...
export async function provisionSessionSandbox(opts: {
  ...
  // path.
  // Local provider skips snapshot resolution entirely — it runs the daemon
  // directly on the host, no cloud image needed.
  let firstImagePromise: Promise<FirstImage> | null = providerName === 'local'
    ? null
    : (async () => {
  ...
  void (async () => {
    ...
    try {
      ...
      let image: EnsureSandboxImageResult | null = null;
      if (providerName === 'local') {
        // Local provider skips snapshot resolution — no cloud image needed.
        console.log(`[session-sandbox] Local provider — skipping snapshot resolution for ${sandbox.sandboxId}`);
      ...
      if (image) {
        ...
        imageInfo = { snapshotName: 'local', slug: 'local', contentHash: '', isDefault: true };
      }
      if (image) {
        console.log(
        ...
        console.log(`[session-sandbox] Booting ${sandbox.sandboxId} from local provider (branch ${branch})`);
      }
      ...
      const bootDecision = decideSessionBoot({
        ...
        imageIsDefault: image?.isDefault ?? true,
      ...
      const createFn = bootDecision.bootByTemplateId
        ...
        : undefined;
      console.log(`[session-sandbox] Calling provider.create for ${sandbox.sandboxId} (provider=${providerName}, createFn=${createFn ? 'pinned' : 'default'})`);
