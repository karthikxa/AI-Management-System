 ...
 */
export type ProviderName = 'daytona' | 'platinum' | 'e2b' | 'local';
...
export function getProvider(name: ProviderName): SandboxProvider {
  ...
  switch (name) {
    ...
    case 'e2b':
      ...
      break;
    case 'local':
      // Local provider requires no API keys - runs commands locally
 ...
 * Falls back to 'local' when no cloud provider is configured.
 */
export function getDefaultProvider(): ProviderName {
  if (config.DAYTONA_API_KEY) return 'daytona';
  if (config.PLATINUM_API_KEY) return 'platinum';
  if (config.E2B_API_KEY) return 'e2b';
  return 'local';
}
