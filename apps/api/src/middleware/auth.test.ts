...
mock.module('../repositories/api-keys', () => ({
  validateSecretKey: async () => ({ isValid: false, error: 'Invalid Zed token' }),
}));
...
describe('project-scoped PAT on the sandbox-proxy path', () => {
  ...
  test('CAN drive its own project sandbox via /v1/p/{sandboxId}/{port}/...', async () => {
    const res = await appWithProbe().request(`/v1/p/${SANDBOX_A}/8000/turn-stream`, {
      headers: { Authorization: 'Bearer zed_pat_project_a' },
    });
  ...
  // The in-sandbox `ZED_CLI_TOKEN` IS a project+session-scoped PAT, and
  ...
  // in a sandbox running the `zed skills get <name>` that every baked image
  ...
  test('project-scoped PAT CAN read a system skill body and a reference file', async () => {
    const body = await appWithProbe().request('/v1/skills/zed-system', {
      headers: { Authorization: 'Bearer zed_pat_project_a' },
    });
    expect(body.status).toBe(200);

    const file = await appWithProbe().request(
      '/v1/skills/zed-system/file?path=references/capabilities.md',
      { headers: { Authorization: 'Bearer zed_pat_project_a' } },
    );
  ...
  test('account-scoped PAT (no project binding) reaches the sandbox proxy unchanged', async () => {
    const res = await appWithProbe().request(`/v1/p/${SANDBOX_A}/8000/turn-stream`, {
      headers: { Authorization: 'Bearer zed_pat_account_scoped' },
    });
