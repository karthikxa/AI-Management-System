...
/**
 * Rejects sandbox-agent tokens (the `zed_` API keys an agent runs with
 * inside ONE project/session sandbox) from account-scoped read routes they
 * have no legitimate reason to hit.
 *
 * Why this exists: `combinedAuth` accepts sandbox `zed_` tokens and sets
 ...
 * `zed_` API key (no `sandboxId`) is also allowed — those are operator
