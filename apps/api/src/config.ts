...
export const config = {
  ...
  isProviderEnabled(name: SandboxProviderName): boolean {
    ...
    switch (name) {
      ...
      case 'local':
        // Local provider is always enabled when allowed - no API key required
        return true;
      default: {
  ...
  getDefaultProvider(): SandboxProviderName {
    // If local is allowed and no cloud providers are configured, use local
    if (this.ALLOWED_SANDBOX_PROVIDERS.includes('local') && 
        !this.isDaytonaEnabled() && !this.isPlatinumEnabled() && !this.isE2BEnabled()) {
      return 'local';
    }
