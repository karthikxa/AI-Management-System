...
export function buildFinalCard(opts: {
  ...
  if (opts.sessionUrl) {
    elements.push({
      type: 'TextBlock',
      text: `[Open session in Zed ↗](${opts.sessionUrl})`,
      wrap: true,
      isSubtle: true,
      size: 'small',
      spacing: 'medium',
...
export function buildAnswerCard(body: string, sessionUrl?: string): Record<string, unknown> {
  ...
  if (sessionUrl) {
    elements.push({
      type: 'TextBlock',
      text: `[Open session in Zed ↗](${sessionUrl})`,
      wrap: true,
      isSubtle: true,
      size: 'small',
      spacing: 'medium',
...
export function buildPanelCard(opts: {
  ...
  const actions = opts.url ? [openUrlAction('Open in Zed', opts.url)] : undefined;
...
export function buildReviewCard(opts: {
  ...
  if (opts.viewUrl) actions.push(openUrlAction('View in Zed', opts.viewUrl));
  return card(body, actions);
}
...
export function buildHelpCard(commands: Array<{ cmd: string; desc: string }>): Record<string, unknown> {
  ...
  return card([
    ...headerBlock('⚡', 'Zed commands', 'Run a command, or just @-mention me with a task.'),
