...
async function handleReview(
  ...
  if (!identity) {
    return cardResponse(buildNoticeCard('Connect your Zed account (`/login`) to act on reviews.'));
  }
...
async function handleRequestAccess(
  ...
  switch (outcome.status) {
    ...
    case 'pending':
      ...
      return cardResponse(buildNoticeCard('Access requested. An admin will approve it in Zed.', '✅'));
    case 'already-member':
      return cardResponse(buildNoticeCard("You already have access — send your message again and I'll pick it up."));
    case 'no-identity':
      return cardResponse(buildNoticeCard('Connect your Zed account first, then request access.'));
    case 'no-project':
