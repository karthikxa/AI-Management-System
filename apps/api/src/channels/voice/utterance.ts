/**
 * Every way Zed puts words into a live call, in ONE place — each paired with
 ...
 * no way to record a Zed utterance without re-deriving the payload out of a
 * prompt, and consequently nothing recorded it at all: the /voice page showed
 * none of what the Zed agent said into the call, because the ONLY record of
 ...
 * itself, so it asked Zed again to resolve the contradiction, indefinitely,
 ...
 * things instead — states that this is what Zed says, that it SUPERSEDES
 ...
 */
const SUPERSEDES =
  'This is what your Zed agent — the one that actually knows this project — says, so it ' +
  ...
  'breath ("I had that wrong — it is actually…") and move on. Do not ask Zed about it again.';
...
export type ZedUtteranceKind = 'say' | 'progress' | 'result' | 'question' | 'review' | 'error';

export interface ZedUtterance {
  kind: ZedUtteranceKind;
...
/**
 * `speaker` for every turn Zed itself puts into a call.
 *
 ...
 * two are different events — what Zed asked the room to hear, and what the
 ...
 * a tool call.
 */
export const ZED_SPEAKER = 'zed';

/**
 * `send_prompt` — the Zed agent speaking into the call unprompted.
 *
 ...
 * unattributed order and it has no idea the words came from its own Zed
 ...
 * one must too.
 */
export function zedSay(text: string): ZedUtterance {
  return {
    kind: 'say',
    instruction:
      `[say] Your Zed agent — the one you hand work to — wants the room to hear this now. ` +
 ...
 */
export function zedProgress(step: string): ZedUtterance {
  return {
    kind: 'progress',
    instruction:
 ...
 */
export function zedResult(text: string): ZedUtterance {
  return {
    kind: 'result',
    instruction:
      `[result] Zed has answered. State this answer to the room now, briefly and ` +
 ...
 */
export function zedError(message?: string | null): ZedUtterance {
...
export function zedQuestion(text: string): ZedUtterance {
  return {
    kind: 'question',
...
export function zedReview(title: string): ZedUtterance {
  return {
    kind: 'review',
