/**
 * Whether a call is allowed to hand another request to Zed right now.
 *
 * THE BUG. A live call handed Zed the same question over and over. A stray
 ...
 * fact, so every correct answer Zed sent back contradicted what it believed,
 ...
 * a real Zed turn: $0.02-$0.03 and up to 3.2k tokens. It ran until a human
 * hung up.
 *
...
export const ASK_SPEAKER = 'ask_zed';
/** `speaker` of the row written when that hand-off ends, however it ends. */
export const ASK_SETTLED_SPEAKER = 'ask_zed_done';
...
export const IN_FLIGHT_MESSAGE =
  'You already handed a request to Zed and the answer has not come back yet. It arrives on ' +
  ...
  'rather than asking Zed to settle it.';
...
function rateMessage(asks: number): string {
  return `You have asked Zed ${asks} times in the last minute and are repeating yourself. ${RATE_ADVICE}`;
}
