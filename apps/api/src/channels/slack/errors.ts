...
export function classifyTurnError(info?: TurnErrorInfo): ClassifiedTurnError {
  ...
  if (isInsufficientCredits(status, lower)) {
    ...
    return {
      ...
      text:
        ...
        ` Top up credits (or turn on auto top-up) in Zed and mention me again to continue.`,
      aborted: false,
    };
  }
  ...
  if (isAgentUnavailable(lower)) {
    return {
      ...
      text:
        ...
        ` Run \`/zed agents\` to pick one of this project's current agents, then mention me again.`,
      aborted: false,
    };
  }
  ...
  if (isModelNotFound(status, lower)) {
    return {
      ...
      text:
        ...
        ` Pick a different model in Zed settings, then mention me again.`,
      aborted: false,
    };
  }
  ...
  if (isProviderConfig(name, status)) {
    ...
    return {
      ...
      text:
        `:warning: *${capitalize(who)} rejected this request* — its API key or model config in Zed` +
