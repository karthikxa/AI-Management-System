...
// Resolve the Slack sender to the Zed user they linked via `/login`, and
...
// A re-link (same Slack user, new Zed user) overwrites the mapping and clears
...
// When an unlinked or no-access sender @-mentions Zed we answer right where
...
//   • unlinked   → "Connect your Zed account" (opens the /login web flow)
  ...
  return [
    {
      ...
      text: {
        type: 'mrkdwn',
        text: 'Zed needs a linked Zed account before it can run from Slack. Connect or create one to continue. _Only you can see this._',
      },
    },
    {
      type: 'actions',
  ...
  return [
    {
      ...
      text: {
        type: 'mrkdwn',
        text: "You're connected, but your Zed account doesn't have access to this project yet. Request access and an admin will approve it. _Only you can see this._",
      },
    },
    {
      type: 'actions',
...
export async function postIdentityPrompt(input: {
  ...
  if (input.reason === 'unlinked') {
    ...
    fallback = 'Zed needs a linked Zed account to continue.';
  } else {
...
export async function createSlackAccessRequest(input: {
  ...
  await db.insert(projectAccessRequests).values({
    ...
    message: 'Requested from Slack. Approve as Editor so they can run Zed from Slack.',
  });
...
// Zed. Best-effort — an admin without a Slack link still sees it on the web
...
export async function notifyAdminsOfAccessRequest(input: {
  ...
  const text = `${who} requested access to a Zed project in this workspace.`;
  const blocks = [
    {
      type: 'section',
      text: { type: 'mrkdwn', text: `${text}\nOpen *Members* in Zed to approve.` },
    },
    {
      type: 'actions',
      elements: [
        {
          type: 'button',
          text: { type: 'plain_text', text: 'Review in Zed', emoji: true },
          style: 'primary',
          url: projectUrl,
  ...
  for (const admin of admins) {
    ...
    const slackId = await lookupSlackUserIdForZedUser(input.teamId, admin.userId);
    if (!slackId) continue;
...
// Reverse of lookupSlackIdentity: the Slack user a Zed user is linked to in a
// given workspace, so we can DM admins about access requests.
export async function lookupSlackUserIdForZedUser(teamId: string, userId: string): Promise<string | null> {
  const [row] = await db
