...
import { lookupSlackIdentity, lookupSlackUserIdForZedUser } from './identity';
  ...
  return [
    {
      ...
      text: {
        type: 'mrkdwn',
        text: `*${escapeMrkdwn(input.requesterLabel)}* wants to join this Zed session.\nThis Slack thread is private until you approve them.`,
      },
    },
    {
      type: 'actions',
...
async function postJoinRequest(input: {
    ...
    [
      {
        ...
        text: {
          type: 'mrkdwn',
          text: `*This Zed session is private.*\n${requesterText}`,
        },
      },
    ],
    input.threadId,
  ...
  const ownerSlackUserId = await lookupSlackUserIdForZedUser(input.teamId, input.ownerUserId);
  ...
  await postEphemeral(
    ...
    input.channel,
    ownerSlackUserId,
    `${label} wants to join this Zed session.`,
  ...
  if (policy === 'owner_only') {
    if (token && input.channel) {
      await postEphemeral(
        ...
        input.channel,
        input.slackUserId,
        'This Zed session is owner-only.',
        [
          {
            type: 'section',
            text: {
              type: 'mrkdwn',
              text: '*This Zed session is owner-only.*\nStart a new thread if you want Zed to work with you separately.',
            },
          },
        ],
        input.threadId,
        ...
        [
          {
            ...
            text: {
              type: 'mrkdwn',
              text: "*You don't have access to this Zed session.*\nThe owner declined your request. Start a new thread to work with Zed separately.",
            },
          },
        ],
        input.threadId,
...
export async function decideSlackThreadJoin(input: {
  ...
  if (!decider) return { ok: false, text: 'Connect your Zed account before approving session access.' };
  ...
  const [session] = await db
    ...
    .limit(1);
  if (!session) return { ok: false, text: 'This Zed session no longer exists.' };
  ...
  if (token) {
    const text = input.decision === 'approved'
      ? `You've been approved for this Zed session. Send your message again and I'll continue.`
      : 'The session owner declined your request for this Zed session.';
    await postEphemeral(
      token,
      input.channelId,
      input.requesterSlackUserId,
      ...
      [
        {
          ...
          text: {
            ...
            text: input.decision === 'approved'
              ? `*Approved.*\nSend your message again in this thread. You can also <${sessionUrl}|open the session in Zed>.`
              : '*Request declined.*\nStart a new thread if you want Zed to work with you separately.',
          },
        },
      ],
  ...
  return {
    ok: true,
    text: input.decision === 'approved'
      ? `Approved ${label} for this Zed session.`
      : `Denied ${label} for this Zed session.`,
  };
}
