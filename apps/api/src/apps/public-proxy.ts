...
export function appPublicStatusResponse(
  ...
  const headers = new Headers({
    ...
    'content-security-policy':
      "default-src 'none'; style-src 'unsafe-inline'; frame-ancestors 'self' https://zed.com https://*.zed.com http://localhost:* http://127.0.0.1:*",
...
export function resolveAppHost(hostname: string): ResolvedAppHost | null {
  ...
  const domain = (process.env.ZED_APPS_BASE_DOMAIN || 'apps.zed.com')
    .toLowerCase()
    .replace(/^\.+|\.+$/g, '');
...
export function verifyAppEdgeRequest(
  ...
  if (process.env.ZED_APPS_ALLOW_DIRECT_EDGE === 'true') return true;
