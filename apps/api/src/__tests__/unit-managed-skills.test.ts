...
import { ZED_MANAGED_SKILL_NAMES } from '@zed/starter';
import {
  buildManagedSkills,
  getManagedSkill,
  getManagedSkillFile,
...
describe('managed skill catalog', () => {
  test('resolves every name in ZED_MANAGED_SKILL_NAMES', () => {
    const built = buildManagedSkills();
    const missing = ZED_MANAGED_SKILL_NAMES.filter((n) => !built.has(n));
  ...
  test('serves only managed skills — no general starter skill leaks in', () => {
    const managed = new Set<string>(ZED_MANAGED_SKILL_NAMES);
  ...
  test('zed-system carries its reference tree, sorted, SKILL.md excluded', () => {
    const skill = getManagedSkill('zed-system');
  ...
  test('the list carries no bodies — it stays small enough to always read', () => {
    ...
    expect(summaries.length).toBe(ZED_MANAGED_SKILL_NAMES.length);
  ...
  test('the served text is byte-identical to the sandbox-baked managed skills', () => {
    // Both sides run the same @zed/starter extraction; this pins them together
    // so the API can never serve instructions a session would not get.
    const skill = getManagedSkill('zed-connectors');
    expect(skill).not.toBeNull();
  ...
  test('file lookup is exact-match, so traversal paths simply miss', () => {
    expect(getManagedSkillFile('zed-system', 'SKILL.md')?.content).toBe(
      getManagedSkill('zed-system')!.body,
    );
    expect(getManagedSkillFile('zed-system', '../zed-slack/SKILL.md')).toBeNull();
    expect(getManagedSkillFile('zed-system', '/etc/passwd')).toBeNull();
    expect(getManagedSkillFile('zed-system', 'references/../../../secret')).toBeNull();
  });

  test('a real reference file round-trips by its listed path', () => {
    const skill = getManagedSkill('zed-system')!;
    const first = skill.references[0];
    expect(getManagedSkillFile('zed-system', first.path)?.content).toBe(first.content);
  });
});
