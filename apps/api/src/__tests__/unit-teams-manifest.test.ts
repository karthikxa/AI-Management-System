...
describe('teamsMode', () => {
  test('enabled + configured → exposes the messaging endpoint and admin-consent url', () => {
    const mode = teamsMode('https://api.zed.com/', { enabled: true });
    expect(mode.enabled).toBe(true);
    expect(mode.available).toBe(true);
    expect(mode.appId).toBe('app-123');
    expect(mode.messagingEndpoint).toBe('https://api.zed.com/v1/webhooks/teams/messages');
  ...
  test('project has the `teams` experiment off → nothing is exposed even though the server is configured', () => {
    const mode = teamsMode('https://api.zed.com/', { enabled: false });
  ...
  test('bring-your-own bot routes the webhook at the project and needs no server credentials', () => {
    const mode = teamsMode('https://api.zed.com/', {
      enabled: true,
      projectId: 'p-1',
      byoAppId: 'byo-app-9',
    });
    ...
    expect(mode.messagingEndpoint).toBe('https://api.zed.com/v1/webhooks/teams/p-1/messages');
  });
});
