 ...
 * starter it seeded used to ship a zed_version 1 manifest with no declared
...
import { DEFAULT_STARTER_TEMPLATE_ID } from '@zed/starter';
...
describe('buildProjectSeedFiles — the seeded manifest satisfies its own require_declared_agents stamp', () => {
  test('seeds a v2 zed.yaml, not a v1 zed.toml', async () => {
    const seed = await buildProjectSeedFiles({
      projectName: 'Acme Co',
      repoFullName: 'zed/acme-co',
      template: 'minimal',
      marketplaceItems: [],
      now: new Date('2026-07-05T00:00:00Z').toISOString(),
    });

    const manifest = seed.files.find((f) => f.path === 'zed.yaml');
    expect(manifest).toBeDefined();
    expect(manifest?.content).toContain('zed_version: 2');
    expect(seed.files.some((f) => f.path === 'zed.toml')).toBe(false);
  });
  ...
  test('the seeded manifest is schema-valid with zero errors', async () => {
    ...
    const manifestFile = seed.files.find((f) => f.path === 'zed.yaml')!;
