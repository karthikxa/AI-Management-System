...
describe('selectSnapshotsToReap — pressure gate', () => {
  it('does nothing while the ORG total is under the high-water mark', () => {
    const defaults = Array.from({ length: 40 }, (_, i) =>
      snap(`zed-default-${i}`, { lastUsedAt: ago(i + 1) }),
    );
  ...
  it('fires on org total even when OUR namespace is tiny', () => {
    const managed = [
      ...Array.from({ length: 13 }, (_, i) => snap(`zed-default-${i}`, { lastUsedAt: ago(i) })),
      snap('zed-tpl-a'),
    ];
...
describe('selectSnapshotsToReap — defaults ranked by freshness, not idle', () => {
  ...
  it('reaps superseded defaults that are not idle at all', () => {
    const defaults = Array.from({ length: 20 }, (_, i) =>
      snap(`zed-default-${i}`, { lastUsedAt: new Date(NOW - i * 60_000).toISOString() }),
    );
    ...
    for (let i = 0; i < QUOTA_GC_KEEP_FRESHEST_DEFAULTS; i++) {
      expect(reaped).not.toContain(`zed-default-${i}`);
    }
    expect(reaped).toContain(`zed-default-${QUOTA_GC_KEEP_FRESHEST_DEFAULTS}`);
    expect(reaped).toContain('zed-default-19');
  });

  it('never reaps a default a local template row still references', () => {
    const defaults = Array.from({ length: 20 }, (_, i) =>
      snap(`zed-default-${i}`, { lastUsedAt: ago(i) }),
    );
    const res = run(padToOrgSize(defaults, 90), ['zed-default-19']);
    expect(names(res)).not.toContain('zed-default-19');
  });
});
...
describe('selectSnapshotsToReap — safety invariants', () => {
  const base = () =>
    padToOrgSize(
      [
        snap('zed-tpl-user', { lastUsedAt: ago(30) }),
        snap('zed-ppwarm-aaaaaaaa-tip', { lastUsedAt: ago(1) }),
  ...
  it('never touches non-zed stock/bench images, however stale', () => {
  ...
  it('never deletes an in-flight build', () => {
    const all = padToOrgSize(
      [
        snap('zed-default-live', { lastUsedAt: ago(0), state: 'building' }),
    ...
    expect(names(run(all))).not.toContain('zed-default-live');
  });

  it('reaps broken-state snapshots in our namespace', () => {
    const all = padToOrgSize([snap('zed-default-bad', { state: 'error' })], 90);
    expect(names(run(all))).toContain('zed-default-bad');
  });
  ...
  it('keeps the freshest ppwarm tip per project and reaps its stragglers', () => {
    const all = padToOrgSize(
      [
        snap('zed-ppwarm-0945686d-new', { lastUsedAt: ago(1) }),
        snap('zed-ppwarm-0945686d-old', { lastUsedAt: ago(2) }),
        snap('zed-ppwarm-0945686d-older', { lastUsedAt: ago(3) }),
        snap('zed-ppwarm-ffffffff-solo', { lastUsedAt: ago(5) }),
      ],
      UNDER_TARGET,
    );
    const reaped = names(run(all));
    expect(reaped).not.toContain('zed-ppwarm-0945686d-new');
    expect(reaped).toContain('zed-ppwarm-0945686d-old');
    expect(reaped).toContain('zed-ppwarm-0945686d-older');
    // A project's only tip is live until proven idle.
    expect(reaped).not.toContain('zed-ppwarm-ffffffff-solo');
  });
  ...
  it('spares a freshly-built "superseded" ppwarm tip (mixed-version protection)', () => {
    ...
    const all = padToOrgSize(
      [
        snap('zed-ppwarm-0945686d-current', { lastUsedAt: minutes(1) }),
        snap('zed-ppwarm-0945686d-otherver', { lastUsedAt: minutes(10), createdAt: minutes(10) }),
        snap('zed-ppwarm-0945686d-trulyold', { lastUsedAt: ago(2) }),
      ],
      UNDER_TARGET,
    );
    const reaped = names(run(all));
    expect(reaped).not.toContain('zed-ppwarm-0945686d-current');
    expect(reaped).not.toContain('zed-ppwarm-0945686d-otherver');
    expect(reaped).toContain('zed-ppwarm-0945686d-trulyold');
  });
  ...
  it('reaps a long-idle ppwarm tip but spares a recently used one', () => {
    const all = padToOrgSize(
      [
        snap('zed-ppwarm-aaaaaaaa-stale', { lastUsedAt: ago(20) }),
        snap('zed-ppwarm-bbbbbbbb-fresh', { lastUsedAt: ago(2) }),
      ],
      UNDER_TARGET,
    );
    const reaped = names(run(all));
    expect(reaped).toContain('zed-ppwarm-aaaaaaaa-stale');
    expect(reaped).not.toContain('zed-ppwarm-bbbbbbbb-fresh');
  });

  it('keeps anything with no usable timestamp — cannot prove it is idle', () => {
    const all = padToOrgSize(
      [snap('zed-tpl-notime', { lastUsedAt: null, createdAt: null })],
      90,
    );
    expect(names(run(all))).not.toContain('zed-tpl-notime');
  });
...
describe('selectSnapshotsToReap — ppwarm LRU budget', () => {
    ...
    padToOrgSize(
      [
        ...Array.from({ length: ppwarmCount }, (_, i) =>
          ...
          snap(`zed-ppwarm-${i.toString(16).padStart(8, '0')}-tip`, {
            lastUsedAt: new Date(NOW - idleHours * 3_600_000).toISOString(),
          }),
        ),
        ...Array.from({ length: 12 }, (_, i) =>
          snap(`zed-default-${i}`, { lastUsedAt: ago(0) }),
        ),
      ],
      107,
    );
  ...
  it('evicts LRU ppwarm to reach target when no liveness rule can free anything', () => {
    ...
    expect(res.underPressure).toBe(true);
    expect(res.doomed.length).toBeGreaterThan(0);
    expect(res.doomed.every((d) => d.snapshot.name.startsWith('zed-ppwarm-'))).toBe(true);
  ...
  it('evicts oldest-first', () => {
    const all = padToOrgSize(
      [
        snap('zed-ppwarm-aaaaaaaa-tip', {
          lastUsedAt: new Date(NOW - 50 * 3_600_000).toISOString(),
        }),
        snap('zed-ppwarm-bbbbbbbb-tip', {
    ...
    const reaped = names(run(all));
    expect(reaped).toContain('zed-ppwarm-aaaaaaaa-tip');
    expect(reaped).not.toContain('zed-ppwarm-bbbbbbbb-tip');
  });
...
describe('selectSnapshotsToReap — FIX-K-lite pinned-image guard', () => {
  const ppw = (proj: string, hash: string, days: number, extra: Partial<SnapshotLike> = {}) =>
    snap(`zed-ppwarm-${proj}-${hash}`, { lastUsedAt: ago(days), createdAt: ago(days), ...extra });
