...
PGPASSWORD=postgres psql -h 127.0.0.1 -p 54322 -U postgres -d postgres \
  -c "SELECT column_name FROM information_schema.columns WHERE table_schema='zed' AND table_name='credit_accounts' AND column_name IN ('billing_model','seat_count','auto_topup_customized');"
# Expected: 3 rows
...
| --- | ------------------------------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A1  | Fresh signup lands on per-seat plan               | New user signs up via web UI                             | `credit_accounts` row: `tier='per_seat'`, `billing_model='per_seat'`, `seat_count=1`, `balance=0`, `stripe_subscription_id=NULL`                                            | `SELECT tier, billing_model, balance, stripe_subscription_id FROM zed.credit_accounts WHERE account_id='<accountId>';`                                                             |
...
| A5  | Webhook lands → $20 grant + seat 1                | (automatic)                                              | `balance=20`, `stripe_subscription_id` set, `seat_subscription_item_id` set, ledger has `seat_grant +$20` (and possibly `tier_grant +$20` from `upsertCreditAccount` path) | `SELECT balance, stripe_subscription_id, seat_subscription_item_id FROM zed.credit_accounts WHERE account_id='<id>'; SELECT * FROM zed.credit_ledger WHERE account_id='<id>' ORDER BY created_at DESC LIMIT 5;` |
| A6  | Owner gets YOLO token on subscribe                | (automatic, via `mintYoloTokensForAllMembers`)           | One active row in `yolo_member_tokens` for the owner                                                                                                                       | `SELECT user_id, token_prefix, revoked_at FROM zed.yolo_member_tokens WHERE account_id='<id>';`                                                                                    |
...
| A9  | Cancel subscription                                | UI cancel button (or `stripe subscriptions cancel`)     | Stripe sub status → `canceled`, local `tier` reverts to `'free'` (or whatever revertToFree does)                                                                          | `SELECT tier, stripe_subscription_status FROM zed.credit_accounts WHERE account_id='<id>';`                                                                                       |
...
| --- | ----------------------------------------------------- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| B1  | Add new email (not yet a user)                        | Owner invites `random123@yopmail.com` via UI                       | Email sent + `accountInvitations` row created, no `account_members` row yet, no seat sync yet                                                                                   | Check inbox via yopmail.com; `SELECT * FROM zed.account_invitations WHERE account_id='<id>';`                                                              |
| B2  | Accept invite                                         | Invitee clicks email link → signs up → accepts                    | `account_members` row added, `onMemberAdded` fires, Stripe quantity → 2, webhook lands, `seat_grant +$20`, balance → $40                                                        | `SELECT seat_count, balance FROM zed.credit_accounts WHERE account_id='<id>'; SELECT * FROM zed.account_members WHERE account_id='<id>';`              |
...
| B4  | Member gets YOLO token on add                         | (automatic)                                                       | Fresh `yolo_member_tokens` row for the new user, `revoked_at=NULL`                                                                                                              | `SELECT user_id, token_prefix FROM zed.yolo_member_tokens WHERE account_id='<id>' AND revoked_at IS NULL;`                                                 |
| B5  | Auto-topup defaults rescale on add                    | (automatic)                                                       | `auto_topup_threshold` = $5 × seat_count, `auto_topup_amount` = $20 × seat_count                                                                                                | `SELECT auto_topup_threshold, auto_topup_amount FROM zed.credit_accounts WHERE account_id='<id>';`                                                         |
| B6  | Auto-topup respects user customization                | Owner sets custom values, then adds a member                      | `auto_topup_customized=true` blocks rescale                                                                                                                                     | Set `UPDATE zed.credit_accounts SET auto_topup_customized=true, auto_topup_amount=100 WHERE account_id='<id>';` then add member → confirm auto_topup_amount stays 100 |
...
| #   | Scenario                                | Action                                                                                                          | Expected                                                                                                                       | Verify                                                                                                                                                                                                |
| --- | --------------------------------------- | --------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C1  | Start sandbox → metering row opens      | User opens a project session                                                                                    | `sandbox_compute_sessions` row with `state='active'`, captured spec, `started_at=now()`, `cost_usd=0`                          | `SELECT id, state, cpu_cores, memory_gb, started_at, cost_usd FROM zed.sandbox_compute_sessions WHERE sandbox_id='<sandboxId>';`                                                                   |
| C2  | Spec comes from `zed.yaml` `sandbox:` | Set `cpu=4 memory=8 disk=50` in `zed.yaml`'s `sandbox:` block, start session                                 | Row reflects 4/8/50; default 1/2/10 if no manifest                                                                             | Compare cpu_cores/memory_gb/disk_gb                                                                                                                                                                   |
| C3  | Hibernate after idle TTL                | Let session idle for `ZED_SANDBOX_IDLE_TTL` (default 1h, can be shortened with env var for testing)          | `pauseComputeSession` fires, row → `state='stopped'`, `ended_at` set, `cost_usd` matches formula, `compute_debit` in ledger    | `SELECT state, ended_at, cost_usd FROM zed.sandbox_compute_sessions WHERE id='<id>'; SELECT * FROM zed.credit_ledger WHERE type='compute_debit' AND account_id='<id>' ORDER BY created_at DESC LIMIT 1;` |
...
| C7  | Cron tick partial-bills long-running    | Wait for `runProjectMaintenance` (every 5 min) with a session running ≥ 1h                                       | `tickRunningComputeCharges` settles a partial window — `cost_usd` increases, `last_billed_at` advances, row stays open         | `SELECT cost_usd, last_billed_at FROM zed.sandbox_compute_sessions WHERE id='<id>';` before/after the tick                                                                                         |
...
| C9  | Multiple concurrent sandboxes           | Spin up 3 sandboxes in different projects under same account                                                     | All 3 have separate `sandbox_compute_sessions` rows; each debits independently on stop                                         | `SELECT sandbox_id, account_id, state FROM zed.sandbox_compute_sessions WHERE account_id='<id>' AND ended_at IS NULL;`                                                                             |
| C10 | Legacy account doesn't meter            | Flip an account to `billing_model='legacy'`, start a sandbox                                                     | NO `sandbox_compute_sessions` row created (gated by `isPerSeatAccount` check in `startComputeSession`)                          | `SELECT COUNT(*) FROM zed.sandbox_compute_sessions WHERE account_id='<legacyId>';` should stay 0                                                                                                   |
| C11 | Per-member attribution                  | Member B starts a sandbox in account A's project                                                                 | Row's `actor_user_id` = member B's user_id, `account_id` = A's account_id                                                      | `SELECT actor_user_id FROM zed.sandbox_compute_sessions WHERE sandbox_id='<id>';`                                                                                                                  |
...
| #   | Scenario                                   | Action                                                                  | Expected                                                                                                                            | Verify                                                                                                                  |
| --- | ------------------------------------------ | ----------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| D1  | Token minted at subscribe (owner)          | Owner subscribes                                                         | One `yolo_member_tokens` row, `revoked_at=NULL`                                                                                     | `SELECT * FROM zed.yolo_member_tokens WHERE account_id='<id>';`                                                       |
...
| D4  | Token revoked on member remove             | Remove a member                                                          | `revoked_at` set on their row                                                                                                       | `SELECT revoked_at FROM zed.yolo_member_tokens WHERE user_id='<userId>';`                                             |
...
| #  | Scenario                                  | Action                                                                                            | Expected                                                                                              | Verify                                                                                                                |
| -- | ----------------------------------------- | ------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| E1 | Topup triggers below threshold            | Set `auto_topup_enabled=true`, drain wallet to $4 (threshold = $5 for 1 seat)                     | Stripe charge for $20, balance back to ~$24                                                           | `SELECT balance, auto_topup_last_charged FROM zed.credit_accounts; stripe events list --limit 1`                   |
...
| E3 | No card on file                            | Disable saved payment method, drain wallet                                                        | Topup attempt fails gracefully, `payment_status='failed'` set, logged warning                          | `SELECT payment_status, last_payment_failure FROM zed.credit_accounts;`                                            |
...
| E5 | Defaults rescale with seats               | Add a 5th member → topup defaults should become $25 threshold / $100 amount                       | Auto-applied unless `auto_topup_customized=true`                                                       | `SELECT auto_topup_threshold, auto_topup_amount FROM zed.credit_accounts;`                                          |
...
| F5 | `subscription.deleted`                             | Cancel sub                                                                            | Account reverts to `tier='free'`, balance retained                                                      | `SELECT tier, stripe_subscription_status, balance FROM zed.credit_accounts;`        |
...
| G2 | Add member to legacy account           | Owner of legacy tier adds a member                                   | `account_members` row created, but NO seat sync to Stripe, NO YOLO token, NO grant                                                | `SELECT * FROM zed.yolo_member_tokens WHERE account_id='<legacyId>';` → 0 rows   |
...
| #  | Scenario                                       | Action                                                                              | Expected                                                                                                  | Verify                                                                                                         |
| -- | ---------------------------------------------- | ----------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| J1 | Race: simultaneous member adds                  | Two browsers add different members at the same second                               | Both succeed; final `seat_count=N+2`; Stripe converges to correct quantity                                | `SELECT seat_count FROM zed.credit_accounts; stripe subscriptions retrieve sub_*` (compare quantity)        |
...
| J6 | Deleted account → orphan cleanup                 | DELETE account                                                                      | `credit_accounts`, `sandbox_compute_sessions`, `yolo_member_tokens` all cascade via FK                     | `SELECT COUNT(*) FROM zed.sandbox_compute_sessions WHERE account_id='<deletedId>';` → 0                      |
...
SELECT scs.id FROM zed.sandbox_compute_sessions scs
LEFT JOIN zed.session_sandboxes ss ON ss.sandbox_id = scs.sandbox_id
WHERE ss.sandbox_id IS NULL AND scs.state = 'active';

-- K2: No YOLO tokens for non-members
SELECT yt.user_id FROM zed.yolo_member_tokens yt
LEFT JOIN zed.account_members am ON am.user_id = yt.user_id AND am.account_id = yt.account_id
...
FROM zed.credit_accounts ca
LEFT JOIN zed.account_members am ON am.account_id = ca.account_id
...
SELECT ca.account_id FROM zed.credit_accounts ca
WHERE ca.billing_model = 'per_seat'
  AND ca.stripe_subscription_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM zed.yolo_member_tokens yt
...
FROM zed.credit_accounts ca
LEFT JOIN zed.credit_ledger cl ON cl.account_id = ca.account_id
...
SELECT stripe_event_id, COUNT(*) FROM zed.credit_ledger
...
Before flipping `INTERNAL_ZED_ENV=prod`:

- [ ] Create prod `per_seat` Stripe price ($20/mo) under prod product
- [x] Prod per-seat price set: price_1TcrQJG6l1KZGqIry1K1cqZY (live "Zed seat" $20/mo)
...
- [ ] Verify `INTERNAL_ZED_ENV=prod` → `ensureSchema()` is a no-op (managed externally)
