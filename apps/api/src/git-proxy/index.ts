/**
 * Zed git smart-HTTP reverse proxy.
 *
 * The UNIVERSAL client-facing git origin for every git-backed project. Clients
 * (sandbox daemon, `zed` CLI, the user's git) clone/push
 *   https://<ZED_URL>/v1/git/<projectId>.git
 * authenticating with a Zed token (sandbox token / account API key / CLI
...
function unauthorized(c: any, message: string) {
  c.header('WWW-Authenticate', 'Basic realm="Zed Git"');
