/**
 * Live, self-contained e2e proof that the Zed git proxy works for EVERY git
 * backing, with the sandbox/CLI holding ONLY a Zed token and the origin
 * always the Zed proxy path. The proxy resolves the real upstream credential
 * server-side per case:
 *
 *   A. managed       — Zed-provisioned repo, App installation token (repo-scoped)
 ...
 * sandbox-scoped token (zed_sb_) and a project PAT (zed_pat_), and checks
 * the negative paths (no auth → 401, cross-project token → 403/404). Everything
 * it creates is tagged and torn down in `finally`.
 *
 * Prereqs: a running API with the git proxy (point ZED_URL at it), the Zed
 * App reachable, and a real private repo the App + the PAT can access.
 *
 *   ZED_URL=http://localhost:8009 \
 *   E2E_REPO=markokraemer/zed-proxy-e2e \
...
const ZED_URL = (process.env.ZED_URL || 'http://localhost:8009').replace(/\/$/, '');
const REPO = process.env.E2E_REPO || 'markokraemer/zed-proxy-e2e';
...
async function acct(): Promise<{ accountId: string; userId: string }> {
  const r: any = await db.execute(sql`select account_id, user_id from zed.account_members order by joined_at limit 1`);
...
async function mkProject(name: string, meta: Record<string, unknown>, conn: Record<string, unknown>, accountId: string): Promise<string> {
  ...
  const uniqueRepoUrl = `zed-proxy://e2e/${randomUUID()}`;
  const pr: any = await db.execute(sql`
    insert into zed.projects (account_id, name, repo_url, default_branch, manifest_path, status, metadata)
    values (${accountId}, ${name}, ${uniqueRepoUrl}, 'main', 'zed.yaml', 'active', ${JSON.stringify(meta)}::jsonb)
    returning project_id`);
  const projectId = (pr.rows ?? pr)[0].project_id;
  created.projects.push(projectId);
  await db.execute(sql`
    insert into zed.project_git_connections
...
async function mkPat(accountId: string, userId: string, projectId: string): Promise<string> {
  ...
  await db.execute(sql`
    insert into zed.account_tokens (account_id, user_id, project_id, name, public_key, secret_key_hash)
...
async function mkSandboxToken(accountId: string, projectId: string): Promise<string> {
  ...
  await db.execute(sql`
    insert into zed.session_sandboxes (sandbox_id, session_id, account_id, project_id, provider, external_id, status)
...
/** Clone the project through the proxy with a Zed token. Returns true on success. */
function cloneThroughProxy(projectId: string, token: string): { ok: boolean; detail: string } {
  ...
  try {
    const url = `${ZED_URL}/v1/git/${projectId}.git`;
    const b64 = Buffer.from(`x-access-token:${token}`).toString('base64');
    const host = new URL(ZED_URL).host;
    const scheme = new URL(ZED_URL).protocol.replace(':', '');
...
function httpCode(projectId: string, token: string | null): string {
  const url = `${ZED_URL}/v1/git/${projectId}.git/info/refs?service=git-upload-pack`;
...
async function main() {
  ...
  process.stdout.write(`\nproxy=${ZED_URL}  repo=${REPO}  install=${INSTALL_ID}\n\n`);
  ...
  await db.execute(sql`
    insert into zed.account_github_installations (account_id, installation_id, owner_login, owner_type, repository_selection)
  ...
  if (PAT) {
    await db.execute(sql`
      insert into zed.project_git_credentials (account_id, project_id, provider, auth_method, value_enc, created_by)
  ...
  for (const cs of cases) {
    ...
    cPat.ok ? ok(`clone via PAT (zed_pat_)`) : bad(`clone via PAT — ${cPat.detail}`);
    const cSb = cloneThroughProxy(cs.id, sb);
    cSb.ok ? ok(`clone via SANDBOX token (zed_sb_)`) : bad(`clone via SANDBOX token — ${cSb.detail}`);
...
async function cleanup() {
  for (const name of created.tokenNames) await db.execute(sql`delete from zed.account_tokens where name=${name}`).catch(() => {});
  await db.execute(sql`delete from zed.account_tokens where name like 'e2e-allcases-%'`).catch(() => {});
  for (const id of created.projects) {
    await db.execute(sql`delete from zed.project_git_credentials where project_id=${id}`).catch(() => {});
    await db.execute(sql`delete from zed.session_sandboxes where project_id=${id}`).catch(() => {});
    await db.execute(sql`delete from zed.project_git_connections where project_id=${id}`).catch(() => {});
    await db.execute(sql`delete from zed.projects where project_id=${id}`).catch(() => {});
  }
  if (created.installRow) await db.execute(sql`delete from zed.account_github_installations where installation_id=${INSTALL_ID}`).catch(() => {});
}
