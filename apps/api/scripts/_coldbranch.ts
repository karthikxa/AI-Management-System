...
for (let i = 0; i < 200; i++) { try { const j: any = await (await fetch(`${base}/zed/health`, { headers: H, signal: AbortSignal.timeout(3000) })).json(); if (j.runtimeReady) break; } catch {} await Bun.sleep(300); }
