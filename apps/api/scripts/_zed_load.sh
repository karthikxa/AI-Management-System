#!/usr/bin/env bash
# MAX zed load test — ramp concurrent agent sessions through the REAL zed flow
...
worker(){
  ...
  local ext="" st="" row
  while :; do row=$(psql "select external_id||'|'||status from zed.session_sandboxes where session_id='$sid' order by created_at desc limit 1;"); ext=${row%%|*}; st=${row##*|}; [ "$st" = active ] && [ -n "$ext" ] && break; [ $(( ($(nowms)-t0)/1000 )) -ge 240 ] && break; sleep 2; done
  [ -z "$ext" ] && { echo "ready=0 correct=0 fail=noactive" > "$out"; psql "delete from zed.session_sandboxes where session_id='$sid';">/dev/null 2>&1; return; }
  local ta=$(( $(nowms)-t0 ))
  local rok=0; while :; do curl -s -m6 "${HDR[@]}" "$BASE/v1/p/$ext/8000/zed/health" 2>/dev/null|grep -q '"runtimeReady":true' && { rok=1; break; }; [ $(( ($(nowms)-t0)/1000 )) -ge 240 ] && break; sleep 1; done
  local tr=$(( $(nowms)-t0 ))
...
echo "=== MAX ZED LOAD TEST (real agent flow, ramp until degradation) ==="
...
echo "=== sweep any leftover running sandboxes for the zed org ==="
