#!/usr/bin/env bash
# DEEP e2e of the REAL Zed runtime (default template = opencode + agent + suna stack) on PROD
...
echo "=== DEEP REAL-ZED e2e on Platinum — $ROUNDS rounds (DEFAULT template: opencode+agent) ==="
pass=0; agentok=0
for r in $(seq 1 "$ROUNDS"); do
  ...
  ext=""; while :; do row=$(psql "select external_id||'|'||status from zed.session_sandboxes where session_id='$sid' order by created_at desc limit 1;"); ext=${row%%|*}; [ "${row##*|}" = active ] && [ -n "$ext" ] && break; [ $(( ($(nowms)-t0)/1000 )) -ge 180 ] && break; sleep 0.3; done
  [ -z "$ext" ] && { echo "round $r: NO_SANDBOX"; psql "delete from zed.session_sandboxes where session_id='$sid';">/dev/null 2>&1; mint; continue; }
  ta=$(( $(nowms)-t0 ))
  rok=0; while :; do curl -s -m5 "${H[@]}" "$BASE/v1/p/$ext/8000/zed/health" 2>/dev/null | grep -q '"runtimeReady":true' && { rok=1; break; }; [ $(( ($(nowms)-t0)/1000 )) -ge 180 ] && break; sleep 0.3; done
  tr=$(( $(nowms)-t0 ))
...
echo "=== CONCURRENCY burst: 4 simultaneous real-zed sessions ==="
...
for j in 1 2 3 4; do
  ...
  ext=""; while :; do row=$(psql "select external_id||'|'||status from zed.session_sandboxes where session_id='$sid' order by created_at desc limit 1;"); ext=${row%%|*}; [ "${row##*|}" = active ] && [ -n "$ext" ] && break; [ $(( ($(nowms)-tb)/1000 )) -ge 220 ] && break; sleep 0.4; done
  CEXT[$j]=$ext
  if [ -n "$ext" ]; then rr=0; while :; do curl -s -m5 "${H[@]}" "$BASE/v1/p/$ext/8000/zed/health" 2>/dev/null|grep -q '"runtimeReady":true' && { rr=1; break; }; [ $(( ($(nowms)-tb)/1000 )) -ge 220 ] && break; sleep 0.4; done; [ "$rr" = 1 ] && okc=$((okc+1)); echo "  session $j: ext=$ext ready=$rr +$(( $(nowms)-tb ))ms"; fi
done
echo "  CONCURRENCY: $okc/4 reached runtimeReady"
for j in 1 2 3 4; do [ -n "${CEXT[$j]:-}" ] && curl -s -m15 -X DELETE "$PURL/v1/sandboxes/${CEXT[$j]}" -H "Authorization: Bearer $PK" >/dev/null 2>&1; [ -n "${CSID[$j]:-}" ] && psql "delete from zed.session_sandboxes where session_id='${CSID[$j]}';">/dev/null 2>&1; done
