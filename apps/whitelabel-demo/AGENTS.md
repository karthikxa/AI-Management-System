...
Lumen is the reference implementation for `@zed/sdk`.

## Required architecture

1. `src/lib/zed.ts` owns the only browser SDK client.
2. `useSession(projectId, sessionId)` owns the complete workbench session.
3. Client code imports only `@zed/sdk` and `@zed/sdk/react`.
4. Client code does not implement runtime transport logic.
5. Server Zed calls use `@zed/sdk/server`.
6. The `/api/zed` route delegates upstream forwarding to `forwardZedRequest()`.
...
- A raw Zed `fetch()` in client or server code.
...
Application tests use `createTestZed()` for Zed product flows. Raw HTTP
...
The `/api/zed` route calls `forwardZedRequest()` from
`@zed/sdk/server`. The SDK owns upstream authentication, request buffering,
...
- `src/lib/zed.ts`: One browser SDK client.
