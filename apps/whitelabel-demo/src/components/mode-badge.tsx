 ...
 *
 *   - BACKEND (wrapper / KaaB): the server holds ONE Zed key, every upstream
 *     call goes through this app's proxy, and local policy enforces project
 *     ownership and request limits.
 *   - DIRECT (SDK-only): the browser talks to Zed with a key the user pasted.
 *     There is no wrapper authentication or local project ownership policy.
 *
 * A missing `ZED_API_KEY` silently falls back to DIRECT, so the badge is also
...
export function ModeBadge({ wrapperMode }: { wrapperMode: boolean }) {
  ...
  const detail = wrapperMode
    ? 'This server holds the Zed key. Local policy applies authentication, project ownership, and request limits.'
    : 'The browser talks to Zed with a key you pasted. Wrapper authentication and local project ownership do not apply.';
