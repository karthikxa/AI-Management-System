...
interface AvatarProps extends ViewProps {
  ...
  /** Use Zed symbol instead of icon (for SUNA agent) */
  useZedSymbol?: boolean;
 ...
 * - Zed symbol: 50% of container (larger for brand recognition)
 * - Border: 1.5px solid
...
export function Avatar({
  ...
  const symbolSize = Math.round(size * 0.55); // 55% for Zed symbol (more prominent)
  ...
  const getDefaultColors = () => {
    ...
    // Zed symbol always uses solid black bg with white icon
    if (useZedSymbol) {
      return {
        bg: '#000000',
        icon: '#FFFFFF',
    ...
    <View 
      ...
      style={[
        { 
          ...
          borderWidth: useZedSymbol ? 0 : (showBorder ? 1.5 : 0),
