...
const OPENCODE_RUNTIME_ENV_NAMES = new Set([
  ...
  // Its content hash, echoed by /zed/health so a client can ask what this box
  // is really running. Pushed with the config; allowlisted so the two cannot
  // drift apart on a live update.
  'ZED_COMPILED_AGENT_CONFIG_ETAG',
...
function applyLlmGatewayMode(enabled: unknown, baseUrl: unknown, denyEnv: unknown): { changed: boolean; names: string[] } {
  ...
  if (!token) {
    throw new Error('ZED_CLI_TOKEN is unavailable; cannot enable LLM gateway in this running sandbox')
  }
  return setOpencodeRuntimeEnv({
    ZED_LLM_API_KEY: token,
    ZED_LLM_BASE_URL: baseUrl,
    // GATEWAY: restore the strip (names supplied by the API) on the next restart.
    ZED_OPENCODE_DENY_ENV: typeof denyEnv === 'string' ? denyEnv : '',
  })
}
...
export function createEnvRouter(
  ...
  router.post('/', async (c) => {
    ...
    // the proxy (which should already block /zed/env) — refuse it.
    if (c.req.header(ZED_USER_CONTEXT_HEADER)) {
      logger.warn('[env] rejecting /zed/env carrying user-context header')
