...
export interface PtyRegistry {
  list(): ZedPtyMeta[]
  create(opts: { command?: string; args?: string[]; cwd?: string; title?: string; env?: Record<string, string> }): ZedPtyMeta
  update(id: string, opts: { title?: string; size?: { rows: number; cols: number } }): ZedPtyMeta | null
...
export type AttachOrCreateResult =
  | { kind: 'attached'; meta: ZedPtyMeta; handle: PtyAttachHandle }
  | { kind: 'created'; meta: ZedPtyMeta; handle: PtyAttachHandle }
  | { kind: 'exited'; meta: ZedPtyMeta }
...
/**
 * `/zed/pty` — Zed's own PTY implementation, independent of whatever
 * agent runtime (OpenCode today) happens to be running. `/zed/*` is
 ...
 * verifies `X-Zed-User-Context` itself.
 */
export function createPtyRouter(cfg: Config, registry: PtyRegistry): Hono {
  ...
  app.use('*', async (c, next) => {
    ...
    const auth = verifyZedUserContext(c.req.header(ZED_USER_CONTEXT_HEADER), cfg.sandboxToken)
    if (!auth.ok) {
