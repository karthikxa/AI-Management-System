...
/**
 * Image-baked copy of the always-latest Zed system skills — `zed-cli`
 * (the front door) plus the managed `zed-*` family. Produced by the snapshot
 ...
 */
const BAKED_MANAGED_SKILLS_DIR = '/opt/zed/managed-skills'
...
/**
 * Always-inject the Zed system skills into the session's OpenCode skills dir.
 *
 * `zed-cli` (and the rest of the `zed-*` family) is the one thing Zed
 ...
 * projects from ever going stale on Zed internals.
 *
...
export async function ensureInjectedManagedSkills(
  ...
  try {
    ...
    if (injected > 0) {
      logger.info('[boot] injected managed zed skills', { configDir, from: bakedDir, injected })
    }
  } catch (err) {
