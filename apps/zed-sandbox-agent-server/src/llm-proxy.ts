...
// restore" mechanism). Two instances run when ZED_LLM_HOTSWAP=1:
//   • the LLM gateway proxy   (opencode's zed provider baseURL → here)
//   • the connector MCP proxy  (zed-connectors MCP's ZED_API_URL → here)
//
...
type ProxyState = {
  /** The real upstream base, e.g. https://gateway-dev.zed.com/v1/llm (LLM) or
   *  the real ZED_API_URL (connector). */
...
// ── instances ────────────────────────────────────────────────────────────────
const llm = createCredentialProxy('llm', 'zed-llm-proxy-injected')
const connector = createCredentialProxy('connector', 'zed-connectors-proxy-injected')
