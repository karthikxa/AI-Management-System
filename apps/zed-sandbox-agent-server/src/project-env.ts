...
function normalizeEnv(input: Record<string, unknown>): Record<string, string> {
  ...
  for (const [rawName, rawValue] of Object.entries(input)) {
    ...
    if (name.startsWith('ZED_')) continue
...
export function createProjectEnvStore(initialEnv: NodeJS.ProcessEnv = process.env): ProjectEnvStore {
  const initialRevision = initialEnv.ZED_PROJECT_SECRETS_REVISION?.trim()
  let revision: string | null = initialRevision || null
  let names = (initialEnv.ZED_PROJECT_SECRET_NAMES ?? '')
    .split(',')
    .map((name) => name.trim().toUpperCase())
    .filter((name) => SECRET_NAME_REGEX.test(name) && !name.startsWith('ZED_'))
    .sort()
