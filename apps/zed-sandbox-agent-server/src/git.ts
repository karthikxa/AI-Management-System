...
export function buildGitAuthArgs(
  ...
  // The Zed git proxy is the sandbox's own control-plane URL. During the
...
async function resolveCloneCredential(cfg: Config): Promise<CloneCredential | undefined> {
  ...
  // Universal proxy origin: when the repo is served by the Zed git proxy
  // (ZED_REPO_URL = `${ZED_URL}/v1/git/<projectId>.git`), the git
  // credential IS our own ZED_TOKEN — the proxy authenticates it and resolves
  ...
  // The control plane is reached over the public internet (ZED_API_URL).
  ...
  // exhaustion throw an error that names the URL so /zed/health explains the
  ...
  throw new Error(
    `could not reach the Zed control plane at ${base} to fetch the git clone ` +
    `credential after ${CLONE_CRED_ATTEMPTS} attempts — is ZED_API_URL publicly ` +
 ...
 * (`zed-agent git-credential`). When git needs a credential for the repo
...
/**
 * Git credential-helper entrypoint (`zed-agent git-credential <action>`).
...
async function checkoutLocalSessionBranch(target: string, branch: string): Promise<void> {
  ...
  // ZED_SESSION_FRESH and ZED_BASE_SHA have no producer left in apps/api,
...
async function initLocalRepoAtBase(cfg: Config, dir: string, base: string): Promise<void> {
  ...
  const commit = await execGit(
    ['-C', dir, 'commit', '--allow-empty', '-m', 'chore: initialize Zed project'],
...
async function createStagePath(target: string, kind: string): Promise<string> {
  ...
  return join(target, `.zed-${kind}-${process.pid}-${Date.now()}`)
}
...
/** Crash-safe enough for boot: a failure mid-swap leaves a `.zed-*` stage
 ...
 * branch. Ported from core/scripts/zed-daemon clone_project_if_requested.
 */
export async function materializeRepo(cfg: Config): Promise<void> {
  if (!cfg.repoUrl) {
    throw new Error('ZED_PROJECT_AUTO_CLONE is enabled but ZED_REPO_URL is unset')
  }
  ...
  if (await pathExists(`${target}/.git`)) {
    ...
    // project (baked HEAD == the server-resolved ZED_BASE_SHA). When it isn't
  ...
  }
  {
    ...
    // /opt/zed/scaffold.git whose root commit is SHARED with every project
    ...
    // ZED_CLONE_DEPTH in config.ts for the measurements. A remote that can't
...
const SCAFFOLD_REPO_PATH = '/opt/zed/scaffold.git'
...
async function tryScaffoldDeltaFetch(
  ...
  try {
    ...
    // project's base tip (resolved server-side, passed as ZED_BASE_SHA) IS
 ...
 * than it looks: `/zed/health` calls it on EVERY request (to compute
...
export async function refreshRepo(cfg: Config): Promise<{ before: RepoInfo; after: RepoInfo }> {
  ...
  // proposes. Treating it as a failure made `POST /zed/refresh` answer 500
 ...
 * `~/.config/zed-opencode.json` and absent from `/config` and `/agent`.
 *
