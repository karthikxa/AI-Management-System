...
describe('OpenCode canonical audit relay', () => {
  test('uses the sandbox credential and never the session PAT', () => {
    expect(
      auditRelayToken({
        ZED_SANDBOX_TOKEN: 'zed_sb_authoritative',
        ZED_TOKEN: 'zed_sb_legacy',
        ZED_CLI_TOKEN: 'zed_pat_must_not_be_used',
      }),
    ).toBe('zed_sb_authoritative');
    expect(
      auditRelayToken({
        ZED_SANDBOX_TOKEN: undefined,
        ZED_TOKEN: 'zed_sb_legacy',
        ZED_CLI_TOKEN: 'zed_pat_must_not_be_used',
      }),
    ).toBe('zed_sb_legacy');
  ...
  test('reloads a spool containing the real nested part, state, info, and time summaries', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'zed-audit-spool-real-shape-'));
  ...
  test('recovers an unsent redacted batch from the atomic spool after restart', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'zed-audit-spool-'));
    const spoolPath = join(directory, 'events.json');
    try {
  ...
  test('rejects a new event before replacing a full durable spool', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'zed-audit-spool-capacity-'));
    const spoolPath = join(directory, 'events.json');
    try {
  ...
  test('fails closed when a spool contains an unknown raw-content field', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'zed-audit-spool-invalid-'));
    const spoolPath = join(directory, 'events.json');
    try {
  ...
  test('recovers lineage after the session events were delivered before restart', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'zed-audit-spool-lineage-'));
    const spoolPath = join(directory, 'events.json');
    try {
