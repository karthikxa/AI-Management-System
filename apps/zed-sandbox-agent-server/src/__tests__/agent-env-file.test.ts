...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'zed-agentenv-'))
})
afterEach(() => {
...
describe('writeAgentEnvFile', () => {
  ...
  test('skips a value containing a NUL byte rather than breaking the file', () => {
    const store = createProjectEnvStore({
      ZED_PROJECT_SECRET_NAMES: 'GOOD,BADNUL',
      GOOD: 'ok',
  ...
  test('drops reserved/dangerous names even if they reach the store', () => {
    const store = createProjectEnvStore({
      ZED_PROJECT_SECRET_NAMES: 'GOOD,PATH,LD_PRELOAD,BASH_ENV',
      GOOD: 'ok',
      PATH: '/evil',
      LD_PRELOAD: '/evil.so',
      BASH_ENV: '/evil.sh',
  ...
  test('rotation + revocation: exports new value, unsets a removed boot secret', () => {
    const store = createProjectEnvStore({
      ZED_PROJECT_SECRET_NAMES: 'API_KEY,OLD',
      API_KEY: 'v1',
      OLD: 'gone-soon',
    } as NodeJS.ProcessEnv)
    const sh = shPath()
    ...
    writeAgentEnvFile(store, {
      sh,
      bootEnv: { ZED_PROJECT_SECRET_NAMES: 'API_KEY,OLD' } as NodeJS.ProcessEnv,
    })
  ...
  test('revokes a hot-added secret that was absent from the boot name list', () => {
    ...
    writeAgentEnvFile(store, {
      sh,
      bootEnv: { ZED_PROJECT_SECRET_NAMES: 'BOOT_SECRET' } as NodeJS.ProcessEnv,
    })
  ...
  test('emits the per-session cred allowlist (no-restart hot-swap fix) but NOT daemon-internal ZED_*', () => {
    ...
    writeAgentEnvFile(store, {
      ...
      bootEnv: {
        ...
        ZED_TOKEN: 'sk_tok',
        ZED_CLI_TOKEN: 'sk_cli',
        ZED_PROJECT_ID: 'proj_1',
        ZED_API_URL: 'https://dev-api.zed.com/v1',
        // daemon-internal — MUST NOT leak into the agent shell
        ZED_WARM_SEED: '1',
        ZED_LLM_PROXY_URL: 'http://127.0.0.1:4319',
        ZED_LLM_HOTSWAP: '1',
        ZED_LLM_API_KEY: 'internal-llm-key',
      } as NodeJS.ProcessEnv,
    })

    const body = readFileSync(sh, 'utf8')
    expect(body).toContain("export API_KEY='secret'") // user secret unchanged
    expect(body).toContain("export ZED_TOKEN='sk_tok'")
    expect(body).toContain("export ZED_CLI_TOKEN='sk_cli'")
    expect(body).toContain("export ZED_PROJECT_ID='proj_1'")
    expect(body).toContain("export ZED_API_URL='https://dev-api.zed.com/v1'")
    // daemon-internal stays filtered (not the agent's business)
    expect(body).not.toContain('ZED_WARM_SEED')
    expect(body).not.toContain('ZED_LLM_PROXY_URL')
    expect(body).not.toContain('ZED_LLM_HOTSWAP')
    expect(body).not.toContain('ZED_LLM_API_KEY')
