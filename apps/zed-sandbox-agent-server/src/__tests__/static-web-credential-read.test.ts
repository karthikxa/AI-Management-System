 ...
 *
 *   ~/.config/zed-opencode.json          the session's LLM gateway key and,
...
beforeAll(() => {
  ...
  scratch = mkdtempSync('/tmp/zed-staticweb-')
})
...
describe('the credential files are not readable', () => {
  ...
  const CREDENTIAL_PATHS = [
    '/home/zed/.config/zed-opencode.json',
    '/home/zed/.local/share/opencode/auth.json',
    '/root/.config/zed-opencode.json',
    '/home/zed/.ssh/id_ed25519',
    '/home/zed/.aws/credentials',
    '/home/zed/.netrc',
    '/home/zed/.git-credentials',
  ]
  ...
  test('a traversal back into a credential directory is refused', async () => {
    // `normalize()` collapses this to /home/zed/.config/... before the check,
    // which is why the deny list is matched on the normalized path.
    const { status } = await get(
      '/abs/workspace/../home/zed/.config/zed-opencode.json',
    )
    expect(status).toBe(403)
  })

  test('the whole /home root is gone, not just the credential paths', async () => {
    const { status } = await get('/abs/home/zed/notes.txt')
    expect(status).toBe(403)
  })
 ...
 * itself:
 *
 *   ln -s /home/zed/.config /workspace/x
 *   GET /abs/workspace/x/zed-opencode.json     ->  the session's PAT
 *
...
describe('symlinks cannot smuggle a path back out', () => {
  test('a link into a credential directory is refused', async () => {
    const fakeHome = mkdtempSync('/tmp/zed-fake-home-')
    mkdirSync(join(fakeHome, '.config'), { recursive: true })
    writeFileSync(join(fakeHome, '.config', 'zed-opencode.json'), '{"apiKey":"SECRET-KEY"}')
    ...
    const { status, body } = await get(`/abs${link}/zed-opencode.json`)
    expect(status).toBe(403)
  ...
  test('a link reaching outside every allowed root is refused', async () => {
    const outside = mkdtempSync(join(process.env.TMPDIR || '/var/tmp', 'zed-outside-'))
