 ...
 * `~/.config/zed-opencode.json` and absent from opencode's own `/config` and
...
import { ZED_SERVICE_CALL_HEADER } from '../zed-user-context'
import type { Opencode } from '../opencode'
import { createRefreshRouter } from '../routes/refresh'

const CONFIG_DIR = '.zed/opencode'
const AGENT = `${CONFIG_DIR}/agents/zed.md`
...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'zed-cfgdir-'))
 ...
 * cfg.baseSha) is dead: ZED_SESSION_FRESH and ZED_BASE_SHA have no
 ...
 * sandbox proxy, which blocks exactly one daemon path (`/zed/env`) and not
...
describe('base=1 requires a DIRECT service call', () => {
  ...
  test('the service-call header alone does not authorize it', async () => {
    ...
    const res = await post('/?base=1', { [ZED_SERVICE_CALL_HEADER]: '1' })
