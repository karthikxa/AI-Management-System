 ...
 * a healthy session permanently: `/zed/health` reported `starting` with
 * `opencode_pid: null` forever, every prompt 503'd, and the readiness probe just
 * re-marked `starting` on a loop. Meanwhile `/zed/env` had answered
 ...
 * then left it spinning instead of ended. `/zed/refresh` (restart on by
