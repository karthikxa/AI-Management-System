/**
 * `ZED_COMPILED_AGENT_CONFIG` must be accepted on the live-env route.
 *
...
describe('the live-env allowlist', () => {
  test('accepts the compiled agent config', () => {
    expect(runtimeEnvAllowlist()).toContain('ZED_COMPILED_AGENT_CONFIG')
  })
  ...
  test('still accepts the model — the precedent this follows', () => {
    ...
    expect(runtimeEnvAllowlist()).toContain('ZED_OPENCODE_MODEL')
  })

  test('the allowlist is a closed set, not a pattern over ZED_*', () => {
    // Anything reaching opencode's process env is reaching the agent. The gate
    // is an explicit list precisely so a new ZED_* name cannot be pushed into
