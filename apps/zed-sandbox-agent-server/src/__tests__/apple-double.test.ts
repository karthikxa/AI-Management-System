...
function poisonedConfigDir(): string {
  const dir = mkdtempSync(join(tmpdir(), 'zed-appledouble-'))
  dirs.push(dir)
  ...
  writeFileSync(join(dir, 'agents', 'zed.md'), '# zed\n')
  ...
  writeFileSync(join(dir, 'agents', '._zed.md'), sidecar)
...
describe('pruneAppleDoubleFiles — repairing a macOS-baked config dir', () => {
  test('removes every sidecar OpenCode would try to build, at any depth', async () => {
    ...
    expect(existsSync(join(dir, 'agents', '._zed.md'))).toBe(false)
  })
  ...
  test('leaves every real tool, agent, and config file untouched', async () => {
    ...
    expect(readFileSync(join(dir, 'agents', 'zed.md'), 'utf8')).toBe('# zed\n')
  })
  ...
  test('reports nothing removed for a clean dir and never throws on a missing one', async () => {
    const clean = mkdtempSync(join(tmpdir(), 'zed-appledouble-clean-'))
    dirs.push(clean)
