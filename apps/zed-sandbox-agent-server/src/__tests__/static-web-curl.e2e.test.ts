...
describe('static web server live curl e2e', () => {
  ...
  beforeAll(() => {
    ...
    root = mkdtempSync('/tmp/zed-static-e2e-')
  ...
  it('serves /open and injects a public <base> from X-Forwarded-Prefix', async () => {
    const prefix = 'https://api.zed.cloud/v1/p/sbx-123/3211'
  ...
  it('escapes malicious forwarded prefixes before injecting the <base> tag', async () => {
    const malicious = 'https://api.zed.cloud/v1/p/sbx-123/3211" autofocus onfocus="alert(1)'
    const res = await curl(`${base}/open?path=${encodeURIComponent(indexPath)}`, [
      '-H',
    ...
    expect(res.body).toContain(
      `<base href="https://api.zed.cloud/v1/p/sbx-123/3211%22%20autofocus%20onfocus=%22alert(1)/abs${siteDir}/">`,
    )
  })

  it('escapes malicious forwarded prefixes on the help page', async () => {
    const malicious = 'https://api.zed.cloud/v1/p/sbx-123/3211"><script>alert(1)</script>'
