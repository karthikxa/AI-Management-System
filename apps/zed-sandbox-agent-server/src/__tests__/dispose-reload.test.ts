 ...
 *   3. `POST /zed/services/system/reload` — which the SDK's `systemReload()`
...
describe('tryDisposeReload', () => {
  test('rewrites the config file BEFORE asking opencode to re-read it', () => {
    ...
    const wrote = body.indexOf('writeZedOpencodeConfig(')
  ...
  test('a deny-list change forces a respawn — dispose cannot re-run env shaping', () => {
    // ZED_OPENCODE_DENY_ENV is not IN the config file. `withoutDeniedProviderEnv`
    ...
    // The route used to inline `opencodeEnvNames.includes('ZED_OPENCODE_DENY_ENV')`.
...
describe('requiresRespawn', () => {
  ...
  test('the deny-list still forces one — dispose cannot re-shape the child env', () => {
    expect(requiresRespawn(['ZED_OPENCODE_DENY_ENV'])).toBe(true)
  })
