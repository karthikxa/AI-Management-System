...
const BASE_ENV = {
  ...
  ZED_SESSION_ID: 'sess-1',
  ZED_SANDBOX_TOKEN: 'sandbox-token-abc',
...
function setEnv(env: Record<string, string | undefined>) {
  for (const key of Object.keys(process.env)) {
    if (key.startsWith('ZED_')) delete process.env[key]
  }
...
describe('relayBootTimelineToApi', () => {
  test('POSTs the timeline to /v1/platform/boot-timeline with a bearer token', async () => {
    ...
    const call = calls[0]!
    expect(call.url).toBe('https://api.zed.test/v1/platform/boot-timeline')
  ...
  test('does not append a trailing /v1 twice when ZED_API_URL already ends in /v1', async () => {
    setEnv({ ...BASE_ENV, ZED_API_URL: 'https://api.zed.test/v1/' })
    const calls: string[] = []
    ...
    expect(calls).toEqual(['https://api.zed.test/v1/platform/boot-timeline'])
  })

  test('is a silent no-op when ZED_API_URL is unset (self-host / local dev with no control plane)', async () => {
    setEnv({ ...BASE_ENV, ZED_API_URL: undefined })
    const calls: string[] = []
    globalThis.fetch = (async (url: string) => {
      calls.push(String(url))
      return new Response('{}', { status: 200 })
  ...
  test('is a silent no-op when no credential is configured', async () => {
    setEnv({ ...BASE_ENV, ZED_SANDBOX_TOKEN: undefined, ZED_CLI_TOKEN: undefined, ZED_TOKEN: undefined })
    const calls: string[] = []
    globalThis.fetch = (async (url: string) => {
      calls.push(String(url))
      return new Response('{}', { status: 200 })
