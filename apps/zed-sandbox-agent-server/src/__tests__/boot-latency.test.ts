...
const BASE_ENV = { ZED_WORKSPACE: '/workspace', ZED_REPO_URL: 'https://example.test/r.git' }
...
async function makeOriginRepo(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'zed-origin-'))
...
describe('clone depth configuration', () => {
  ...
  test('depth 0 opts back into a full-history clone', () => {
    expect(loadConfig({ ...BASE_ENV, ZED_CLONE_DEPTH: '0' } as NodeJS.ProcessEnv).cloneDepth).toBe(0)
  })

  test('an explicit depth is honoured', () => {
    expect(loadConfig({ ...BASE_ENV, ZED_CLONE_DEPTH: '25' } as NodeJS.ProcessEnv).cloneDepth).toBe(25)
  })

  test('a negative depth is rejected rather than silently passed to git', () => {
    expect(() => loadConfig({ ...BASE_ENV, ZED_CLONE_DEPTH: '-1' } as NodeJS.ProcessEnv)).toThrow()
  })
})
...
describe('isShallowRepo', () => {
  test('reports true for a depth-limited clone and false once unshallowed', async () => {
    ...
    const clone = await mkdtemp(join(tmpdir(), 'zed-clone-'))
  ...
  test('reports false for a full clone', async () => {
    const origin = await makeOriginRepo()
    const clone = await mkdtemp(join(tmpdir(), 'zed-full-'))
    tempDirs.push(origin, clone)
    const target = join(clone, 'repo')
  ...
  test('a depth-1 clone carries exactly one commit while a full clone carries every commit', async () => {
    const origin = await makeOriginRepo()
    const clone = await mkdtemp(join(tmpdir(), 'zed-depths-'))
    tempDirs.push(origin, clone)
...
describe('scheduleHistoryBackfill', () => {
  test('restores full history for a shallow clone without blocking the caller', async () => {
    const origin = await makeOriginRepo()
    const clone = await mkdtemp(join(tmpdir(), 'zed-backfill-'))
    tempDirs.push(origin, clone)
    const target = join(clone, 'repo')
    git(clone, 'clone', '--depth', '1', '--branch', 'main', `file://${origin}`, target)

    const cfg = loadConfig({ ...BASE_ENV, ZED_REPO_URL: `file://${origin}` } as NodeJS.ProcessEnv)
  ...
  test('is a no-op on an already-complete repo', async () => {
    const origin = await makeOriginRepo()
    const clone = await mkdtemp(join(tmpdir(), 'zed-noop-'))
    tempDirs.push(origin, clone)
    const target = join(clone, 'repo')
    git(clone, 'clone', '--branch', 'main', `file://${origin}`, target)
...
describe('building the opencode boot config never touches the network', () => {
  ...
  const GATEWAY_ENV = {
    ZED_LLM_BASE_URL: 'https://gateway.zed.test/v1',
    ZED_LLM_API_KEY: 'k-test',
    ZED_API_URL: 'https://api.zed.test/v1',
  }

  test('the env under test really does engage the gateway path', () => {
    expect(hasZedLlmGateway(GATEWAY_ENV as NodeJS.ProcessEnv)).toBe(true)
  })
  ...
  test('falls back to the minimal model set rather than blocking, and says so', async () => {
    ...
    const raw = await buildOpencodeConfigContent(GATEWAY_ENV as NodeJS.ProcessEnv)
    const parsed = JSON.parse(raw!) as { provider: { zed: { models: Record<string, unknown> } } }

    expect(Object.keys(parsed.provider.zed.models)).toEqual(Object.keys(MINIMAL_FALLBACK_MODELS))
  })

  test('a catalog file on disk is used verbatim, still with no fetch', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'zed-catalog-'))
    tempDirs.push(dir)
    ...
    const raw = await buildOpencodeConfigContent({
      ...GATEWAY_ENV,
      ZED_LLM_CATALOG_FILE: file,
    } as NodeJS.ProcessEnv)
    const parsed = JSON.parse(raw!) as { provider: { zed: { models: Record<string, unknown> } } }

    expect(Object.keys(parsed.provider.zed.models)).toEqual(['test/only-model'])
    expect(calls).toEqual([])
  })

  test('catalogIsDegraded reports true with no file and false with one', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'zed-degraded-'))
    tempDirs.push(dir)
    const file = join(dir, 'catalog.json')
...
describe('catalog written to disk is rebuilt to a known shape, never passed through', () => {
  ...
  const GATEWAY = { ZED_LLM_BASE_URL: 'https://gw.zed.test/v1', ZED_LLM_API_KEY: 'k' }

  async function warmThenRead(catalog: unknown): Promise<Record<string, any> | null> {
    const dir = await mkdtemp(join(tmpdir(), 'zed-warm-'))
    tempDirs.push(dir)
    ...
    scheduleCatalogWarmToPathForTests(GATEWAY.ZED_LLM_BASE_URL, GATEWAY.ZED_LLM_API_KEY, target)
