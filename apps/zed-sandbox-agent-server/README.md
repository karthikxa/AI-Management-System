...
Thin sandbox-side daemon that runs inside every Zed project-session sandbox.

**Scope:**

1. Process supervisor for `opencode serve`.
2. Reverse proxy that fronts opencode's HTTP + SSE surface on
   `ZED_SERVICE_PORT` (default `8000`).
3. Managed Zed system-skill injection into OpenCode's native discovery
   directory.
4. Small Zed-namespaced control surface: `GET /zed/health` and
   `POST /zed/refresh`.
5. Static web server on `ZED_STATIC_PORT` (default `3211`) — serves any
...
2. Start the static web server on `0.0.0.0:ZED_STATIC_PORT` (in-process).
...
3. If `ZED_PROJECT_AUTO_CLONE=1`, `git clone` the project repo to
   `/workspace/.zed` and check out the requested branch. Failures are
   logged but non-fatal — the daemon still serves `/zed/health`.
4. Inject managed system skills into `.zed/opencode/skills`.
...
7. Start the Hono proxy on `0.0.0.0:ZED_SERVICE_PORT`.
...
| Path             | Purpose                                                                  |
| ---------------- | ------------------------------------------------------------------------ |
| `GET /zed/health` | Daemon liveness + opencode state + repo info (always 200 from daemon) |
| `POST /zed/refresh` | Signed-context protected repo fast-forward + opencode restart.     |
| `/*`             | Reverse-proxied to opencode. 503 while `opencode !== 'ok'`.              |

### `GET /zed/health` response shape
...
- `repo`, `branch`, `commit_sha` come from `git` in `ZED_PROJECT_TARGET` and
  are `null` when no repo has been materialized.

### `POST /zed/refresh`

Requires a valid `X-Zed-User-Context` signed with `ZED_TOKEN`. On success,
...
- **Secrets** — cloud API decides which secrets a sandbox needs and sets them
  ...
  The daemon does not read them and has no `/zed/secrets` route.
...
ZED_PROJECT_TARGET=/workspace/.zed
ZED_DEFAULT_BRANCH=main
ZED_BRANCH_FETCH_ATTEMPTS=60
ZED_BRANCH_FETCH_DELAY=0.25
ZED_DEFAULT_OPENCODE_CONFIG_DIR=/ephemeral/zed-master/opencode
...
Produces `dist/zed-agent` — a single-file Bun binary targeting the current
...
```
ZED_PROJECT_AUTO_CLONE=0 ZED_SERVICE_PORT=9999 bun run src/main.ts
curl -s http://localhost:9999/zed/health
```
