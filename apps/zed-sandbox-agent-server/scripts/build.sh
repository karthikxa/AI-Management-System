...
compile_with_retry() {
  ...
  while true; do
    if bun build --compile --target="$target" --outfile=dist/zed-agent src/main.ts; then
      return 0
    fi
...
compile_with_retry
chmod +x dist/zed-agent
size="$(stat -f%z dist/zed-agent 2>/dev/null || stat -c%s dist/zed-agent)"
echo "Built dist/zed-agent for ${target} (${size} bytes)"
