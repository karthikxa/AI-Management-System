...
A tiny, dependency-free tool that turns the official Zed brand SVGs into
...
The original goal was to replace the hand-typed `ZED` block letters in the
...
- `zed-wordmark.txt` — the canonical `ZED` figlet wordmark (source input).
...
```bash
# from apps/web/
node playground/zed-ascii/generate.mjs            # print the catalog to stdout
node playground/zed-ascii/generate.mjs --write     # (re)generate ascii-logo.md + .ts
```
