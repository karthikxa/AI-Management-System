...
> **Auto-generated** by `apps/web/playground/zed-ascii/generate.mjs` from the official
...
## Full logomark — symbol + "Zed" (100% rasterized SVG)  ★ (the CLI banner)

`LOGOMARK_HALF_L` is what `zed` prints on startup — the official lockup straight from the brand file, one consistent half-block style. Only half-block / shaded are kept (solid renders the wordmark too rough at terminal size).
...
_Regenerate: `node apps/web/playground/zed-ascii/generate.mjs --write`._
