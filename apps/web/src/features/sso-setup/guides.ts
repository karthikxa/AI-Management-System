 ...
 *  - 'import'         — the inline "connect to Zed" form (metadata import)
 ...
 * (from the zedssotest test tenants) under /public/sso-setup/<provider>/ —
...
export interface GuideStep {
   ...
   *  so an admin never has to guess whether a step means the Zed
   ...
   *  Zed → Provisioning → Users and groups" — rendered as a breadcrumb
...
export interface ProviderConfig {
  /** Attribute name the IdP emits for groups — Zed's group_claim_name MUST equal it. */
  ...
  /** SCIM guides: when the IdP pushes changes to Zed — shown next to the
...
const importStep = (claimHint: string): GuideStep => ({
  id: 'connect',
  title: 'Connect to Zed',
  kind: 'import',
  where: 'zed',
  intro:
    'The federation metadata you captured earlier is prefilled below. Zed registers your IdP and routes sign-ins for your email domain through it.',
...
const testStep = (
  ...
  bullets: [
    ...
    'On success the user lands in Zed and appears under Members on the account’s Identity page.',
    'Groups: if you left “Auto-provision groups” ON at the connect step (the default), your IdP groups appear automatically under Groups — just grant each one a project role. If you turned it off, map them yourself on the Identity page → SAML SSO card → “Group mappings” (IdP group name/ID → Zed group).',
...
export const PROVIDER_GUIDES: ProviderGuide[] = [
    ...
    steps: [
      {
        ...
        content: [
          ...
          {
            kind: 'text',
            text: 'Enter an appropriate app name, such as "Zed". Select the "Integrate any other application you don\'t find in the gallery (Non-gallery)" option. Click "Create".',
          },
          {
            kind: 'image',
            ...
            schematic: {
              ...
              rows: [
                { label: "What's the name of your app?", value: 'Zed', as: 'field' },
                {
                  label:
      ...
      },
      {
        ...
        content: [
          ...
          {
            kind: 'text',
            text: 'Copy the "Identifier (Entity ID)" and the "Reply URL (Assertion Consumer Service URL)" below and paste them into the "Basic SAML Configuration" panel — mark the Identifier as Default, and set "Sign on URL" to your Zed sign-in page. Leave Relay State and Logout URL empty. Click "Save" and close the edit panel.',
          },
        ...
        note: 'After Save, Entra pops its own "Test single sign-on with Zed?" dialog — choose "No, I\'ll test later". Zed isn\'t connected yet; the guided test comes at the last step.',
      ...
      },
      {
        ...
        content: [
          ...
          {
            kind: 'text',
            text: 'Click "Add user/group", click "None Selected" under Users and groups, select the users or groups that should sign in to Zed, click "Select", then click "Assign".',
          },
          {
            kind: 'image',
      ...
      importStep('memberOf'),
      testStep({
        extra: 'Removed from the Entra group → the mapped Zed access is gone on next sign-in.',
      }),
    ],
  },
  {
    ...
    steps: [
      {
        ...
        content: [
          ...
          {
            kind: 'text',
            text: 'The "Create SAML Integration" wizard opens. On the "General Settings" step, enter an appropriate app name, such as "Zed" — optionally upload an app logo. Click "Next".',
          },
          {
            kind: 'image',
            ...
            schematic: {
              ...
              rows: [
                { label: 'App name', value: 'Zed', as: 'field' },
        ...
        note: 'On the wizard\'s last step ("Help Okta Support understand how you configured this application"), select "This is an internal app that we have created" and click "Finish" — it\'s just Okta\'s own telemetry question, not a Zed setting.',
      ...
      },
      {
        ...
        content: [
          ...
          {
            kind: 'text',
            text: 'Check "Use this for Recipient URL and Destination URL". Set "Name ID format" to EmailAddress and "Application username" to Email — Zed matches accounts by email.',
          },
          {
            kind: 'image',
      ...
      },
      {
        ...
        note: 'Okta sends the matching groups by NAME — those names are what you map in Zed. The attribute name (groups) is what Zed reads as the group claim.',
    ...
    steps: [
      {
        ...
        content: [
          ...
          {
            kind: 'text',
            text: 'Enter an app name, such as "Zed" — optionally upload an app icon. Click "Continue".',
          },
          {
            kind: 'image',
      ...
      },
      {
        ...
        note: "Google only offers the XML download — there is no hosted metadata URL. Come back to re-download it if you change the app's configuration later; Zed reads whatever is in the file at import time.",
      ...
      },
      {
        ...
        warning:
          'Google only sends groups you EXPLICITLY select here (max 75). Add every group you plan to map in Zed — an unselected group is silently omitted from the claim.',
      },
      {
        id: 'assign-users',
        where: 'idp',
  ...
  },
  {
    ...
    config: {
      groupClaimName: 'groups',
      groupValueHint:
        'Cloudflare forwards the upstream IdP’s group NAMES on a "groups" SAML attribute (sent automatically for Okta, Entra ID, Google Workspace, and GitHub) — map those names in Zed.',
      preferredMetadata: 'url',
      metadataSource:
    ...
    steps: [
      {
        ...
        intro:
          'Cloudflare Access sits BETWEEN Zed and your real identity provider: it authenticates users against your IdP, then presents itself to Zed as a SAML IdP. So set up the upstream connection first — in Zero Trust → Settings → Authentication, add a login method (Okta, Entra, Google, …) per Cloudflare’s docs.',
        bullets: [
          'This guide covers the DOWNSTREAM half (Cloudflare → Zed). The upstream half (your IdP → Cloudflare) follows Cloudflare’s own documentation for your provider.',
          'Whatever attributes Zed needs (email, first/last name, and groups) must survive the upstream hop — Cloudflare forwards them on.',
        ],
      ...
      },
      {
        ...
        content: [
          ...
          {
            kind: 'text',
            text: 'Choose the "SaaS" application type, then select SAML (not OIDC) as the protocol. Give it a name such as "Zed".',
          },
          {
            kind: 'image',
      ...
      },
      {
        ...
        intro:
          'You configure everything in Cloudflare’s single “Add an application” wizard (the Configuration / Authentication / Policies / Overview tabs only appear when you EDIT the app later). First, paste Zed’s service-provider values into Cloudflare’s fields.',
        content: [
          {
            kind: 'sp-values',
          ...
          {
            kind: 'text',
            text: 'Set "Name ID format" to Email — Zed correlates accounts by email address.',
          },
        ],
      ...
      },
      {
        ...
        intro:
          'Cloudflare Access passes email by default. Add the other attributes Zed reads — id, firstName, lastName — as "SAML attribute statements": each is a Name plus the upstream IdP claim it maps to (a dropdown of your login method’s claims).',
        content: [
          {
            kind: 'claims-table',
            rows: [
      ...
      },
      {
        ...
        note: 'The attribute name (groups) must match Zed’s group claim, which is prefilled at the connect step. Cloudflare passes through whatever group NAMES the upstream IdP sends.',
  ...
  },
  {
    ...
    config: {
      groupClaimName: 'groups',
      groupValueHint:
        'OneLogin sends the user’s Role names on the "groups" parameter — map those names in Zed. The parameter MUST be flagged multi-value, or OneLogin collapses every role into one string.',
      preferredMetadata: 'url',
      metadataSource:
    ...
    steps: [
      {
        ...
        intro:
          'In the OneLogin admin console: Applications → Applications → "Add App" (top-right) → search the catalog for "SAML Custom Connector (Advanced)" → select it → set the Display Name to "Zed" → Save.',
        content: [
          {
            kind: 'image',
            ...
            schematic: {
              ...
              rows: [
                ...
                { label: 'Display Name', value: 'Zed', as: 'field' },
                { label: 'Save', as: 'button' },
              ],
            },
          },
      ...
      },
      {
        ...
        intro:
          'On the Configuration tab, paste Zed’s values. Note OneLogin uses several fields for the ACS URL.',
        content: [
          {
            kind: 'sp-values',
          ...
          {
            kind: 'text',
            text: 'Put the ACS URL in both "Recipient" and "ACS (Consumer) URL". The "ACS (Consumer) URL Validator" is a REGEX field, not a plain URL: take the ACS URL above, escape every dot (. becomes \\.), and anchor it with ^ … $. For example, if the ACS URL is https://api.zed.com/auth/v1/sso/saml/acs, paste ^https:\\/\\/api\\.zed\\.com\\/auth\\/v1\\/sso\\/saml\\/acs$ — a pattern that doesn’t match the exact ACS URL makes the sign-in fail with no clear error.',
          },
        ],
        warning:
      ...
      },
      {
        ...
        note: 'The parameter name (groups) must match Zed’s group claim, prefilled at connect.',
      ...
      },
      {
        ...
        content: [
          {
            kind: 'text',
            text: 'For a single tester: Users → open the test user → Applications tab → "+" → add "Zed". For a team: Users → Roles → create or edit a Role that includes the Zed app, then add members to that Role.',
          },
        ],
    ...
    steps: [
      {
        ...
        intro:
          'In the JumpCloud admin console: Access → SSO Applications → "+ Add New Application" → "Custom Application" (search "Custom SAML App") → "Configure SSO with SAML" → name it "Zed". This opens the app’s SSO tab.',
        content: [
          {
            kind: 'image',
      ...
      },
      {
        ...
        intro: 'On the SSO tab, paste Zed’s ACS URL and Entity ID into the two SP fields.',
        content: [
          {
            kind: 'sp-values',
            acsLabel: 'ACS URL',
          ...
          {
            kind: 'text',
            text: 'Paste the Zed Entity ID into "SP Entity ID" ONLY. Leave "IdP Entity ID" as the value JumpCloud pre-populates — that is JumpCloud’s own identifier and it flows into the exported metadata for you. Then check "Sign Assertion".',
          },
        ],
      ...
      },
      {
        ...
        intro:
          'Under "User Attributes", add each Service-Provider-Attribute-Name → JumpCloud-Attribute-Name pair. The left column is the SAML claim Zed receives; the right is the JumpCloud user field.',
        content: [
          {
            kind: 'claims-table',
            rows: [
      ...
      },
      {
        ...
        note: 'JumpCloud only sends groups the application is BOUND to (User Groups tab). Bind each group you plan to map in Zed; the name must match the connect-step claim.',
      ...
      },
      {
        ...
        warning:
          'Order matters: tick "Declare Redirect Endpoint" and Save BEFORE copying metadata — copying before Save yields metadata with the wrong binding or a missing certificate, and Zed silently gets the wrong SSO endpoint.',
    ...
    steps: [
      {
        ...
        intro:
          'In the PingOne admin console: Connections → Applications → open the app for Zed, or click "+" to add one: enter the name "Zed", choose the "SAML Application" type (not OIDC/SPA/Worker/Native), then Configure.',
        content: [
          {
            kind: 'image',
            ...
            schematic: {
              ...
              rows: [
                { label: 'Application name', value: 'Zed', as: 'field' },
      ...
      },
      {
        ...
        note: 'Zed’s Entity ID IS the SP metadata endpoint, so importing it fills ACS + Audience for you. Pick "Manually Enter" instead and there is no "SP Metadata URL" field — you’d have to type the ACS URL and Entity ID by hand.',
      ...
      },
      {
        ...
        intro:
          'In "Attribute Mapping", first set the SAML Subject (the "saml_subject" row / Name ID) to "Email Address" — it defaults to "User ID" (a GUID), but Zed correlates accounts by email. Then add these outgoing SAML attributes (Zed name → PingOne source):',
        content: [
          {
            kind: 'claims-table',
            rows: [
        ...
        note: 'The saml_subject / Name ID defaults to a GUID — set it to Email Address (format urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress) so the subject matches the email Zed keys on, belt-and-braces with the email attribute.',
    ...
    steps: [
      {
        ...
        intro:
          'In the Auth0 dashboard: Applications → Applications → open (or create) the application for Zed. Creating a new one? Choose "Regular Web Application" — the SAML2 addon works regardless of type, but this avoids second-guessing the picker.',
        content: [
          {
            kind: 'image',
      ...
      },
      {
        ...
        intro:
          'In the addon’s Settings modal, paste Zed’s ACS URL into "Application Callback URL". The Entity ID is NOT a form field — it goes inside the JSON.',
        content: [
          {
            kind: 'sp-values',
          ...
          {
            kind: 'text',
            text: 'In the "Settings" JSON object below the callback field, set TWO things: (1) "audience" = Zed’s Entity ID, and (2) force the NameID to the user’s email — it defaults to the opaque Auth0 user_id (auth0|…), which Zed can’t correlate. Add: "audience": "…/saml/metadata", "nameIdentifierFormat": "urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress", "nameIdentifierProbes": ["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress"]. Then scroll down and click "Enable".',
          },
          {
            kind: 'image',
      ...
      },
      {
        ...
        note: 'Groups: the default SAML2 Web App addon sends only email + name. To sync groups, add an Auth0 Action/Rule that emits a claim NAMED exactly "groups" (matching the connect-step claim). Auth0’s built-in group attribute URI "http://schemas.xmlsoap.org/claims/Group" will NOT match — map it to "groups". Then map those names in Zed.',
        doneLabel: 'I’ve added the identity provider metadata',
      },
      importStep('groups'),
      testStep(),
  ...
  },
  {
    ...
    config: {
      groupClaimName: 'groups',
      groupValueHint:
        'Group values arrive exactly as your IdP emits them (names or IDs) — create Zed mappings from what actually arrives.',
      preferredMetadata: 'url',
    ...
    steps: [
      {
        id: 'basic-saml',
        title: 'Register Zed in your IdP',
        intro:
...
const scimTestStep = (
  ...
  where: 'idp',
  intro:
    'Back in Zed, watch the live status below while you push or wait for the sync — no need to tab back and forth to check.',
...
export const SCIM_PROVIDER_GUIDES: ProviderGuide[] = [
  {
    ...
    config: {
      groupClaimName: 'memberOf',
      groupValueHint:
        'Groups pushed via SCIM are created in Zed under their Entra display names.',
      syncCadenceHint:
    ...
    steps: [
      ...
      scimConnectStep({
        ...
        content: [
          ...
          {
            kind: 'text',
            text: 'Expand "Mappings" → "Provision Microsoft Entra ID Users". The one row that matters: "userName" must map to source attribute "user.userprincipalname" — that is how Zed matches the SCIM user to a Zed account. Leave the default "objectId → externalId" mapping as-is (that\'s how Entra recognizes a record it already pushed on later syncs) and leave the rest at their defaults.',
          },
          {
            kind: 'image',
          ...
          {
            kind: 'text',
            text: '"Sync only assigned users and groups" makes this app\'s Users and groups list your allowlist: roll out team-by-team, and unassigning someone removes their Zed access. "Sync all users and groups" gives every person in your Entra tenant a Zed account — fine for a small or dedicated tenant, rarely what a company tenant wants on day one.',
          },
          {
            kind: 'image',
        ...
        warning:
          '#1 failure mode: Test Connection fails. Almost always a hand-typed or truncated Tenant URL — re-copy it exactly from above (it is not the regular Zed API URL and has no /v1 suffix). Assigning a whole GROUP (rather than individual users) needs Entra ID P1/P2; on Free, assign users one at a time.',
  ...
  },
  {
    ...
    config: {
      groupClaimName: 'groups',
      groupValueHint: 'Groups pushed via Push Groups are created in Zed under their Okta names.',
      syncCadenceHint:
    ...
    steps: [
      ...
      },
      {
        ...
        content: [
          ...
          {
            kind: 'text',
            text: '"Assign to People/Groups" makes the Assignments tab your allowlist: roll out team-by-team, and unassigning someone removes their Zed access. There is no "sync everyone" toggle in Okta the way Entra has one — Assignments IS the scope, always.',
          },
        ],
  ...
  },
  {
    ...
    config: {
      groupClaimName: 'groups',
      groupValueHint:
        'Groups pushed from OneLogin Rules are created in Zed under their OneLogin names.',
      syncCadenceHint:
    ...
    steps: [
      {
        ...
        where: 'idp',
        intro:
          'OneLogin pushes users and groups to Zed with its "SCIM Provisioner with SAML" connector — a SEPARATE app from the SAML-only connector. Outbound provisioning is a paid OneLogin tier; the Provisioning tab only appears when your plan includes it.',
        bullets: [
      ...
      scimConnectStep({
        ...
        content: [
          {
            kind: 'text',
            text: 'Applications → Applications → "Add App" → search "SCIM" → pick "SCIM Provisioner with SAML (SCIM v2 Core)" → name it "Zed" → Save. Then open the app’s "Configuration" tab.',
          },
          {
            kind: 'schematic',
            schematic: {
          ...
          {
            kind: 'text',
            text: 'On the "Parameters" tab, set the "SCIM Username" parameter’s value to the user’s Email — that makes SCIM userName the email Zed correlates on. Leave the default externalId mapping as-is.',
          },
          {
            kind: 'text',
            text: 'On the "Provisioning" tab, tick "Enable provisioning". Then UNCHECK "Require admin approval before this action is performed" for Create, Update, and Delete — otherwise every change waits in a pending queue and nothing reaches Zed until you approve it by hand.',
          },
          {
            kind: 'schematic',
            schematic: {
        ...
        warning:
          '#1 OneLogin gotcha: users seem to sync but nothing lands in Zed — the actions are stuck in the Provisioning "pending" queue because "Require admin approval" is still checked. Uncheck it for Create/Update/Delete (or approve the queue).',
      ...
      }),
      {
        ...
        content: [
          {
            kind: 'text',
            text: 'Assign the app: Users → open a user → "Applications" → "+" → add "Zed" (or assign the app to a Role so everyone in that Role is provisioned).',
          },
          {
            kind: 'text',
            text: 'Push groups: on the app’s "Provisioning" tab, under "Entitlements", click "Refresh" so Zed’s groups load. Then on the "Rules" tab add a Rule — a condition (e.g. member of a OneLogin Role) with the action "Set Groups in Zed" → the group.',
          },
          {
            kind: 'text',
        ...
        note: 'Only assigned users are provisioned; users created directly in Zed are not linked back to OneLogin.',
  ...
  },
  {
    ...
    config: {
      groupClaimName: 'groups',
      groupValueHint:
        'The JumpCloud user groups you bind to the app are created in Zed under their JumpCloud names.',
      syncCadenceHint:
    ...
    steps: [
      {
        ...
        where: 'idp',
        intro:
          'JumpCloud pushes users and groups to Zed from a "Custom Application" using its Identity Management (SCIM) tab. Provisioning needs the JumpCloud SSO entitlement.',
        bullets: [
      ...
      scimConnectStep({
        ...
        content: [
          ...
          {
            kind: 'text',
            text: 'Paste the Tenant URL into "Base URL" and the secret into "Token Key" (auth is HTTP Header → Authorization: Bearer). Enter a FRESH test-user email that does NOT already exist in Zed, click "Test Connection", then click "Activate" — do NOT click Save during the test-user step or you lose the configuration.',
          },
          {
            kind: 'text',
            text: 'Under "Export Attribute Mapping", confirm the user’s email flows into SCIM "userName" — JumpCloud sets this by default, so there’s usually nothing to change. Zed correlates on that email.',
          },
        ],
        success:
          'Test Connection passes with a fresh test email, and the app’s Identity Management shows Activated.',
        warning:
          '#1 JumpCloud gotcha: Test Connection fails because the test-user email already exists in Zed — it must be a brand-new address. (And click "Activate", not "Save", during that step.)',
      ...
      }),
      {
        ...
        content: [
          {
            kind: 'text',
            text: 'On the "Identity Management" tab itself (once Test Connection has succeeded), check "Enable management of User Groups and Group Membership in this application" so bound groups (and their members) are pushed to Zed.',
          },
          {
            kind: 'text',
  ...
  },
  {
    ...
    config: {
      groupClaimName: 'groups',
      groupValueHint:
        'The internal PingOne groups you select on the provisioning rule are created in Zed under their PingOne names.',
      syncCadenceHint:
    ...
    steps: [
      {
        ...
        where: 'idp',
        intro:
          'PingOne pushes users and groups to Zed through a generic "SCIM Outbound" connection under Integrations → Provisioning. Use the modern PingOne cloud console (Workforce) — the legacy "PingOne for Enterprise" product does not have this.',
        bullets: [
          'Connect SAML SSO first so provisioned users can sign in.',
      ...
      scimConnectStep({
        ...
        content: [
          {
            kind: 'text',
            text: 'Integrations → Provisioning → "+ New Connection" → on the "Identity Store" line click "Select" → choose the "SCIM Outbound" tile → "Select". Name it "Zed", then "Configure Authentication".',
          },
          {
            kind: 'schematic',
            schematic: {
          ...
          {
            kind: 'text',
            text: 'Set userName to the email Zed correlates on: open the "Attribute Mapping" section (separate from the auth screen), in the "PingOne Directory" column expand "Username" and select "Email Address". Then in the connection’s preferences/actions set "User Identifier" = userName and "User Filter Expression" = `userName eq "%s"`. Getting this wrong is the #1 PingOne failure — it defaults to the internal username, not the email.',
          },
        ],
        success:
  ...
  },
  {
    ...
    config: {
      groupClaimName: 'groups',
      groupValueHint: 'Pushed groups are created in Zed under their displayName.',
      syncCadenceHint:
    ...
    steps: [
      scimConnectStep({
        title: 'Mint a token & point your IdP at Zed',
        content: [
          {
            kind: 'text',
          ...
          {
            kind: 'text',
            text: 'Set the matching attribute so "userName" is the user’s email — that is how Zed correlates a SCIM user to an account.',
          },
        ],
        note: 'Zed supports SCIM 2.0 Users + Groups, PATCH, and `attribute eq "value"` filters. Bulk operations are not supported.',
      }),
      scimTestStep(),
    ],
  },
