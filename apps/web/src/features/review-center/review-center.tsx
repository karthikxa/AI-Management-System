...
function AnimatedCount({ value }: { value: number }) {
  // `useReducedMotion()` returns `null` on the server but a boolean on the
  // client (true when the OS asks for reduced motion). Branching the tree on
  // it directly would hydrate the animated variant server-side and the plain
  // variant client-side — a tree-shape mismatch. Gate on `mounted` so the
  // first client render matches SSR, then settle into the real preference.
...
export function ReviewCenter({
  ...
  // using it directly would make `initial` (and the whole row's opacity) differ
  // between SSR and the first client render — a hydration mismatch (see the
  // `AnimatedCount` comment for the same pattern). Treat motion as enabled
  // until mounted, then settle into the OS preference.
  const reduceMotion = useReducedMotion() ?? false;
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);
  const reduce = mounted ? reduceMotion : false;
