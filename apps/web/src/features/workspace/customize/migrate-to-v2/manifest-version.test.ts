...
describe('detectManifestVersion', () => {
  test('TOML v1 manifest ("zed_version = 1")', () => {
    expect(detectManifestVersion('zed_version = 1\n\n[project]\nname = "x"\n')).toBe(1);
  });

  test('YAML v2 manifest ("zed_version: 2")', () => {
    expect(detectManifestVersion('zed_version: 2\ndefault_agent: zed\n')).toBe(2);
  });
