 ...
 * Zed-specific split. Migration therefore touches ONLY governance
 * (zed.toml's `[[agents]]` → zed.yaml's `agents:` map); every agent's
 ...
 * canonical JSON Schema (`zed schema --version 2` in the sandbox, or the
 * published zed.com/schema documents) — so the agent never has to guess at
 * the target shape and `zed validate` passes on the first try.
 *
 ...
 */
export const MIGRATE_TO_V2_PROMPT = `Migrate this project's manifest from zed_version 1 (zed.toml) to zed_version 2 (zed.yaml). Read everything first, then make the change, then land it as a change request — do not merge it yourself.

## 1. Read before you write

- The current manifest: \`zed.toml\` (or \`zed.yaml\` if this project already partially moved — check \`zed_version\` at the top either way).
- Any \`[[agents]]\` entries in the v1 manifest — these carry \`connectors\`, \`zed_cli\`, and \`env\` grants per agent name. An agent name with NO \`[[agents]]\` entry at all is today unrestricted (v1's back-compat default is "all" when a grant key is omitted).
- \`.zed/opencode/opencode.jsonc\` — if it sets a top-level \`default_agent\`, that is the project's existing default; use it. If it doesn't, pick the agent whose \`.zed/opencode/agents/*.md\` frontmatter has \`mode: primary\` and reads as the general/primary one (usually the first-created or the one with the broadest permissions). Record which you picked and why in the change request description — a human reviews this before it merges, so a defensible choice beats blocking on it.
...
1. \`zed marketplace updates\` — a hash-diff report of every marketplace-tracked item (the zed-managed skills like \`zed-system\`/\`zed-memory\`, plus any marketplace skills the user installed).
2. If updates are listed, apply them: \`zed marketplace update --all\`. **This commits directly to \`main\` through the platform's own hash-safe update path — it is intentionally NOT part of your change request.** It only rewrites files whose installed hash no longer matches the catalog, so untouched user files are never clobbered.
...
5. Marketplace-tracked items are the ONLY thing you refresh this way. Do NOT hand-update anything else "to latest": agents' \`.md\` files, \`.zed/memory/\`, \`opencode.jsonc\`, the continuation plugin, \`tools/*.ts\`, and custom skills are user- or platform-owned files with no update tracking — leave every one of them untouched.
...
- \`zed schema --version 2\` — prints the exact v2 schema the validator and the CR-merge gate enforce. Works offline inside your sandbox. \`zed schema --version 1\` prints the v1 shape you are migrating FROM.
- The same documents are published at \`https://zed.com/schema/zed.v2.schema.json\` (and \`zed.v1.schema.json\`, plus the combined \`zed.schema.json\` that dispatches on \`zed_version\`).

The schema, this prompt, and \`zed validate\` all enforce the same rules — if they ever appear to disagree, trust \`zed validate\`'s output and say so in the change request description.

## 4. The v2 shape you're producing

v2's \`agents:\` map is GOVERNANCE ONLY — connectors/secrets/skills/zed_cli/workspace/enabled. OpenCode behavior (mode/model/temperature/permission/the prompt itself) is NOT part of the manifest at all; it lives entirely in each agent's own native \`.zed/opencode/agents/<name>.md\` frontmatter + body, exactly as it does today. The agent's NAME is the join between this map's keys and that \`.md\`'s filename.
...
agents:
  <name>:
    ...
    zed_cli: all                # zed_cli leaf names | "all" | "none"
    skills: all                    # names of .zed/opencode/skills/* this agent may invoke | "all" | "none"
...
Rules that the schema enforces (get these right or \`zed validate\` fails):
...
- \`env\` is a hard error in v2 — it is renamed \`secrets\`. **v2 defaults every omitted grant (\`connectors\`/\`secrets\`/\`zed_cli\`/\`skills\`) to \`"none"\` (deny-by-default), unlike v1 which defaulted an omitted grant to \`"all"\`.** To avoid silently narrowing an agent's access during migration, write the EXPLICIT value that reproduces today's behavior for every agent — if a v1 agent had no \`[[agents]]\` entry, or its \`env\`/\`connectors\`/\`zed_cli\` were omitted or set to \`all\`, write \`secrets: all\`, \`connectors: all\`, \`zed_cli: all\` explicitly in its v2 block. Only narrow a grant if the v1 manifest already narrowed it (an explicit list, or \`none\`) — carry that exact list over. \`skills\` has no v1 equivalent; default new agents to \`all\` unless you have a specific reason to narrow.
...
- If an agent has no \`.md\` today (a bare \`[[agents]]\` entry with no matching OpenCode agent file), still declare it in \`agents:\` with its governance grants carried over — don't drop it. It will simply have no behavior until someone adds \`.zed/opencode/agents/<name>.md\`.
...
- **Legacy singular \`[sandbox]\` image keys** (\`image\`, \`dockerfile\`, \`cpu\`, \`memory\`, \`disk\`, …) — already an error in v1's validator; if \`zed validate\` flags them, move the image definition under \`[[sandbox.templates]]\` → \`sandbox.templates:\` with a named slug.

## 6. Worked example

A representative v1 \`zed.toml\`:
...
env = "all"
zed_cli = ["project.file.read", "project.file.write", "project.session.exec"]
...
agents:
  dev:
    ...
    zed_cli:           # project.session.exec dropped — retired no-op action
      - project.file.read
      - project.file.write
    skills: all
  support:
...
- Delete the old \`zed.toml\` in the same commit — don't leave both files (the platform always prefers \`zed.yaml\` when both exist, but a stale v1 file next to it is confusing for the next person who edits by hand).
- You do not need to touch any project setting outside git — the platform resolves \`zed.yaml\` automatically once it exists, regardless of the configured manifest filename.

## 9. Validate before you're done

Run \`zed validate\` (it auto-detects \`zed.yaml\`). Fix every error it reports — do not open the change request with a manifest that fails validation. Warnings are fine to leave if they're informational, but read them. If an error surprises you, cross-check the field against \`zed schema --version 2\`.

## 10. Land it as a change request — never merge

First, re-sync: \`git fetch origin\` and check whether \`origin/main\` advanced while you worked (\`git log HEAD..origin/main --oneline\`) — on active projects it will (connectors added from the dashboard, other sessions merging). If it moved, rebase; if the rebase conflicts on \`zed.toml\` (main changed the manifest you deleted), don't fight it — \`git rebase --abort\`, \`git reset --hard origin/main\`, and redo the conversion against the CURRENT manifest, then continue. Never revert main's changes to win a conflict.
...
\`\`\`
git add -A && git commit -m "Migrate manifest to zed_version 2 (zed.yaml)"
git push origin HEAD
zed cr open --head <your-branch> --title "Migrate manifest to zed_version 2 (zed.yaml)" --description "<what you converted, which agent you picked as default_agent and why, every legacy key you removed, and any grant you had to leave narrowed>"
\`\`\`
...
Then verify the CR actually carries your diff: run \`zed cr diff <number>\` — if it reports no changes, your push didn't land; push again and re-check (the CR updates automatically, do not open a second one).

Do **not** run \`zed cr merge\`. This is a human-reviewed change like any other — stop once the CR is open and verified non-empty, and tell the user its number so they can review the diff and merge it themselves.`;
