import { Reveal } from '@/components/home/reveal';
import { Badge } from '@/components/ui/badge';
import { cn } from '@/lib/utils';
import type { ReactNode } from 'react';

/**
 * The one heading block every section on `/company-as-code` uses: eyebrow
 * badge, headline, sub. Same three elements at the same rhythm as
 * `/agent-computer`, so the two argument pages read as one site.
 */
export function SectionHeading({
  eyebrow,
  title,
  sub,
  className,
}: {
  eyebrow: string;
  title: string;
  sub: string;
  className?: string;
}): ReactNode {
  return (
    <Reveal>
      <div className={cn('max-w-3xl', className)}>
        <Badge variant="zed" className="rounded">
          {eyebrow}
        </Badge>
        <h2 className="text-foreground mt-6 text-3xl font-medium tracking-tight text-balance sm:text-4xl">
          {title}
        </h2>
        <p className="text-muted-foreground mt-4 text-base leading-relaxed">{sub}</p>
      </div>
    </Reveal>
  );
}
