...
export const hero = {
  ...
  microline: 'Two types · Declared in zed.yaml · Reviewed like everything else',
...
export const types = {
  ...
  cards: [
    ...
    },
    {
      ...
      body: 'An external service POSTs to the trigger URL. Zed checks the signature, renders the payload into the prompt, and starts the session. A payload that fails your filter is accepted and ignored.',
    },
  ],
  notes: [
    ...
    'Both are entries in zed.yaml, so both have a history and an author.',
  ],
} as const;
...
export const declared = {
  ...
  sub: 'Triggers live in zed.yaml next to your agents and sandbox images. Each one names its agent, its schedule or its secret, and the prompt template that becomes the session’s first message.',
  yaml: {
    title: 'zed.yaml',
    lines: [
      '# fires on the clock',
...
export const webhook = {
  ...
  header: 'X-Zed-Signature: sha256=<hmac>',
  headerNote:
  ...
  rows: [
    { code: '202', v: 'Signature valid. The session fired, queued behind the concurrency limit, or deduped against a delivery Zed already saw.' },
...
export const session = {
  ...
  sub: 'By default every fire is a clean slate. When the work is a running thread rather than a fresh errand, a trigger can re-prompt a session it already owns. Zed tries the modes in order and falls through on failure, so a fire never simply disappears.',
  steps: [
    {
      n: '01',
      mode: 'pinned',
  ...
  rows: [
    {
      ...
      v: 'A trigger names an agent, and inherits exactly that agent’s grants — the connectors, secrets and skills its block in zed.yaml lists, and nothing else. An agent with no grants gets no access.',
    },
    {
      id: 'sandbox',
