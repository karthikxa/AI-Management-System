...
describe('scrape web helpers', () => {
  ...
  test('buildScrapeFailureResults maps per-url errors from aggregate message', () => {
    const output =
      'Error: Failed to scrape all 1 URLs. https://zed.com: timeout of 35000ms exceeded';
    const results = buildScrapeFailureResults(output, ['https://zed.com']);
  ...
  test('resolveScrapeResults falls back to input urls on plain error output', () => {
    const output =
      'Error: Failed to scrape all 1 URLs. https://zed.com: timeout of 35000ms exceeded';
    const results = resolveScrapeResults(output, { urls: 'https://zed.com' });
    expect(results).toHaveLength(1);
