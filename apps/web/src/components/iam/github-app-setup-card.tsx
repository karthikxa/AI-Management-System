...
function isManifestFormSubmitting(): boolean {
  try {
    return sessionStorage.getItem(MANIFEST_GUARD_KEY) === 'true';
  } catch {
    return false;
  }
}

function setManifestFormSubmitting(value: boolean) {
  try {
    if (value) {
      sessionStorage.setItem(MANIFEST_GUARD_KEY, 'true');
    } else {
      sessionStorage.removeItem(MANIFEST_GUARD_KEY);
    }
  } catch {
...
function submitManifestForm(
  ...
  if (isManifestFormSubmitting()) {
    console.debug('[github-app] manifest form already submitted, skipping duplicate');
    return;
  }
  setManifestFormSubmitting(true);

  // Clean up any orphaned forms from previous attempts
  document.querySelectorAll('form[data-github-manifest]').forEach((f) => f.remove());
  
  const form = document.createElement('form');
  form.method = 'POST';
  form.action = `${githubCreateUrl}${githubCreateUrl.includes('?') ? '&' : '?'}state=${encodeURIComponent(state)}`;
  form.style.display = 'none';
  form.setAttribute('data-github-manifest', 'true');
...
export function GitHubAppSetupCard({ canManage }: GitHubAppSetupCardProps) {
  ...
  const startMutation = useMutation({
    ...
    onSuccess: ({ github_create_url, manifest, state }) => {
      ...
      // Guard: prevent multiple popups — use sessionStorage (survives re-mounts + HMR)
      if (isManifestFormSubmitting()) {
        console.debug('[github-app] already submitting, skipping duplicate');
        return;
      }
    ...
    onError: (err: Error) => {
      errorToast(err.message || 'Failed to start GitHub App setup');
            ...
            <TabsContent value="manifest" className="space-y-3 pt-4">
              ...
              <div className="bg-popover space-y-4 rounded-md border px-4 py-5">
                ...
                <Button
                  ...
                  size="sm"
                  className="gap-1.5"
                  disabled={startMutation.isPending || isSubmitting}
                  onClick={() => startMutation.mutate()}
                >
                  {(startMutation.isPending || isSubmitting) ? (
                    <Loading className="size-4 shrink-0" />
                  ) : (
                    <Github className="size-4" />
                  )}
                  {(startMutation.isPending || isSubmitting) ? 'Redirecting to GitHub' : 'Create GitHub App'}
                </Button>
              </div>
            </TabsContent>
