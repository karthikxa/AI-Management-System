...
/**
 * ZedLoader - A unified circular loading animation component
 *
 ...
 * <ZedLoader variant="white" />
 *
 * // Always black (for light backgrounds in any theme)
 * <ZedLoader variant="black" />
 *
 * // Custom size
 * <ZedLoader size="large" />
 * ```
 */
export function ZedLoader({
  size = 'medium',
  speed = 1.2,
  customSize,
  className,
    ...
    >
      <div
        style={{
          ...
          animation:
            autoPlay && loop
              ? `zed-spin ${animationDuration}s linear infinite`
              : autoPlay
                ? `zed-spin ${animationDuration}s linear`
                : 'none',
        }}
      />
    </div>
