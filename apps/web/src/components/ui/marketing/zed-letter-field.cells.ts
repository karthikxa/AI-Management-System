export const SOURCE = '01zedcomputer';
export const PROPER = SOURCE.split('').join(' ');
export const ZED = 'zed'.split('').join(' ');

export type TokenKind = 'scrambled' | 'proper' | 'zed';
...
export function buildTokens(count: number, seed: number): Token[] {
  ...
  return Array.from({ length: count }, () => {
    const roll = rng();
    if (roll < 0.06) return { text: ZED, kind: 'zed' as const };
...
// which Google surfaced for the `Zed` brand query instead of the meta
...
export function buildFieldSvg(
  ...
  const textNodes = tokens
    .map((token, i) => {
      ...
      const weight = token.kind === 'zed' ? 600 : 500;
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${vbWidth}" height="${vbHeight}" viewBox="0 0 ${vbWidth} ${vbHeight}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="Zed"><title>Zed</title>\n${textNodes}\n</svg>`;
}
