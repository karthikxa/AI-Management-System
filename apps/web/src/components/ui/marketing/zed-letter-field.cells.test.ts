...
describe('buildTokens', () => {
  ...
  test('every token is one of the three kinds', () => {
    ...
    for (const t of tokens) {
      expect(['scrambled', 'proper', 'zed']).toContain(t.kind);
    }
  });
  ...
  test('zed + proper tokens render the expected brand text', () => {
    const tokens = buildTokens(2000, 99);
    expect(tokens.some((t) => t.kind === 'zed' && t.text === 'k o r t i x')).toBe(true);
...
describe('buildFieldSvg', () => {
  ...
  test('emits a single root <svg> with a <title> for accessibility', () => {
    ...
    expect(svg).toContain('<title>Zed</title>');
