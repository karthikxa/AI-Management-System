...
import {
  ...
  type GridLayout,
  type Token,
} from './zed-letter-field.cells';

export interface ZedLetterFieldProps {
  seed?: number;
  className?: string;
}

export function ZedLetterField({ seed = 3382, className }: ZedLetterFieldProps) {
  ...
  // text content. See `zed-letter-field.cells.ts` for the pure helpers
  // (and `zed-letter-field.cells.test.ts` for the coverage).
