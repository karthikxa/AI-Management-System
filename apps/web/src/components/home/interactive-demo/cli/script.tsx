 ...
 * Output text mirrors the real `zed` CLI so the terminal reads as authentic.
 * ─────────────────────────────────────────────────────────────────────────── */
  ...
  projects: [
    {
      ...
      files: 9,
      branch: 'main',
      repo: `git.zed.com/acme/${PROJECT}`,
      url: `zed.com/p/${PROJECT}`,
    },
  ],
...
export const SCRIPT: Command[] = [
  /* 1 ── zed init → a draft project appears on the Projects tab ─────────── */
  {
    input: `zed init ${PROJECT}`,
    beats: [
      nav('projects'),
      wait(260),
      blank(),
      out([
        t('Initialized Zed project '),
        t(`"${PROJECT}"`, 'fg'),
        t(' in '),
        t(`~/${PROJECT}`, 'faded'),
      ]),
      ...
      out([t('Wrote 9 files:')]),
      out([t('  + ', 'faded'), t('zed.yaml')]),
      out([t('  + ', 'faded'), t('.zed/opencode/agents/zed.md')]),
      out([t('  + ', 'faded'), t('.claude/skills/zed/SKILL.md')]),
  ...
  /* 2 ── zed ship → the draft flips shipping → live ─────────────────────── */
  {
    input: 'zed ship',
    beats: [
      nav('projects'),
      ...
      okLine(t('zed.yaml verified')),
      blank(),
      out([t('  '), t('zed ship', 'zed'), t('  new project → managed Zed git', 'dim')]),
      out(meta('name', PROJECT, 'fg')),
      blank(),
      wait(450),
      okLine(t('Committed: '), t('zed: ship', 'fg')),
      ...
      out(meta('repo', `git.zed.com/acme/${PROJECT}`)),
      out([t('  live  ', 'dim'), t(`zed.com/p/${PROJECT}`, 'cyan')]),
      wait(1050),
    ],
  },
  ...
  {
    input: 'zed connectors connect linear',
    beats: [
      nav('connectors'),
      wait(260),
  ...
  {
    input: 'zed connectors connect notion',
    beats: [
      nav('connectors'),
      wait(220),
      out([t('  Open this URL to authorize:', 'dim')]),
  ...
  {
    input: 'zed providers login openai',
    beats: [
      nav('models'),
      wait(260),
  ...
  {
    input: `zed sessions new --prompt "Summarize this week's pipeline…"`,
    beats: [
  ...
  {
    input: `zed chat --prompt "What changed in our repos since Monday?"`,
    beats: [
      okLine(t('session '), t('repo-changes', 'fg'), t(' · streaming', 'dim')),
      nav('chat'),
      fx((a) => a.runChat(chat('repos'))),
      wait(2000),
      out([t('  ▸ ', 'cyan'), t('github.list_commits ', 'fg'), t('zed-ai/zed', 'faded')]),
      wait(2500),
  ...
  {
    input: `zed chat --prompt "Run the weekly finance report and email the team…"`,
    beats: [
  ...
  {
    input: `zed triggers add daily-briefing --cron "0 8 * * *"`,
    beats: [
      nav('scheduling'),
      wait(280),
      fx((a) => a.addSchedule()),
      okLine(t('Added trigger '), t('daily-briefing', 'fg'), t(' (cron) to zed.yaml')),
  ...
  {
    input: 'zed secrets set SENDGRID_API_KEY=SG.•••',
    beats: [
      nav('security'),
      wait(280),
      fx((a) => a.addSecret()),
  ...
  {
    input: 'zed access invite alex@acme.ai --role member',
    beats: [
      nav('security'),
      wait(220),
      fx((a) => a.inviteMember()),
  ...
  /* 12 ── channels connect → Channels: Slack goes live ────────────────────── */
  {
    ...
    beats: [
      ...
      out([t('  webhook  ', 'dim'), t('api.zed.com/v1/webhooks/slack/1f3a', 'faded')]),
      wait(2400),
    ],
  },
];
