...
// download modal; on zed.com the hook resolves to the canonical command
...
test('all deployment-aware CLI installer surfaces use the hook', () => {
  for (const path of DEPLOYMENT_AWARE_SURFACES) {
    ...
    expect(source).not.toContain('https://zed.com/install');
    expect(source).not.toContain('ZED_CLI_INSTALL_COMMAND');
  }
});

test('the zed.com marketing page keeps the canonical installer command', () => {
  const source = readFileSync(
    join(WEB_SRC, 'app/(public)/(marketing)/developers/page.tsx'),
    'utf8',
  );
  expect(source).toContain('ZED_CLI_INSTALL_COMMAND');
});
