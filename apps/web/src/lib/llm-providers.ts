...
export function buildLlmProviderCatalog(raw: RawCatalog): LlmProviderEntry[] {
  // A live fetch can resolve to a body without `providers` (an error envelope,
  // a mock, a partial response). Callers treat any resolved value as usable,
  // so absorb it here rather than throwing through the render tree.
...
export function applyLiveLlmProviderCatalog(raw: RawCatalog): void {
  // A live fetch can resolve to a body without `providers` (an error envelope,
  // a mock, a partial response). Keep the baked seed rather than replacing a
