...
import { GET as getAgentSkill } from '@/app/(public)/.well-known/agent-skills/zed-public-content/SKILL.md/route';
...
describe('agent discovery documents', () => {
  test('publishes an RFC 9727 linkset with working Zed relations', async () => {
    ...
    expect(body.linkset[0].anchor).toBe('https://api.zed.com/v1');
    expect(body.linkset[0]['service-desc'][0].href).toBe('https://api.zed.com/v1/openapi.json');
    expect(body.linkset[0]['service-doc'][0].href).toBe('https://zed.com/docs');
    expect(body.linkset[0].status[0].href).toBe('https://api.zed.com/v1/health');
  });
  ...
  test('publishes truthful OAuth discovery and protected resource metadata', async () => {
    ...
    expect(oauth.agent_auth.user_authorization.credential_types_supported).toEqual([
      'zed_personal_access_token',
    ]);

    const resource = (await getProtectedResource(
      new Request('https://zed.com/.well-known/oauth-protected-resource'),
    ).json()) as any;
    expect(resource.resource).toBe('https://zed.com');
    ...
    expect(authMd).toStartWith('# Zed auth.md');
    expect(authMd).toContain('Zed does not publish dynamic OAuth client registration');
    expect(authMd).toContain('Registration URI: /cli/authorize');
    expect(authMd).toContain('POST https://api.zed.com/v1/accounts/tokens');
    expect(authMd).toContain('DELETE https://api.zed.com/v1/accounts/tokens/{token_id}');
  ...
  test('publishes an MCP discovery card for the real public endpoint', async () => {
    ...
    expect(card.serverInfo.name).toBe('zed-public-content');
    expect(card.transport).toEqual({
      type: 'streamable-http',
      endpoint: 'https://zed.com/mcp',
    });
  ...
  test('serves MCP initialize, list, and call responses', () => {
    ...
    expect(initialize.result.serverInfo.name).toBe('zed-public-content');
    ...
    expect(call.result.content[0].text).toContain('# Zed');
  });
  ...
  test('registers two read-only WebMCP tools with abort-controlled lifetimes', async () => {
    ...
    expect(registered.map(({ tool }) => tool.name)).toEqual([
      'search_zed_public_content',
      'read_zed_public_page',
    ]);
  ...
  test('negotiates the homepage to a Markdown route before auth middleware', async () => {
    const request = new NextRequest('https://zed.com/', {
      headers: { Accept: 'text/markdown' },
    });
    const rewritten = await middleware(request);
    expect(rewritten.headers.get('x-middleware-rewrite')).toBe(
      'https://zed.com/markdown-negotiation?path=%2F',
    );

    const response = getNegotiatedMarkdown(
      new Request('https://zed.com/markdown-negotiation?path=%2F'),
    );
    expect(response.status).toBe(200);
    expect(response.headers.get('Content-Type')).toBe('text/markdown; charset=utf-8');
    expect(response.headers.get('Vary')).toBe('Accept');
    expect(await response.text()).toContain('# Zed');
  });

  test('adds agent discovery links to the homepage middleware response', async () => {
    const response = await middleware(new NextRequest('https://zed.com/'));
  ...
  test('declares content signals and agent discovery routes in robots.txt', () => {
    const robots = renderRobotsTxt('zed.com');
