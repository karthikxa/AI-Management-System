...
describe('public SEO/AEO content coverage', () => {
  test('publishes the QM comparison as canonical agent-readable Markdown', () => {
    const resolved = resolvePublicMarkdown(['blog', 'zed-vs-qm.md']);
    expect(resolved?.record.htmlPath).toBe('/blog/zed-vs-qm');
    expect(resolved?.markdown).toContain('# Zed vs QM: two open agent platforms');
    expect(resolved?.markdown).toContain('| Dimension | QM | Zed |');
    ...
    expectCleanAgentMarkdown(resolved!.markdown, '/markdown/blog/zed-vs-qm.md');
  });
  ...
  test('uses one non-www canonical origin and no hardcoded canonical tag', () => {
    ...
    const source = files
      ...
      .join('\n');
    expect(source).not.toContain('https://www.zed.com');
...
describe('bounded public agent index', () => {
  test('paginates a metadata-only index and clamps page size', async () => {
    const first = getAgentIndex(
      new Request('https://zed.com/api/ai?limit=999', { headers: { 'x-real-ip': '192.0.2.1' } }),
    );
    ...
    if (body.pagination.next_cursor) {
      const second = getAgentIndex(
        new Request(`https://zed.com/api/ai?limit=999&cursor=${body.pagination.next_cursor}`, {
  ...
  test('orders the index recency-first so freshest content leads each kind', async () => {
    const response = getAgentIndex(
      new Request('https://zed.com/api/ai?kind=blog&limit=999', {
    ...
    const unfiltered = getAgentIndex(
      new Request('https://zed.com/api/ai?limit=999', { headers: { 'x-real-ip': '192.0.2.5' } }),
    );
  ...
  test('stamps docs and marketing records with a git-derived last_modified when the build manifest is present', async () => {
    ...
    const unfiltered = getAgentIndex(
      new Request('https://zed.com/api/ai?limit=50', { headers: { 'x-real-ip': '192.0.2.6' } }),
    );
    const unfilteredBody = (await unfiltered.json()) as any;
  ...
  test('rejects malformed pagination and enforces the documented rate limit', async () => {
    const invalid = getAgentIndex(
      new Request('https://zed.com/api/ai?cursor=not-valid!', {
    ...
    for (let index = 0; index <= 120; index += 1) {
      response = getAgentIndex(
        new Request('https://zed.com/api/ai', { headers: { 'x-real-ip': '192.0.2.3' } }),
      );
    }
  ...
  test('robots blocks crawling on every non-canonical host', () => {
    for (const host of ['dev.zed.com', 'staging.zed.com', 'seo-fix.vercel.app', null]) {
