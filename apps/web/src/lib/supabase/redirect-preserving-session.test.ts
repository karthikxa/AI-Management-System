...
describe('redirectPreservingCookies', () => {
  test('carries cookies staged on the source response onto the redirect', () => {
    ...
    staged.cookies.set('sb-zed-auth-token', '', { path: '/', maxAge: 0 });
    staged.cookies.set('other-cookie', 'keep-me');

    const redirect = redirectPreservingCookies(
      new URL('https://zed.local/auth?redirect=%2Fprojects'),
      staged.cookies,
    );

    expect(redirect.headers.get('location')).toBe('https://zed.local/auth?redirect=%2Fprojects');
    expect(redirect.cookies.get('sb-zed-auth-token')?.value).toBe('');
  ...
  test('a plain NextResponse.redirect() drops those cookies — the exact regression this guards against', () => {
    ...
    const bareRedirect = NextResponse.redirect(new URL('https://zed.local/auth'));

    expect(bareRedirect.cookies.get('sb-zed-auth-token')).toBeUndefined();
  });

  test('no cookies staged on the source means no cookies on the redirect', () => {
    const staged = NextResponse.next();
    const redirect = redirectPreservingCookies(new URL('https://zed.local/auth'), staged.cookies);
