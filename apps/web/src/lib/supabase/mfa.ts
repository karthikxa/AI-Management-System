...
export const supabaseMFAService = {
  ...
  async listFactors(): Promise<ListFactorsResponse> {
    ...
    try {
      ...
      // No session = signed out, not a failure. All three callers render a
      // "no factors" state anyway; throwing here spams the console and makes
      // react-query retry a call that can never succeed while logged out.
      if (error || !user) {
        return { factors: [] };
      }
