...
const RuntimeEnvSchema = z.object({
   ...
   *  ZED_BILLING_INTERNAL_ENABLED. Set via NEXT_PUBLIC_BILLING_ENABLED. */
  BILLING_ENABLED: z.boolean().default(false),
  /** CLOUD-ONLY. Whether Zed's own managed model lineup ("Managed ·
   *  Included with your plan") can appear in the UI at all. Mirrors the
   *  backend's ZED_MANAGED_PROVIDER_ENABLED, which already empties the
   ...
   *  showing it with zero models. Off by default (self-host); Zed Cloud
   *  sets NEXT_PUBLIC_MANAGED_PROVIDER_ENABLED / ZED_PUBLIC_MANAGED_PROVIDER_ENABLED
   *  to 'true'. */
   ...
   *  ZED_PUBLIC_CONNECTORS_ENABLED / NEXT_PUBLIC_CONNECTORS_ENABLED. */
   ...
   *  ZED_PUBLIC_DISABLE_LANDING_PAGE / NEXT_PUBLIC_DISABLE_LANDING_PAGE. */
   ...
   *  ZED_PUBLIC_RESTRICT_ACCOUNT_CREATION / NEXT_PUBLIC_RESTRICT_ACCOUNT_CREATION. */
   ...
   *  session runtime sandbox is the source of truth). No legacy 'zed-sandbox'
