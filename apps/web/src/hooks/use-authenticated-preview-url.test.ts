...
describe('buildPreviewAuthEndpoint', () => {
  test('builds auth URLs from canonical preview paths', () => {
    expect(buildPreviewAuthEndpoint('http://localhost:8008/v1/p/zed-sandbox/4502/index.html'))
  ...
  test('derives the auth endpoint from a trusted server origin', () => {
    expect(buildPreviewAuthEndpoint(
      'http://localhost:8008/proxy/4502/v1/p/zed-sandbox/4502/index.html',
      'http://localhost:8008/v1/p/zed-sandbox/8000',
    )).toBe('http://localhost:8008/v1/p/auth');
  });

  test('rejects a proxied preview whose origin is not the trusted server', () => {
    expect(buildPreviewAuthEndpoint(
      'http://localhost:8000/proxy/4502/v1/p/zed-sandbox/4502/index.html',
      'http://localhost:8008/v1/p/zed-sandbox/8000',
    )).toBeNull();
  });

  test('never nests auth requests under a proxied app path', () => {
    expect(buildPreviewAuthEndpoint('http://localhost:8000/proxy/4502/v1/p/zed-sandbox/4502/index.html'))
