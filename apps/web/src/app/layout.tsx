...
export default async function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return (
    ...
    >
      <head>
        {/* Suppress THREE.js WebGL shader compilation console.error noise.
            The TSL shader compiler generates variable names with double underscores
            (reserved in GLSL). These are cosmetic — shaders render fine — but
            Next.js's dev overlay captures them as "issues". Patch early in <head>
            so it runs before any client scripts (including the overlay). */}
        <script
          dangerouslySetInnerHTML={{
            __html: `(function(){var o=console.error;console.error=function(){var a=arguments[0];if(typeof a==='string'&&/THREE\\.WebGLProgram:[\\s\\S]*identifiers containing two consecutive underscores/.test(a))return;o.apply(console,arguments)};})();`,
          }}
        />
