 ...
 * used to read "Zed – The AI Command Center for Your Company" for every open
...
export async function generateMetadata({ params }: SessionRouteProps): Promise<Metadata> {
  ...
  return {
    // `absolute` bypasses the root layout's `%s | Zed` template: the tab
    // reads "<session name> — Zed", and a session named "Zed" must not
    // come out as "Zed | Zed".
