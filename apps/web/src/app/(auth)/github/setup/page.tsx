...
function isGithubProofInFlight(): boolean {
  try {
    return sessionStorage.getItem(GITHUB_PROOF_GUARD_KEY) === 'true';
  } catch {
    return false;
  }
}

function setGithubProofInFlight(value: boolean) {
  try {
    if (value) {
      sessionStorage.setItem(GITHUB_PROOF_GUARD_KEY, 'true');
    } else {
      sessionStorage.removeItem(GITHUB_PROOF_GUARD_KEY);
    }
  } catch {
...
function requestGitHubUserProof(): Promise<string> {
  // If a popup is already in-flight, we still need to open it (the old one
  // may be dead after HMR) but we should not open duplicates rapidly.
  // Use a short cooldown via sessionStorage to prevent rapid-fire opens.
  if (isGithubProofInFlight()) {
    return Promise.reject(new Error('A GitHub verification popup is already open. Please complete it or close it and try again.'));
  }

  setGithubProofInFlight(true);
  ...
  if (!proofPopup) {
    setGithubProofInFlight(false);
    return Promise.reject(new Error('Allow pop-ups to verify your GitHub access.'));
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const closePoll = window.setInterval(() => {
      if (proofPopup.closed) finish({ error: new Error('GitHub verification was cancelled.') });
    }, 500);
