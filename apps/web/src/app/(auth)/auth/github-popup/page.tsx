...
function isGithubPopupInFlight(): boolean {
  try {
    return sessionStorage.getItem(GITHUB_POPUP_GUARD_KEY) === 'true';
  } catch {
    return false;
  }
}

function setGithubPopupInFlight(value: boolean) {
  try {
    if (value) {
      sessionStorage.setItem(GITHUB_POPUP_GUARD_KEY, 'true');
    } else {
      sessionStorage.removeItem(GITHUB_POPUP_GUARD_KEY);
    }
  } catch {
...
export default function GitHubOAuthPopup() {
  ...
  useEffect(() => {
    ...
    if (!isGithubPopupInFlight()) {
      setGithubPopupInFlight(true);
      handleOAuth();
    }
