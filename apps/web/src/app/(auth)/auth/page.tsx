...
function AuthCardForm({
  ...
  // Track client-side mount so SSR and the first hydrated render use
  // `initial={false}` — avoiding a hydration mismatch caused by
  // `useReducedMotion()` returning `null` on the server but a boolean on
  // the client (which changes the `y` initial value).
  const [mounted, setMounted] = useState(false);
  useEffect(() => { setMounted(true); }, []);

  // Gentle two-part entrance per step: header first, body 60ms behind.
  const rise = (delay = 0) => ({
    initial: mounted ? { opacity: 0, y: prefersReducedMotion ? 0 : 8 } : false,
