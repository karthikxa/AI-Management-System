...
function isGithubConnectInFlight(): boolean {
  try {
    return sessionStorage.getItem(GITHUB_CONNECT_GUARD_KEY) === 'true';
  } catch {
    return false;
  }
}

function setGithubConnectInFlight(value: boolean) {
  try {
    if (value) {
      sessionStorage.setItem(GITHUB_CONNECT_GUARD_KEY, 'true');
    } else {
      sessionStorage.removeItem(GITHUB_CONNECT_GUARD_KEY);
    }
  } catch {
...
export default function GitHubConnectPopup() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (isCallback) {
          ...
          closeTimer = setTimeout(() => {
            setGithubConnectInFlight(false);
            window.close();
          }, 200);
          return;
        }
    ...
    // Guard: prevent multiple OAuth flows (React StrictMode double-invoke, rapid re-mounts, HMR)
    if (!isGithubConnectInFlight()) {
      setGithubConnectInFlight(true);
      handle();
    }

    return () => {
      disposed = true;
      // Don't reset here — it should stay locked until the popup closes
      // The popup's beforeunload or window.close() will clean up
