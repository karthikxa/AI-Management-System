...
description: The release-notes agent we run on Zed — connected to GitHub. On each release it reads the merged PRs since the last one, groups them, writes the notes, and opens a changelog PR.
date: "2026-05-06"
author: team
tags:
  - Engineering
...
We run a release-notes agent on Zed that does it from the PR history. On each
...
On Zed, each release triggers an agent. When a tag is pushed, the release
