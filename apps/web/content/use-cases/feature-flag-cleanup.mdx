...
description: The flag-cleanup agent we run on Zed — a weekly sweep that finds feature flags that are fully rolled out or long dead, deletes the flag and the dead code branch it guards, and opens a PR for a human to merge.
date: "2026-03-05"
author: team
tags:
  - Engineering
...
We run a flag-cleanup agent on Zed that sweeps our repo every week, finds the
...
On Zed, a weekly cron re-prompts one **reusable session** that treats flag
