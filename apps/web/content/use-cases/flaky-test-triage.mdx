...
description: The flaky-test agent we run on Zed — connected to GitHub CI history and Slack. It scores every test's non-determinism, opens a quarantine PR for the worst offenders, and files a tracking issue for a human to review.
date: "2026-02-20"
author: team
tags:
  - Testing
...
We run a flaky-test-triage agent on Zed that reads the CI run history every
...
On Zed, a daily cron re-prompts one persistent agent session. It resumes
