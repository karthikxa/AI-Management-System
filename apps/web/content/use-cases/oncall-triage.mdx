...
description: The triage agent we run on Zed — connected to Sentry, our logs, and GitHub. It works up a first-pass diagnosis on every alert and only pages a human when it can't resolve it.
date: "2026-02-25"
author: team
tags:
  - SRE
...
We run a triage agent on Zed that does the first pass. When an alert fires, the
...
On Zed, each alert triggers an agent. When Sentry fires, the alert spawns an
