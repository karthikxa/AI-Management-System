...
description: The sales-forecast agent we run on Zed — a weekly cron rolls up HubSpot's open pipeline into a weighted forecast vs quota by stage, rep, and segment, flags deals slipping the quarter, and posts it to Slack. Read-only — it never touches a deal's amount or close date.
date: "2026-05-02"
author: team
tags:
  - Sales
...
We run a sales-forecast agent on Zed that reads HubSpot every week and
...
On Zed, a weekly cron triggers an agent. It spawns a fresh session with
