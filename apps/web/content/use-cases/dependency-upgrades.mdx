...
description: The upgrade agent we run on Zed — a weekly cron that opens dependency PRs, runs the full suite in a sandbox, and only opens the PR when it's green.
date: "2026-02-11"
author: team
tags:
  - Engineering
...
We run an upgrade agent on Zed that does the checking before it asks for a
...
On Zed, a weekly cron triggers an upgrade agent. Each run spawns an isolated
