...
description: The customer onboarding agent we run on Zed — a daily cron that checks every new account against its onboarding milestones in Postgres, drafts a nudge email when one stalls, and alerts the owning CSM in Slack with the context to act.
date: "2026-06-25"
author: team
tags:
  - Customer Success
...
We run a customer-onboarding agent on Zed that checks every new account
...
On Zed, a daily cron triggers an agent that reads HubSpot for every
