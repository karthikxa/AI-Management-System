...
description: The outbound-sourcing agent we run on Zed — daily, it finds candidates on LinkedIn for each open role, checks Greenhouse so no one already in the pipeline gets a duplicate reach-out, and drafts a personalized email for the recruiter to review and send.
date: "2026-06-08"
author: team
tags:
  - Recruiting
...
We run a candidate-sourcing agent on Zed that does the search and the
