...
An account holds your [projects](/docs/project) and the people who work in them. When you sign up, Zed creates a personal account for you. Invite a teammate and it becomes a team account. Personal and team accounts use the same roles, billing, and limits.
...
Zed signs you in with a personal access token (`zed_pat_...`). A service account (`zed_sa_...`) is a separate, non-human credential with no project access until you grant it.

See [SDK authentication](/docs/sdk/auth) for the full token model.

## Billing

Billing applies at the account level and covers every project in the account. Zed offers a free tier, a pro tier, and a per-seat team tier, each with its own model access and credits. Using your own model key does not make usage free: Zed still charges a platform fee on top of it, except on the free tier.
