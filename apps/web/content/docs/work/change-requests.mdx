...
1. Commit on the session branch (`$ZED_BRANCH_NAME`). Make small, working commits. Do not rewrite history or force-push.
2. Push the branch: `git push origin HEAD`.
3. Open the CR: `zed cr open --title "..." --description "..."`. Inside a session sandbox, `--head` and `--session` default to `$ZED_BRANCH_NAME` and `$ZED_SESSION_ID`. `--base` defaults to the project's default branch.
...
Zed refuses to create a CR whose branch has no commits ahead of the default branch. This usually means the agent committed locally but never pushed. Push the commits, then create the CR again.

## SHA refresh

For an open CR, Zed refreshes `head_commit_sha` and `base_commit_sha` against the live branches on every `GET`. If the repository is unreachable, or a branch is missing, Zed skips the refresh and serves the CR's last known metadata. A merged CR keeps the SHAs captured at merge time — Zed never refreshes them again.

## Merge mechanics

`POST /v1/projects/:projectId/change-requests/:crId/merge` runs these steps.

1. Zed reads the manifest (`zed.yaml`) from `head_ref` and validates it against the manifest schema. A branch with no manifest passes. An invalid manifest returns `422` with `code: "MANIFEST_INVALID"` and stops the merge.
2. Zed fast-forwards `base_ref` if `head_ref` is strictly ahead of it.
3. Otherwise, Zed creates a merge commit. The default message is `Merge CR #<n>: <title>`, and you can override it with `message` in the request body. The commit author is `Zed <noreply@zed.ai>`.
4. A conflict returns `409` with the conflict list. Check the same list with `GET /:crId/merge-preview` before you merge.
5. On success, Zed sets `status` to `merged`, records `merged_at`, `merged_by`, and `merge_commit_sha`, and invalidates the project's git cache.
