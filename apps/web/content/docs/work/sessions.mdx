...
A session is one unit of agent work. Zed cuts a git branch and provisions
...
| Status | Meaning |
|---|---|
| `provisioning` | Zed cuts the branch and requests the sandbox. |
...
Zed does not write them for a session today.
...
Zed also stops an idle session for you:
...
Self-host operators can set `ZED_SANDBOX_AUTOSTOP_MINUTES` to change the
...
Deleting a session destroys its sandbox for good. Zed keeps the session
...
Every session uses OpenCode REST. Zed stores the selected OpenCode agent and
...
Zed runs sessions on Daytona, Platinum, or E2B Cloud. A project follows
