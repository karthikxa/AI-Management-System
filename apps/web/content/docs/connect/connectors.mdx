...
calls it as a tool. Zed brokers each call, so the sandbox never holds the
connector credential.

You declare most connectors in `zed.yaml`. See the
[manifest reference](/docs/project/manifest) for every field. Zed declares
...
Zed encrypts connection data and resolves it server-side for each tool
call. The agent requests an action. Zed attaches the credential, checks the
...
not grant authority. The page requires a signed-in Zed account. Zed then
...
Approve or deny the call once. Zed sends the decision back into the session
...
Complete the OAuth flow. Zed stores the connected account as a connection.
...
Zed encrypts the OAuth2 configuration and tokens. It refreshes access tokens
...
Zed reads the manifest from the default branch. The connector becomes
...
Open the connector and set its credential. Zed stores the value
...
```text
zed connectors ls
zed connectors call stripe-read <action> '<json-args>'
```

`connectors` lists the connectors in scope. `call` runs one action.

Slack and Microsoft Teams connect from the dashboard the same way as OAuth apps. Connecting Slack writes a `channel` connector to `zed.yaml` for you. See [Slack & channels](/docs/connect/slack).
