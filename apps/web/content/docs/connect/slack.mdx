...
Zed supports four channel platforms today:

- **Slack** — connect with one click through the Zed-managed app, or bring
  your own bot token.
...
Zed links each chat sender to a Zed account before the agent runs for
them. Run `/zed login` in Slack and follow the link to sign in. Teams uses
...
page. Zed never injects it into a sandbox. Zed resolves the token
...
On a host with the shared Slack app configured (Zed Cloud, for example),
...
On a self-hosted deployment with no shared Slack app configured, `zed
channels connect` falls back to manual mode automatically. Run `zed
...
```
zed channels connect --manual --bot-token xoxb-... --signing-secret ...
```
</Step>
...
Run `zed channels disconnect` to remove the connection. See the
...
to `zed.yaml` for you. You never write this entry by hand.
...
Type these as `/zed <command>` in Slack, or as plain text in a DM. Slash
commands do not run inside the Assistant DM pane, so Zed parses the same
...
- Slash commands do not fire inside the Assistant DM pane. Type
  `/zed <command>` as plain text there instead.
