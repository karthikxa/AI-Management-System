...
This page shows how the pieces of a self-hosted Zed instance fit
...
Self-hosted Zed is one generic Docker Compose system, not a family of
deployment targets. `zed self-host init` renders a `docker-compose.yml`
and `.env` file (plus a `Caddyfile` and `updater.sh` when you set a domain)
into `~/.config/zed/self-host/<instance>/`. `zed self-host start` runs
`docker compose up`. The same artifact runs on a laptop, a VPS, or any cloud
VM. A domain is only the `ZED_DOMAIN` environment variable, not a
different setup.
...
a domain, use `zed self-host init --tunnel cloudflare` instead.
...
- **Caddy** — reverse proxy and ACME TLS. Zed renders this service only
  when you set `ZED_DOMAIN`; a domain-less instance never opens ports
  ...
  frontend.
- **`zed-api`, `llm-gateway`, `frontend`** — the three application
...
- **The Supabase Docker distribution** — Kong, GoTrue auth, PostgREST,
  ...
  connection pooler. Zed vendors this from upstream Supabase and pins
  every image by digest.
- **`zed-updater`** — a small container with the Docker socket mounted.
  It checks for a new image once a day, at a fixed local clock time
  (`ZED_UPDATE_TIME`, default `02:00`, in `ZED_UPDATE_TZ`, default
  `America/New_York`). If an image changed, it runs the `zed-migrate`
...
- **Agent sandboxes** — by default, Daytona. You can configure Platinum or
  E2B instead. `zed-api` reaches the sandbox provider over egress;
  sandbox compute never runs on the self-host box.
- **The image registry** — `docker.io/zed/*`. The updater and
  `zed self-host start` pull from it. It needs no credentials.

<Callout type="warn">
`zed self-host uninstall` runs `docker compose down --volumes
...
`zed-updater` and `zed self-host update` (alias `reconcile`) resolve
