---
title: React hooks
description: Run a Zed session in React with the useSession hook.
---

`@zed/sdk/react` adds React hooks on top of the SDK. This page covers
...
| `sendError` | `ZedSendError \| null` | The last `send` failure: `billing`, `runtime-not-ready`, or `runtime-error`. |
...
| `rewindError` | `ZedSendError \| null` | The last rewind or restore failure. |
...
`@zed/sdk/react` also exports React Query hooks for data that does not
...
- [Sessions](/docs/sdk/sessions) — the session handle `useSession` wraps, and the `ZedSendError` kinds.
