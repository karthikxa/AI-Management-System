...
`@zed/sdk` is the typed client for the Zed platform. It wraps the Zed
...
Call `createZed` once, with your API base URL and a function that returns your token.
...
Create an API key at **User settings → API keys**. The key starts with `zed_pat_` and shows only once. Store it as a secret and return it from `getToken`. See [Auth](/docs/sdk/auth) for token types and scopes.
...
`zed.connectors` when the agent does not have a separate `projectId` value.
...
1. Create a session in your project.
   ...
   ```ts
   const created = await zed.project(projectId).sessions.create();
   const session = zed.session(projectId, created.session_id);
   ```
...
`createZed` gives you an imperative client: call methods for every action, like projects, sessions, secrets, and triggers. `@zed/sdk/react` gives you hooks for live UI data — `useSession` runs a whole session in one hook.
