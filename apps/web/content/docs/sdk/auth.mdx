...
Zed accepts one bearer token per request. Pass it through `getToken` in
`createZed`. The SDK sends it as `Authorization: Bearer <token>`.
...
Create one at **User settings → API keys**. The key starts with `zed_pat_`
...
`zed login` mints a PAT and stores it locally — it is the same credential
type, not a separate token kind.

## Service accounts

A service account is a separate credential family for non-human callers,
prefixed `zed_sa_`. Create one at **Account settings → Service accounts**.
...
If your app uses Zed's own sign-in, return the live session token instead
of a PAT:
...
| The Zed CLI | `zed login` (mints a personal access token) |
| A web app on Zed sign-in | A Supabase JWT |
