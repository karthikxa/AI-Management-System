---
title: Legacy zed.toml
description: Support status and the migration path from v1 zed.toml to v2 zed.yaml.
---

import { Steps, Step } from 'fumadocs-ui/components/steps';
import { Callout } from 'fumadocs-ui/components/callout';

This page covers the v1 manifest (`zed.toml`, `zed_version: 1`). For the current manifest, see [Manifest reference](/docs/project/manifest).

## Support status

Zed still supports v1 manifests. The platform resolves a project's manifest in this order: `zed.yaml`, then `zed.yml`, then `zed.toml`. The current starter creates one v2 `zed.yaml`. A v1 project keeps working with no forced upgrade.
...
<Steps>
<Step>
Create `zed.yaml` at the repo root. Set `zed_version: 2`.
</Step>
<Step>
...
<Step>
Run `zed validate` against `zed.yaml`. Fix any error it reports.
</Step>
<Step>
Delete `zed.toml`.
</Step>
...
Agent behavior — system prompt, `model`, `mode`, `temperature`, and more — never moves. It already lives in each agent's `.md` frontmatter under `.zed/opencode/agents/`, in both v1 and v2. See [Agents](/docs/project/agents).

Check the result with `zed validate --file zed.yaml`. Fetch the full v2 schema with `zed schema --version 2`. See [CLI reference](/docs/cli) for both commands.

## Key differences

| v1 (`zed_version: 1`) | v2 (`zed_version: 2`) | Change |
|---|---|---|
