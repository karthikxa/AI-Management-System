...
Zed backs a project two ways:

- **Zed-managed repo** — Zed creates and hosts a private repo. Default.
- **Imported GitHub repo** — connect an existing repo. Zed operates on it
...
and its memory. Keep it small — Zed clones it into every session.
...
Every project has a manifest at its repo root, `zed.yaml` by default. The
...
Older projects may still run `zed.toml` (`zed_version: 1`). Zed
...
<Cards>
  <Card title="Manifest" href="/docs/project/manifest">The full zed.yaml reference.</Card>
  ...
  <Card title="Legacy TOML" href="/docs/project/legacy-toml">Migrating from zed.toml.</Card>
</Cards>
