...
`zed-agent` daemon starts it as `opencode serve`, with its config
directory pointed at the project's `.zed/opencode/` folder.

## An agent is two files

Each agent has two parts:

- **Behavior** — a markdown file at `.zed/opencode/agents/<name>.md`. Its
  frontmatter sets the model, mode, and tools. Its body is the system prompt.
- **Governance** — an entry in `zed.yaml`'s `agents` map, keyed by the same
  name. It sets what the agent may access on Zed: connectors, secrets,
...
An agent with no `connectors`, `secrets`, `skills`, or `zed_cli` key gets
none of that access. Set a field to `all` to grant full access, or list
specific names. `default_agent` in `zed.yaml` names the agent a session
starts when you request no agent.

Legacy `zed.toml` (v1) projects list agents in an array, not a map, and
...
A skill is a markdown file at `.zed/opencode/skills/<name>/SKILL.md`.
OpenCode loads a skill on demand when the agent calls it — Zed does not
inject skills into every prompt. An agent's `skills` grant in `zed.yaml`
controls which skills it may load.

## How a session picks its agent

Creating a session accepts an `agent_name`. Zed starts that agent for the
session. If you omit `agent_name`, Zed uses the project's `default_agent`.
Set the default agent from the dashboard, the API, or the SDK. Each writes
`default_agent` to `zed.yaml` and commits the change to the project's
default branch.
