...
/**
 * zed-master React Query layer — the generic (host-agnostic) hooks over
 * the `./zed-master` transport client: query keys, caching/polling config,
 ...
 * (`use-credentials.ts`, `use-zed-projects.ts`, `use-zed-tasks.ts`,
 * `use-zed-tickets.ts`, `use-milestones.ts`, `use-sandbox-services.ts`).
 *
 ...
 * (`ZedMasterIdentity`) instead — see that type below. Any host (web,
...
import {
  ...
  listZedProjects,
  getZedProject,
  getZedProjectBySession,
  listZedProjectSessions,
  deleteZedProject,
  patchZedProject,
  // Tasks
  listTasks,
...
// them from `@zed/sdk/opencode-client` (via the web hook files) keep that
 ...
 */
export interface ZedMasterIdentity {
  userId: string | null;
  handle: string;
  isLoading: boolean;
}
...
export const credentialKeys = {
  list: (pid?: string) => ['zed', 'credentials', pid ?? ''] as const,
  events: (pid: string, name: string) => ['zed', 'credentials', pid, name, 'events'] as const,
};
...
// ═════════════════════════════════════════════════════════════════════════
// Zed projects — /zed/projects
// ═════════════════════════════════════════════════════════════════════════

export const zedKeys = {
  projects: () => ['zed', 'projects'] as const,
  project: (id: string) => ['zed', 'projects', id] as const,
};

export interface ZedProjectQueryOptions {
  enabled?: boolean;
}

export function useZedProjects(
  identity: ZedMasterIdentity,
  _args?: undefined,
  options: ZedProjectQueryOptions = {},
) {
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useQuery<ZedProject[]>({
    queryKey: [...zedKeys.projects(), identity.userId ?? 'anonymous', serverUrl],
    queryFn: () => listZedProjects(serverUrl),
...
export function useZedProject(identity: ZedMasterIdentity, id: string) {
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useQuery<ZedProject>({
    queryKey: [...zedKeys.project(id), identity.userId ?? 'anonymous', serverUrl],
    queryFn: () => getZedProject(serverUrl, id),
...
export function useZedProjectForSession(
  identity: ZedMasterIdentity,
  sessionId: string,
  options: ZedProjectQueryOptions = {},
) {
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useQuery<ZedProject | null>({
    queryKey: ['zed', 'projects', 'by-session', sessionId, identity.userId ?? 'anonymous', serverUrl],
    queryFn: async () => {
      try {
        return await getZedProjectBySession(serverUrl, sessionId);
      } catch {
        return null;
      }
    },
 ...
 */
export function useZedProjectSessions(
  identity: ZedMasterIdentity,
  projectId: string,
  options: ZedProjectQueryOptions = {},
) {
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  // `listZedProjectSessions` itself returns `unknown[]` (its enriched
  ...
  return useQuery<unknown[]>({
    queryKey: ['zed', 'projects', projectId, 'sessions', identity.userId ?? 'anonymous', serverUrl],
    queryFn: () => listZedProjectSessions(serverUrl, projectId),
...
export function useDeleteProject() {
  ...
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useMutation({
    mutationFn: (id: string) => deleteZedProject(serverUrl, id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: zedKeys.projects() });
    },
  });
}
...
export function usePatchProject() {
  ...
  return useMutation({
    mutationFn: ({ id, ...body }: { id: string; name?: string; description?: string; user_handle?: string | null }) =>
      patchZedProject(serverUrl, id, body),
    onSuccess: (_d, vars) => {
      qc.invalidateQueries({ queryKey: zedKeys.project(vars.id) });
      qc.invalidateQueries({ queryKey: zedKeys.projects() });
    },
  });
}
...
interface ZedTaskQueryOptions {
  enabled?: boolean;
  pollingEnabled?: boolean;
}

const taskKeys = {
  all: ['zed', 'tasks'] as const,
  byProject: (projectId: string) => ['zed', 'tasks', projectId] as const,
  single: (id: string) => ['zed', 'tasks', 'detail', id] as const,
  events: (id: string) => ['zed', 'tasks', 'events', id] as const,
  status: (id: string) => ['zed', 'tasks', 'status', id] as const,
};

const VALID_STATUSES: ZedTaskStatus[] = [
 ...
 * `ZedTask['status']`. `raw` is genuinely unvalidated wire data — duck-type
 * via `unknown` rather than assume the shape. Exported for direct unit testing. */
export function normalizeTask(raw: unknown): ZedTask {
  const r = (raw && typeof raw === 'object' ? raw : {}) as Record<string, unknown>;
  const status = VALID_STATUSES.includes(r.status as ZedTaskStatus)
    ? (r.status as ZedTaskStatus)
    : 'todo';
  return {
    id: r.id as string,
...
export function useZedTasks(
  projectId?: string,
  status?: string,
  options: ZedTaskQueryOptions = {},
) {
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useQuery({
...
export function useZedTask(id: string, options: ZedTaskQueryOptions = {}) {
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useQuery({
...
export function useZedTaskEvents(id: string, options: ZedTaskQueryOptions = {}) {
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useQuery({
...
export function useZedTaskStatus(id: string, options: ZedTaskQueryOptions = {}) {
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useQuery({
...
export function useCreateZedTask() {
  const qc = useQueryClient();
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useMutation({
    mutationFn: (data: {
...
export function useUpdateZedTask() {
  const qc = useQueryClient();
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useMutation({
    mutationFn: ({ id, ...data }: { id: string } & Partial<ZedTask>) => updateTask(serverUrl, id, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: taskKeys.all });
    },
  });
...
export function useStartZedTask() {
  const qc = useQueryClient();
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useMutation({
    ...
    onSuccess: (task) => {
      qc.invalidateQueries({ queryKey: taskKeys.all });
      qc.invalidateQueries({ queryKey: ['zed', 'projects'] });
      if (task?.project_id) {
        qc.invalidateQueries({ queryKey: ['zed', 'projects', task.project_id] });
        qc.invalidateQueries({ queryKey: ['zed', 'projects', task.project_id, 'sessions'] });
      }
    },
  });
}

export function useApproveZedTask() {
  const qc = useQueryClient();
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useMutation({
...
export function useDeleteZedTask() {
  const qc = useQueryClient();
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useMutation({
...
// ═════════════════════════════════════════════════════════════════════════
// Tickets — /zed/tickets, plus columns/fields/templates/agents/pm-session
// /activity project sub-routes
// ═════════════════════════════════════════════════════════════════════════

export const ticketKeys = {
  tickets: (pid?: string) => ['zed', 'tickets', pid ?? ''] as const,
  ticket: (id: string) => ['zed', 'ticket', id] as const,
  events: (id: string) => ['zed', 'ticket', id, 'events'] as const,
  columns: (pid: string) => ['zed', 'columns', pid] as const,
  fields: (pid: string) => ['zed', 'fields', pid] as const,
  templates: (pid: string) => ['zed', 'templates', pid] as const,
  agents: (pid: string) => ['zed', 'agents', pid] as const,
};
...
export function useCreateTicket(identity: ZedMasterIdentity) {
  const qc = useQueryClient();
...
export function useUpdateTicket(identity: ZedMasterIdentity) {
  const qc = useQueryClient();
  const handle = identity.handle;
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useMutation({
...
export function useUpdateTicketStatus(identity: ZedMasterIdentity) {
  const qc = useQueryClient();
  const handle = identity.handle;
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useMutation({
...
export function useAssignTicket(identity: ZedMasterIdentity) {
  const qc = useQueryClient();
  const handle = identity.handle;
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useMutation({
...
export function useUnassignTicket(identity: ZedMasterIdentity) {
  const qc = useQueryClient();
  const handle = identity.handle;
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useMutation({
...
export function useCommentTicket(identity: ZedMasterIdentity) {
  const qc = useQueryClient();
  const handle = identity.handle;
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useMutation({
...
export function useDeleteTicket() {
  ...
  return useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['zed', 'tickets'] }); },
  });
}
...
export function useAgentPersona(projectId?: string, slug?: string, opts?: { enabled?: boolean }) {
  ...
  return useQuery<{ agent: ProjectAgent; body_md: string }>({
    queryKey: ['zed', 'agent-persona', projectId ?? '', slug ?? ''],
...
export function useProjectActivity(projectId?: string, opts?: { enabled?: boolean; pollingEnabled?: boolean }) {
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useQuery<TicketEvent[]>({
    queryKey: ['zed', 'project-activity', projectId ?? ''],
...
const LAST_SEEN_KEY = (projectId: string, handle: string) => `zed:activity-last-seen:${projectId}:${handle}`;
export const LAST_SEEN_EVENT = 'zed:last-seen-changed';
...
export const milestoneKeys = {
  list: (pid?: string, status: 'open' | 'closed' | 'all' = 'all') => ['zed', 'milestones', pid ?? '', status] as const,
  detail: (pid: string, ref: string) => ['zed', 'milestone', pid, ref] as const,
  events: (pid: string, ref: string) => ['zed', 'milestone', pid, ref, 'events'] as const,
};
...
export function useCreateMilestone() {
  ...
  return useMutation<Milestone, Error, CreateMilestoneInput>({
    ...
    onSuccess: (_res, vars) => {
      qc.invalidateQueries({ queryKey: ['zed', 'milestones', vars.projectId] });
    },
  });
}
...
/** Link or unlink a ticket's milestone. Goes through PATCH /zed/tickets/:id. */
export function useSetTicketMilestone() {
  ...
  return useMutation<unknown, Error, { projectId: string; ticketId: string; milestoneId: string | null }>({
    ...
    onSuccess: (_res, vars) => {
      qc.invalidateQueries({ queryKey: ['zed', 'milestones', vars.projectId] });
      qc.invalidateQueries({ queryKey: ['zed', 'tickets', vars.projectId] });
      qc.invalidateQueries({ queryKey: ['zed', 'ticket', vars.ticketId] });
    },
  });
}

// ═════════════════════════════════════════════════════════════════════════
// Sandbox services — /zed/services
// ═════════════════════════════════════════════════════════════════════════
...
export function useSandboxServices(identity: ZedMasterIdentity, options?: { enabled?: boolean; includeAll?: boolean }) {
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
...
export function useSandboxServiceTemplates(identity: ZedMasterIdentity, options?: { enabled?: boolean }) {
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
...
export function useSandboxServiceLogs(identity: ZedMasterIdentity, serviceId: string | null, options?: { enabled?: boolean }) {
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
...
// backing `POST /zed/services/system/reload`) — reused directly rather
