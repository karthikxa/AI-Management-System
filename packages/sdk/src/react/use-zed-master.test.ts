...
// made by OTHER test files (per the `../opencode/zed-master.test.ts`
...
// the REAL `use-zed-master.ts` -> `../opencode/client` -> `../opencode/
// zed-master` chain run for real, with only the actual fetch boundary
// faked, matching `zed-master.test.ts`'s own established pattern.
//
...
const M = await import('./use-zed-master');
...
const IDENTITY_AUTHED: import('./use-zed-master').ZedMasterIdentity = {
  userId: 'user-1',
  handle: 'alice',
  isLoading: false,
};
...
function ticketEvent(overrides: Partial<import('./use-zed-master').TicketEvent> = {}): import('./use-zed-master').TicketEvent {
  return {
    id: 'ev-1',
    ticket_id: 'tk-1',
    project_id: 'p1',
...
describe('query key factories', () => {
  test('credentialKeys', () => {
    expect(M.credentialKeys.list('p1')).toEqual(['zed', 'credentials', 'p1']);
    expect(M.credentialKeys.list()).toEqual(['zed', 'credentials', '']);
    expect(M.credentialKeys.events('p1', 'API_KEY')).toEqual(['zed', 'credentials', 'p1', 'API_KEY', 'events']);
  });

  test('zedKeys', () => {
    expect(M.zedKeys.projects()).toEqual(['zed', 'projects']);
    expect(M.zedKeys.project('p1')).toEqual(['zed', 'projects', 'p1']);
  });

  test('ticketKeys', () => {
    expect(M.ticketKeys.tickets('p1')).toEqual(['zed', 'tickets', 'p1']);
    expect(M.ticketKeys.tickets()).toEqual(['zed', 'tickets', '']);
    expect(M.ticketKeys.ticket('tk-1')).toEqual(['zed', 'ticket', 'tk-1']);
    expect(M.ticketKeys.events('tk-1')).toEqual(['zed', 'ticket', 'tk-1', 'events']);
    expect(M.ticketKeys.columns('p1')).toEqual(['zed', 'columns', 'p1']);
    expect(M.ticketKeys.fields('p1')).toEqual(['zed', 'fields', 'p1']);
    expect(M.ticketKeys.templates('p1')).toEqual(['zed', 'templates', 'p1']);
    expect(M.ticketKeys.agents('p1')).toEqual(['zed', 'agents', 'p1']);
  });

  test('milestoneKeys', () => {
    expect(M.milestoneKeys.list('p1', 'open')).toEqual(['zed', 'milestones', 'p1', 'open']);
    expect(M.milestoneKeys.list()).toEqual(['zed', 'milestones', '', 'all']);
    expect(M.milestoneKeys.detail('p1', 'M-1')).toEqual(['zed', 'milestone', 'p1', 'M-1']);
    expect(M.milestoneKeys.events('p1', 'M-1')).toEqual(['zed', 'milestone', 'p1', 'M-1', 'events']);
  });
...
describe('useZedTasks (hook query-key + query behavior)', () => {
  test('builds the query key exactly as the pre-migration hook did', () => {
    const cfg = M.useZedTasks('proj-1', 'todo') as any;
    expect(cfg.queryKey).toEqual(['zed', 'tasks', 'proj-1', 'todo']);
  });

  test('is disabled without a projectId', () => {
    const cfg = M.useZedTasks(undefined) as any;
    expect(cfg.enabled).toBe(false);
  });

  test('is enabled once a projectId is supplied', () => {
    const cfg = M.useZedTasks('proj-1') as any;
    expect(cfg.enabled).toBe(true);
  });

  test('respects pollingEnabled: false by turning off refetchInterval', () => {
    const cfg = M.useZedTasks('proj-1', undefined, { pollingEnabled: false }) as any;
  ...
  test('queryFn calls the transport with the active server url + params, and normalizes rows', async () => {
    ...
    expect(last().url).toBe(`${FAKE_SERVER_URL}/zed/tasks?project_id=proj-1&status=todo`);
...
describe('useZedProjects (identity gating + cache partitioning)', () => {
  test('query key partitions by identity.userId and the active server url', () => {
    const cfg = M.useZedProjects(IDENTITY_AUTHED) as any;
    expect(cfg.queryKey).toEqual(['zed', 'projects', 'user-1', FAKE_SERVER_URL]);
  });

  test('falls back to "anonymous" in the query key with no user id', () => {
    const cfg = M.useZedProjects({ userId: null, handle: 'me', isLoading: false }) as any;
    expect(cfg.queryKey).toEqual(['zed', 'projects', 'anonymous', FAKE_SERVER_URL]);
  });

  test('is disabled while identity is loading, even with a user id already known', () => {
    const cfg = M.useZedProjects({ userId: 'user-1', handle: 'alice', isLoading: true }) as any;
    expect(cfg.enabled).toBe(false);
  });
...
describe('useCreateTicket (identity injection into mutation body + invalidation)', () => {
  test('stamps actor_id/created_by_id from identity.handle, not a hardcoded value', async () => {
    ...
    expect(last().url).toBe(`${FAKE_SERVER_URL}/zed/tickets`);
