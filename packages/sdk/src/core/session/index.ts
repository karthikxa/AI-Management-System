/**
 * `@zed/sdk/session` — a session's runtime surface.
 *
 ...
 * through the `createZed(...).session(pid, sid)` handle (`.health()`,
 ...
 * the `/zed/health` liveness probe, and the preview-proxy auth helpers. Dead
 * legacy endpoints (`/zed/ports`, `/env` CRUD, `/zed/services`, the board)
 * are excluded.
 */
