 ...
 * capability isn't wired on the current deployment â€” e.g. connector
 ...
 * set a model their account can't use â€” e.g. a managed model on a free tier,
 ...
 * `isModelServableForAccount`. This is an EXPECTED condition â€” a UI validation
 * error, not a server bug â€” so `makeRequest` classifies a 409 carrying this
 ...
 * carrying the same `idempotency_key` is still mid-provision â€” see
 ...
 * be SILENT to `onError` â€” otherwise the web host's global handler shows a red
 ...
 * idempotent reads. A real 500 is NOT here â€” it's a deterministic bug and must
 ...
 *  â€” POST/PUT/PATCH/DELETE mutate state and must not be replayed by the client. */
...
async function makeRequest<T = any>(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // actionable authz denial â€” the user can complete an MFA challenge and
      ...
      // Expected "feature not enabled on this deployment" state â€” the backend
      ...
      // InfoBanner), so they must NEVER page Better Stack â€” a bare 501
      // "not supported" previously leaked as an opaque `ApiError` in Sentry
      // (pattern `1f3c4d96â€¦`). Treat it as SILENT here: skip the global
      ...
      // Expected "this model isn't available for this account" state â€” the
      ...
      // Stack â€” a bare `ApiError: Model "â€¦" is not available for this account`
      // previously leaked to Sentry as an unhandled-looking rejection (pattern
      // `ed07f6c5â€¦`) because the model-defaults `useMutation` had no `onError`
      ...
      // Expected "a concurrent attempt with this key is still running" state â€”
    ...
    if (requestWasAborted) {
      ...
      // timeout â€” surfacing it as one produced the mysterious, URL-less
      ...
      // Genuine timeout â€” our timer fired. Attach the endpoint so it's clear
