...
import { SessionNotReadyError, createZed } from './zed';
...
test('project(id).apps exposes the complete App lifecycle with the project id bound', async () => {
  const apps = zed.project('PID123').apps;
...
test('project(id).connectors exposes the complete connector data plane', async () => {
  const connectors = zed.project('PID123').connectors;
...
test('top-level connectors supports a project-scoped agent token without a project id', async () => {
  expect(typeof zed.connectors.catalog).toBe('function');
  expect(typeof zed.connectors.call).toBe('function');

  await zed.connectors.call('slack.send_message', { channel: 'C1' });
...
test('project(id).secrets.broker binds the project and encoded identifier', async () => {
  await zed.project('PID123').secrets.broker('primary/key', {
...
test('session(projectId, sessionId) binds both ids', async () => {
  await zed.session('PID123', 'SID456').previews();
...
test('session(projectId, sessionId).cost binds project scope without starting the runtime', async () => {
  await zed.session('PID123', 'SID456').cost();
...
test('session(...).rescope writes canonical connection bindings', async () => {
  await zed.session('PID123', 'SID456').rescope({
    connector_bindings: {
...
test('project(id).sessions.list forwards manager inventory scope', async () => {
  await zed.project('PID123').sessions.list({ scope: 'project' });
...
test('project(id).sessions exposes server-owned warm-session ensure and claim', async () => {
  ...
  await zed.project('PID123').sessions.ensureWarm();
  expect(last().url).toContain('/projects/PID123/sessions/warm');
  expect(last().method).toBe('POST');

  await zed.project('PID123').sessions.claimWarm({ session_id: 'SID456' });
...
test('session(...).audit hits the audit endpoint with the given limit', async () => {
  await zed.session('PID123', 'SID456').audit(10);
...
test('project(id).access.invite forwards a time-bound expiry to the backend', async () => {
  ...
  await zed.project('PID123').access.invite('teammate@acme.com', 'member', expiry);
...
test('project(id).access.invite omits expires_at for a permanent grant', async () => {
  await zed.project('PID123').access.invite('teammate@acme.com', 'member');
  expect(last().body).not.toHaveProperty('expires_at');
});

test('project(id).access.invite sends expires_at:null to clear a bound', async () => {
  await zed.project('PID123').access.invite('teammate@acme.com', 'member', null);
...
test('project(id).gateway hits the gateway observability + budget + key endpoints', async () => {
  await zed.project('PID123').gateway.logs({ limit: 10 });
  expect(last().url).toContain('/projects/PID123/gateway/logs?limit=10');

  await zed.project('PID123').gateway.overview(7);
  expect(last().url).toContain('/projects/PID123/gateway/overview?days=7');

  await zed.project('PID123').gateway.budgets();
  expect(last().url).toContain('/projects/PID123/gateway/budgets');

  await zed.project('PID123').gateway.setBudget({ scope: 'project', limit_usd: 50 });
  expect(last().url).toContain('/projects/PID123/gateway/budgets');
  expect(last().method).toBe('PUT');

  await zed.project('PID123').gateway.createKey('ci-key');
  expect(last().url).toContain('/projects/PID123/gateway/keys');
  expect(last().method).toBe('POST');

  await zed.project('PID123').gateway.revokeKey('KEY1');
...
test('project(id).gateway.routing binds policy CRUD and preview to the project', async () => {
  await zed.project('PID123').gateway.routing.get();
  expect(last().url).toContain('/projects/PID123/gateway/routing-policy');
  expect(last().method).toBe('GET');

  await zed.project('PID123').gateway.routing.set({
  ...
  await zed.project('PID123').gateway.routing.preview({
  ...
  await zed.project('PID123').gateway.routing.reset();
  expect(last().method).toBe('DELETE');
});

test('project(id).channels covers slack, email and voice', async () => {
  await zed.project('PID123').channels.slack.installation();
  expect(last().url).toContain('/projects/PID123/channels/slack/installation');

  await zed.project('PID123').channels.email.mode();
  expect(last().url).toContain('/projects/PID123/channels/email/mode');

  await zed.project('PID123').channels.voice.setBotName('Zed');
...
test('project(id) exposes provider-neutral Apps without a generic deployments alias', () => {
  const handle = zed.project('PID123') as unknown as Record<string, unknown>;
...
test('project(id).modelDefaults gets/sets/clears the default model', async () => {
  await zed.project('PID123').modelDefaults.get();
  expect(last().url).toContain('/projects/PID123/model-defaults');

  await zed.project('PID123').modelDefaults.set({ scope: 'project', model: 'anthropic/claude' });
  expect(last().method).toBe('PUT');

  await zed.project('PID123').modelDefaults.clear({ scope: 'project' });
  expect(last().method).toBe('DELETE');
});

test('project(id).modelPicker loads the compact selector catalog', async () => {
  await zed.project('PID123').modelPicker();
...
test('project(id).sandbox hits the sandbox/snapshot/template admin endpoints', async () => {
  await zed.project('PID123').sandbox.list();
  expect(last().url).toContain('/projects/PID123/sandboxes');

  await zed.project('PID123').sandbox.snapshots();
  expect(last().url).toContain('/projects/PID123/snapshots');

  await zed.project('PID123').sandbox.rebuildSnapshot();
...
test('project(id).setAgentScope binds the project id + agent name', async () => {
  await zed.project('PID123').setAgentScope('researcher', { env: 'all' });
  expect(last().url).toContain('/projects/PID123/agents/researcher/scope');
  expect(last().method).toBe('PUT');
});

test('zed.github covers install/list/link/repo endpoints (account-scoped, not project-scoped)', async () => {
  await zed.github.getInstallation('ACC1');
  expect(last().url).toContain('/projects/github/installation?account_id=ACC1');

  await zed.github.listRepositories('ACC1');
  expect(last().url).toContain('/projects/github/repositories?account_id=ACC1');
});

test('zed.sandboxShares hits /p/share (sandbox-scoped, not project-scoped)', async () => {
  await zed.sandboxShares.list('SB1');
  expect(last().url).toContain('/p/share?sandbox_id=SB1');

  await zed.sandboxShares.create({ sandboxId: 'SB1', port: 8000 });
  expect(last().url).toContain('/p/share');
  expect(last().method).toBe('POST');

  await zed.sandboxShares.revoke('SB1', 'TOK1');
...
test('zed.accounts covers cancel/resend invite (account-scoped)', async () => {
  await zed.accounts.cancelInvite('ACC1', 'INV1');
  expect(last().url).toContain('/accounts/ACC1/invites/INV1');
  expect(last().method).toBe('DELETE');

  await zed.accounts.resendInvite('ACC1', 'INV1');
  expect(last().url).toContain('/accounts/ACC1/invites/INV1/resend');
  expect(last().method).toBe('POST');
});

test('zed.accountInvites covers describe/accept/decline (invite-token scoped, no account id)', async () => {
  await zed.accountInvites.describe('INV1');
  expect(last().url).toContain('/account-invites/INV1');
  expect(last().method).toBe('GET');

  await zed.accountInvites.accept('INV1');
  expect(last().url).toContain('/account-invites/INV1/accept');
  expect(last().method).toBe('POST');

  await zed.accountInvites.decline('INV1');
...
test('project(id).access covers group-grant attach/update/detach', async () => {
  await zed.project('PID123').access.attachGroupGrant('GRP1', 'member');
  expect(last().url).toContain('/projects/PID123/group-grants');
  expect(last().method).toBe('POST');

  await zed.project('PID123').access.updateGroupGrant('GRP1', 'editor');
  expect(last().url).toContain('/projects/PID123/group-grants/GRP1');
  expect(last().method).toBe('PATCH');

  await zed.project('PID123').access.detachGroupGrant('GRP1');
  expect(last().url).toContain('/projects/PID123/group-grants/GRP1');
  expect(last().method).toBe('DELETE');
});

test('project(id).access.resourceGrants covers list/create/remove', async () => {
  await zed.project('PID123').access.resourceGrants.list();
  expect(last().url).toContain('/projects/PID123/resource-grants');
  expect(last().method).toBe('GET');

  await zed.project('PID123').access.resourceGrants.create({
    resourceType: 'secret',
  ...
  await zed.project('PID123').access.resourceGrants.remove('G9');
...
test('project(id).secrets covers provider OAuth start, poll, and removal', async () => {
  await zed.project('PID123').secrets.startProviderOAuth('chatgpt');
  expect(last().url).toContain('/projects/PID123/oauth/chatgpt/start');
  expect(last().method).toBe('POST');

  await zed.project('PID123').secrets.pollProviderOAuth('chatgpt', 'FLOW1');
  expect(last().url).toContain('/projects/PID123/oauth/chatgpt/poll');
  expect(last().method).toBe('POST');

  await zed.project('PID123').secrets.removeProviderOAuth('chatgpt');
...
test('project(id).connectors covers credential-mode/sensitive/policies/pipedream', async () => {
  await zed.project('PID123').connectors.auth.discover({
    slug: 'hubspot',
    provider: 'postman',
...
test('project(id).connectors exposes the connection lifecycle', async () => {
  await zed.project('PID123').connectors.connections.list();
  expect(last().url).toContain('/projects/PID123/connections');

  await zed.project('PID123').connectors.connections.reconcile({
...
test('zed.connectStatus hits the top-level connect-status endpoint (not project-scoped)', async () => {
  await zed.connectStatus();
...
test('project(id) covers experimental-feature toggle, sandbox provider pin, and repo-collaborator invite', async () => {
  await zed.project('PID123').updateExperimentalFeature('marketplace', true);
  expect(last().url).toContain('/projects/PID123/experimental');
  expect(last().method).toBe('PATCH');

  await zed.project('PID123').setDefaultAgent('zed');
  expect(last().url).toContain('/projects/PID123/default-agent');
  expect(last().method).toBe('PUT');

  await zed.project('PID123').sandbox.setProvider('daytona');
  expect(last().url).toContain('/projects/PID123/sandbox-provider');
  expect(last().method).toBe('PATCH');

  await zed.project('PID123').git.inviteCollaborator('octocat');
  expect(last().url).toContain('/projects/PID123/git/collaborators');
  expect(last().method).toBe('POST');
});

test('zed.projects.createRepo hits the create-repo endpoint (not bound to an existing project id)', async () => {
  await zed.projects.createRepo({ name: 'new-repo' });
  expect(last().url).toContain('/projects/create-repo');
  expect(last().method).toBe('POST');
});

test('zed.transcribe hits the top-level /transcription endpoint (not project-scoped)', async () => {
  const file = new File(['audio'], 'clip.webm', { type: 'audio/webm' });
  await zed.transcribe(file);
...
test('zed.accounts.tokens covers list/create/revoke (account-scoped CLI PATs)', async () => {
  await zed.accounts.tokens.list('ACC1');
  expect(last().url).toContain('/accounts/tokens?account_id=ACC1');
  expect(last().method).toBe('GET');

  await zed.accounts.tokens.create({ name: 'ci-key', accountId: 'ACC1', projectId: 'PID1' });
  expect(last().url).toContain('/accounts/tokens');
  expect(last().method).toBe('POST');

  await zed.accounts.tokens.revoke('TOK1', 'ACC1');
...
test('project(id).tokens covers list/create/revoke (project-scoped CLI PATs)', async () => {
  await zed.project('PID123').tokens.list();
  expect(last().url).toContain('/projects/PID123/cli-token');
  expect(last().method).toBe('GET');

  await zed.project('PID123').tokens.create({ name: 'agent-token' });
  expect(last().url).toContain('/projects/PID123/cli-token');
  expect(last().method).toBe('POST');

  await zed.project('PID123').tokens.revoke('TOK1');
  expect(last().url).toContain('/projects/PID123/cli-token/TOK1');
  expect(last().method).toBe('DELETE');
});

test('zed.billing covers the read surface (account-state, transactions, credits, tiers)', async () => {
  await zed.billing.accountState();
  expect(last().url).toContain('/billing/account-state');

  await zed.billing.accountStateMinimal();
  expect(last().url).toContain('/billing/account-state/minimal');

  await zed.billing.transactions({ accountId: 'ACC1', limit: 10 });
  expect(last().url).toContain('/billing/transactions?account_id=ACC1&limit=10');

  await zed.billing.transactionsSummary({ days: 7 });
  expect(last().url).toContain('/billing/transactions/summary?days=7');

  await zed.billing.creditBreakdown();
  expect(last().url).toContain('/billing/credit-breakdown');

  await zed.billing.usageHistory(14);
  expect(last().url).toContain('/billing/usage-history?days=14');

  await zed.billing.tierConfigurations();
...
test('session(...).transcript hits the compact transcript endpoint with limit/chars', async () => {
  await zed.session('PID123', 'SID456').transcript({ limit: 10, chars: 200 });
...
test('project(id).changeRequests.requestChanges hits the request-changes endpoint', async () => {
  await zed.project('PID123').changeRequests.requestChanges('CR1', 'please fix the tests');
  expect(last().url).toContain('/projects/PID123/change-requests/CR1/request-changes');
  expect(last().method).toBe('POST');
});

test('zed.accounts.audit covers log/export/webhooks CRUD', async () => {
  await zed.accounts.audit.log('ACC1', { action: 'iam.', limit: 20 });
  expect(last().url).toContain('/accounts/ACC1/audit?action=iam.&limit=20');
  expect(last().method).toBe('GET');

  await zed.accounts.audit.export('ACC1', { format: 'csv' });
  expect(last().url).toContain('/accounts/ACC1/audit/export?format=csv');

  await zed.accounts.audit.webhooks.list('ACC1');
  expect(last().url).toContain('/accounts/ACC1/audit/webhooks');
  expect(last().method).toBe('GET');

  await zed.accounts.audit.webhooks.create('ACC1', {
    name: 'siem',
  ...
  await zed.accounts.audit.webhooks.update('ACC1', 'WH1', { enabled: false });
  expect(last().url).toContain('/accounts/ACC1/audit/webhooks/WH1');
  expect(last().method).toBe('PATCH');

  await zed.accounts.audit.webhooks.remove('ACC1', 'WH1');
  expect(last().url).toContain('/accounts/ACC1/audit/webhooks/WH1');
  expect(last().method).toBe('DELETE');
});

test('project(id).audit and session(id).audit expose canonical cursor pagination', async () => {
  await zed.project('PID123').audit({ phase: 'completed', cursor: 'CUR1' });
  expect(last().url).toContain('/projects/PID123/audit?phase=completed&cursor=CUR1');

  await zed.session('PID123', 'SID456').audit(50, { cursor: '42|EVENT' });
...
test('project(id).channels.slack covers file download + upload proxies', async () => {
  await zed.project('PID123').channels.slack.getFile('https://files.slack.com/x');
  expect(last().url).toContain('/projects/PID123/channels/slack/file?url=');
  expect(last().method).toBe('GET');

  await zed.project('PID123').channels.slack.uploadFile({
    channel: 'C1',
    filename: 'report.pdf',
    contentBase64: 'YWJj',
  });
...
test('project(id).gateway.playground posts prompt + models', async () => {
  await zed.project('PID123').gateway.playground('Say hi', ['gpt-4o', 'claude-3']);
  expect(last().url).toContain('/projects/PID123/gateway/playground');
  expect(last().method).toBe('POST');
});

test('zed.billing.checkout covers create + confirm session', async () => {
  await zed.billing.checkout.createSession({
    tierKey: 'pro',
  ...
  await zed.billing.checkout.confirmSession('cs_123');
  expect(last().url).toContain('/billing/confirm-checkout-session');
  expect(last().method).toBe('POST');
});

test('zed.billing.subscription covers portal/cancel/reactivate/downgrade/proration', async () => {
  await zed.billing.subscription.createPortalSession('https://app.example.com/billing');
  expect(last().url).toContain('/billing/create-portal-session');
  expect(last().method).toBe('POST');

  await zed.billing.subscription.cancel('too expensive');
  expect(last().url).toContain('/billing/cancel-subscription');

  await zed.billing.subscription.reactivate();
  expect(last().url).toContain('/billing/reactivate-subscription');

  await zed.billing.subscription.scheduleDowngrade('starter');
  expect(last().url).toContain('/billing/schedule-downgrade');

  await zed.billing.subscription.cancelScheduledChange();
  expect(last().url).toContain('/billing/cancel-scheduled-change');

  await zed.billing.subscription.prorationPreview('price_123');
  expect(last().url).toContain('/billing/proration-preview?new_price_id=price_123');
  expect(last().method).toBe('GET');
});

test('zed.billing.credits covers purchase + auto-topup get/configure', async () => {
  await zed.billing.credits.purchase({ amount: 20 });
  expect(last().url).toContain('/billing/purchase-credits');
  expect(last().method).toBe('POST');

  await zed.billing.credits.autoTopupSettings();
  expect(last().url).toContain('/billing/auto-topup/settings');
  expect(last().method).toBe('GET');

  await zed.billing.credits.configureAutoTopup({ enabled: true, threshold: 5, amount: 20 });
  expect(last().url).toContain('/billing/auto-topup/configure');
  expect(last().method).toBe('POST');
});

test('zed.billing.sessionCosts covers paginated list and detail reads', async () => {
  await zed.billing.sessionCosts.list({
    accountId: 'ACC1',
    projectId: 'PID1',
    limit: 20,
    offset: 0,
  ...
  await zed.billing.sessionCosts.get('SID/1', {
    accountId: 'ACC1',
    projectId: 'PID1',
  });
  expect(last().url).toBe(
...
test('zed.marketplace covers public catalog browse + authed sources CRUD (top-level, not project-scoped)', async () => {
  await zed.marketplace.items({ query: 'slack' });
  expect(last().url).toContain('/marketplace/items?query=slack');
  expect(last().method).toBe('GET');

  await zed.marketplace.item('zed:researcher');
  expect(last().url).toContain('/marketplace/items/zed%3Aresearcher');

  await zed.marketplace.itemFile('zed:researcher', 'agent.md');
  expect(last().url).toContain('/marketplace/items/zed%3Aresearcher/file?path=agent.md');

  await zed.marketplace.marketplaces();
  expect(last().url).toContain('/marketplace/marketplaces');

  await zed.marketplace.featured();
  expect(last().url).toContain('/marketplace/marketplaces/featured');

  await zed.marketplace.sources.list();
  expect(last().url).toContain('/marketplace/sources');
  expect(last().method).toBe('GET');

  await zed.marketplace.sources.add({ address: 'https://github.com/acme/registry' });
  expect(last().url).toContain('/marketplace/sources');
  expect(last().method).toBe('POST');

  await zed.marketplace.sources.remove('SRC1');
  expect(last().url).toContain('/marketplace/sources/SRC1');
  expect(last().method).toBe('DELETE');
});

test('zed.validateToken hits /accounts/me and never throws', async () => {
  const result = await zed.validateToken();
...
test('two session handles resolve independent sandboxes: A.send never crosses to B (or back)', async () => {
  ...
  const k = createZed({ backendUrl: 'http://test.local', getToken: async () => 'tok' });
...
test('changeModel invalidates the persisted default before the next send', async () => {
  let persistedModel = 'zed/glm-5.2';
  globalThis.fetch = mock(async (input: unknown, init?: RequestInit) => {
    const url = requestUrl(input);
    const request = input instanceof Request ? input : null;
    const bodyText = request ? await request.clone().text() : String(init?.body ?? '');
  ...
  await handle.changeModel('zed/gpt-5.6-mini');
  ...
  expect(prompts.map((call) => call.body)).toEqual([
    ...
    expect.objectContaining({
      model: { providerID: 'zed', modelID: 'gpt-5.6-mini' },
    }),
  ]);
});
...
test('changeModel surfaces push_failed so a half-applied change is not read as saved', async () => {
  ...
  const k = createZed({ backendUrl: 'http://test.local', getToken: async () => 'tok' });
  const result = await k.session('PROJ', 'SESS-HALF').changeModel('zed/deepseek-v4-flash');
...
// `zed.session(...)` handle, see apps/whitelabel-demo/session-header.tsx)
...
test("health() resolves against the handle's own runtime URL once ready", async () => {
  ...
  expect(calls.some((c) => c.url.includes('/p/sb-A/8000/zed/health'))).toBe(true);
});

// ── shared session-runtime registry (regression: apps/whitelabel-demo's
// session-header.tsx polls health() on a FRESH `zed.session(...)` handle
...
test('a second fresh handle for the same session adopts the registry entry — no ensureReady() of its own needed', async () => {
  ...
  expect(calls.some((c) => c.url.includes('/p/sb-reg1/8000/zed/health'))).toBe(true);
});
...
// their own `/start` long-poll — a real hazard for a "Zed as a Backend"
...
// module-global "active" sandbox the top-level `@zed/sdk` `files` export
