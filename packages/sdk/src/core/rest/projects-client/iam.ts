...
export async function getEnterpriseDemo(accountId: string): Promise<boolean> {
  try {
    const result = unwrap(await iamGet<{ enabled: boolean }>(`/accounts/${accountId}/iam/enterprise-demo`));
    // Handle case where API returns boolean directly instead of { enabled: boolean }
