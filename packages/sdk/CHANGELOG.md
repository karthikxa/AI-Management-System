...
All notable changes to `@zed/sdk` are documented here. Format loosely
...
- `getProjectModelPicker()` plus `zed.projects.modelPicker` and
  `zed.project(id).modelPicker()` for the compact, connection-aware selector
...
- CDN builds: a minified ESM bundle (`dist/zed.esm.min.js`) and an IIFE
  exposing `window.Zed` (`dist/zed.global.js`), wired via
...
- CI now packs, installs, and imports the tarball, and asserts the two export
  maps agree.
- The install smoke test (`pnpm run smoke:install`) packs `@zed/llm-catalog`
...
Headline: **"Zed as a Backend"** — everything a third-party server needs to
safely wrap Zed on behalf of multiple concurrent end users, plus a
...
- **`@zed/sdk/server`** — `runWithZed(config, fn)` / `createScopedZed(config)`.
  ...
  `configureZed()` singleton. Never reachable from the root `@zed/sdk` or
  `@zed/sdk/react` entry points, so `node:async_hooks` never enters a
  browser bundle.
- **Headless chat kit** — `classifyPart`/`classifyTurn`/`toolInfo`/`ToolView`
  (`@zed/sdk/turns`, also re-exported from the root barrel) normalize every
  ...
  `ClassifiedPart` — plus `ZedChatEvent`/`narrowChatEvent` (a curated
  14-member event union narrowed from the raw ~50-variant SSE wire union) and
  `useChatTurns`/`renderParts`/`PartRenderers` (`@zed/sdk/react`), a
...
- **`session(pid, sid).stream()`** — a framework-free facade over the
  ...
  required. `@zed/sdk/react`'s `useOpenCodeEventStream` is now a thin
...
- **New facade namespaces**, all with client fns already in
  ...
  - `accounts.tokens` / `project(id).tokens` — CLI PAT minting (account- and
    project-scoped `zed_pat_...` tokens).
  ...
  - `project(id).validateManifest(raw)` / `project(id).gitToken()` —
    server-side `zed.toml` validation and scoped git push token minting
  ...
  - `validateToken()` — pasted-API-key validation helper (`GET /accounts/me`,
    never throws).
- **Domain hooks** (`@zed/sdk/react`) — `useProjectSecrets`,
...
- Root barrel (`@zed/sdk`) now type-only re-exports the full set of domain
...
- **One typed error hierarchy across every HTTP layer.** `ApiError`/`AuthError`
  ...
  exported from the root barrel and `@zed/sdk/api-client`, not just
  `@zed/sdk/react` — a server-side wrapper can now `instanceof BillingError`
...
- **30-second default request timeout** on `authenticatedFetch` and the
  ...
  wedge a "Zed as a Backend" server handler forever. The long-lived SSE
...
- `previewUrl()`/`proxyUrl()` tolerate a relative `backendUrl` (a same-origin
  ...
  `createZed()` time — so a host that calls `configureZed()` again after
...
Initial internal release — the single opinionated `createZed()` facade
over the Zed REST API + OpenCode v2 runtime, `@zed/sdk/react` hooks,
