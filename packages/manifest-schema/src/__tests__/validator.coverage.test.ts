...
describe('validateManifest — object input', () => {
  test('accepts an already-parsed object without re-parsing TOML', () => {
    const result = validateManifest({ zed_version: 1 });
    expect(result.valid).toBe(true);
    expect(result.parsed).toEqual({ zed_version: 1 });
  });

  test('object input echoes the same object back as parsed', () => {
    const obj = { zed_version: 1, project: { name: 'x' } };
    const result = validateManifest(obj);
    expect(result.parsed).toBe(obj);
  });

  test('object input with bad version is rejected', () => {
    const result = validateManifest({ zed_version: 0 });
    expect(result.valid).toBe(false);
    expect(result.issues.map((i) => i.path)).toContain('zed_version');
  });

  test('version above known max produces an unsupported error', () => {
    const result = validateManifest({ zed_version: 99 });
...
describe('validateManifest — [project]', () => {
  test('non-table project is rejected', () => {
    expect(errorPaths('zed_version = 1\nproject = "oops"')).toContain('project');
  });
  ...
  test('a fully valid project passes', () => {
    expect(validateManifest('zed_version = 1\n[project]\nname = "X"\ndescription = "Y"').valid).toBe(true);
  });
});
...
describe('validateManifest — [opencode]', () => {
  ...
  test('a clean relative config_dir passes', () => {
    expect(validateManifest('zed_version = 1\n[opencode]\nconfig_dir = ".zed/opencode"').valid).toBe(true);
  });
});
...
describe('validateManifest — [[agents]]', () => {
  ...
  test('duplicate agent names are rejected', () => {
    const paths = errorPaths('zed_version = 1\n[[agents]]\nname = "a"\n[[agents]]\nname = "a"');
  ...
  test('zed_cli accepts a grantable action', () => {
    expect(
      validateManifest('zed_version = 1\n[[agents]]\nname = "w"\nzed_cli = ["project.read"]').valid,
    ).toBe(true);
  });

  test('zed_cli accepts the wildcard star', () => {
    expect(validateManifest('zed_version = 1\n[[agents]]\nname = "w"\nzed_cli = ["*"]').valid).toBe(true);
  });

  test('zed_cli rejects a non-grantable account-scoped action', () => {
    expect(errorPaths('zed_version = 1\n[[agents]]\nname = "w"\nzed_cli = ["billing.read"]')).toContain(
      'agents[0].zed_cli[0]',
    );
  });
...
describe('exported constants', () => {
  ...
  test('GRANTABLE_ZED_CLI_ACTIONS includes project actions but not billing or channel.*', () => {
    expect(GRANTABLE_ZED_CLI_ACTIONS).toContain('project.read');
    expect(GRANTABLE_ZED_CLI_ACTIONS).toContain('project.connector.write');
    expect(GRANTABLE_ZED_CLI_ACTIONS).not.toContain('billing.read');
    // channel.* was removed from the catalog — never wired to any route.
    expect(GRANTABLE_ZED_CLI_ACTIONS).not.toContain('channel.send');
  });
});
