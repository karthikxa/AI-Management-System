...
-- deploy-dev.yml/deploy-prod.yml run `pnpm --filter @zed/db migrate` before the app
-- rollout -- so the widened constraint is always live before any pod can violate it.

-- Recording each ask_zed/run_command tool call the voice-agent worker makes (mcp.ts) as
