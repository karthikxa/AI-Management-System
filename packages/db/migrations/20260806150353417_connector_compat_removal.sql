...
DROP VIEW IF EXISTS zed.workspace_session_connector_bindings;

DROP VIEW zed.executor_connection_policies;
DROP VIEW zed.executor_oauth_sessions;
DROP VIEW zed.executor_oauth_applications;
DROP VIEW zed.executor_executions;
DROP VIEW zed.executor_credentials;
DROP VIEW zed.executor_connection_profiles;
DROP VIEW zed.executor_attachments;
DROP VIEW zed.executor_project_settings;
DROP VIEW zed.executor_project_policies;
DROP VIEW zed.executor_connector_policies;
DROP VIEW zed.executor_connector_grants;
DROP VIEW zed.executor_connector_actions;
DROP VIEW zed.executor_connectors;

DROP TRIGGER sync_session_connector_binding_connection_ids
ON zed.project_session_connector_bindings;
DROP FUNCTION zed.sync_session_connector_binding_connection_ids();
...
WHERE pg_publication_rel.prrelid =
  'zed.project_session_connector_bindings'::regclass;
...
BEGIN
  FOR publication IN SELECT pubname FROM connector_binding_publications LOOP
    EXECUTE format(
      'ALTER PUBLICATION %I DROP TABLE zed.project_session_connector_bindings',
      publication.pubname
    );
  END LOOP;
END
...
ALTER TABLE zed.project_session_connector_bindings DROP COLUMN profile_id;
...
BEGIN
  ...
  FOR publication IN SELECT pubname FROM connector_binding_publications LOOP
    EXECUTE format(
      'ALTER PUBLICATION %I ADD TABLE zed.project_session_connector_bindings (%s)',
      publication.pubname,
      column_list
    );
  END LOOP;
...
DROP TYPE zed.project_secret_consumer;
CREATE TYPE zed.project_secret_consumer AS ENUM (
  'sandbox',
  'llm_gateway',
  'connector',
  'git_proxy',
...
ALTER TABLE zed.project_secrets
  -- squawk-ignore changing-column-type
  ALTER COLUMN consumer TYPE zed.project_secret_consumer
    USING consumer::zed.project_secret_consumer,
  ALTER COLUMN consumer SET DEFAULT 'sandbox'::zed.project_secret_consumer;
ALTER TABLE zed.project_secrets
