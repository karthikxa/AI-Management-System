...
-- zed without any GRANT EXECUTE. This database revokes the default PUBLIC
-- EXECUTE on functions, so the AFTER INSERT trigger on zed.credit_ledger
...
GRANT EXECUTE ON FUNCTION zed.credit_ledger_lifetime_deltas(numeric, text) TO service_role, authenticated;
GRANT EXECUTE ON FUNCTION zed.apply_credit_ledger_lifetime_rollup() TO service_role, authenticated;
GRANT EXECUTE ON FUNCTION zed.recompute_credit_account_lifetime(uuid) TO service_role;
