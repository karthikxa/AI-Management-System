...
-- zed.sync_credit_account_precision_columns BEFORE trigger mirrors them into
...
CREATE OR REPLACE FUNCTION zed.credit_ledger_lifetime_deltas(
  p_amount numeric,
  p_type text
)
...
COMMENT ON FUNCTION zed.credit_ledger_lifetime_deltas(numeric, text) IS
  'How one credit_ledger row moves credit_accounts.lifetime_{granted,purchased,used}. Used by both the maintenance trigger and zed.recompute_credit_account_lifetime.';

-- ── Incremental maintenance ─────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION zed.apply_credit_ledger_lifetime_rollup()
RETURNS trigger
LANGUAGE plpgsql
SET search_path TO ''
AS $$
...
BEGIN
  ...
  FROM zed.credit_ledger_lifetime_deltas(NEW.amount_precise, NEW.type) AS d;
...
DROP TRIGGER IF EXISTS apply_credit_ledger_lifetime_rollup ON zed.credit_ledger;
CREATE TRIGGER apply_credit_ledger_lifetime_rollup
AFTER INSERT ON zed.credit_ledger
FOR EACH ROW
EXECUTE FUNCTION zed.apply_credit_ledger_lifetime_rollup();
...
CREATE OR REPLACE FUNCTION zed.recompute_credit_account_lifetime(
...
BEGIN
  WITH totals AS (
    ...
    FROM zed.credit_ledger l
    CROSS JOIN LATERAL zed.credit_ledger_lifetime_deltas(l.amount_precise, l.type) AS d
    WHERE p_account_id IS NULL OR l.account_id = p_account_id
    GROUP BY l.account_id
  )
  UPDATE zed.credit_accounts a
...
COMMENT ON FUNCTION zed.recompute_credit_account_lifetime(uuid) IS
...
-- `SELECT zed.recompute_credit_account_lifetime(NULL);` to reconcile if that
-- matters. These are reporting figures, not balances — no money depends on them.
SELECT zed.recompute_credit_account_lifetime(NULL);
