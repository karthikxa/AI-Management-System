...
// which under the gateway is ALWAYS "Zed", because every gateway model is
// registered under one synthetic `zed` opencode provider.
//
// So a MISSING entry here does not degrade gracefully: it silently mislabels a
// whole provider group as "Zed". That is exactly what happened to BYOK
