...
describe("hydrateEnvironmentSecret", () => {
  test("hydrates string values and removes the aggregate secret", () => {
    process.env.ZED_ENV_JSON = JSON.stringify({
    ...
    expect(process.env.ZED_ENV_JSON).toBeUndefined();
  });

  test("keeps explicit task environment overrides", () => {
    process.env.ZED_VERSION = "0.12.4";
    process.env.ZED_ENV_JSON = JSON.stringify({ ZED_VERSION: "stale" });

    expect(hydrateEnvironmentSecret()).toBe(0);
    expect(process.env.ZED_VERSION).toBe("0.12.4");
  });

  test("rejects malformed or non-string secret values", () => {
    process.env.ZED_ENV_JSON = "not-json";
    expect(() => hydrateEnvironmentSecret()).toThrow("ZED_ENV_JSON must contain a JSON object");

    process.env.ZED_ENV_JSON = JSON.stringify({ PORT: 8000 });
    expect(() => hydrateEnvironmentSecret()).toThrow('ZED_ENV_JSON key "PORT" must be a string');
  });
});
