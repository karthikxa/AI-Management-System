...
ADMIN_USER="${2:-admin}"
ADMIN_PASSWORD="${3:-}"
INSTALL_DIR="${ZED_HOME:-$HOME/.zed}"

if [ -z "$DOMAIN" ]; then
  echo "Usage: $0 <domain-or-ip> [admin-user] [admin-password]"
  echo ""
  echo "  If admin-password is not provided, reads from ~/.zed/.credentials"
  exit 1
fi
...
if echo "$FRONTEND_BODY" | grep -qi 'zed\|next\|__next'; then
  pass "frontend serves HTML with Zed/Next.js content"
else
  fail "frontend serves HTML with Zed/Next.js content"
fi
...
if command -v docker &>/dev/null; then
  for svc in caddy frontend zed-api postgres zed-sandbox; do
...
if [ -f "$INSTALL_DIR/.env" ]; then
  ...
  if grep -q 'ZED_TOKEN=' "$INSTALL_DIR/.env" 2>/dev/null; then
    TOKEN_VAL=$(grep 'ZED_TOKEN=' "$INSTALL_DIR/.env" | cut -d= -f2)
    if [ ${#TOKEN_VAL} -ge 32 ]; then
      pass "ZED_TOKEN is set (${#TOKEN_VAL} chars)"
    else
      fail "ZED_TOKEN is too short (${#TOKEN_VAL} chars)"
    fi
  else
    fail "ZED_TOKEN is set in .env"
  fi
