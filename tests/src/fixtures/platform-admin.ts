 ...
 * user's role from zed.platform_user_roles. Release QA has direct access to
...
export async function grantEphemeralPlatformAdmin(
  ...
  await execute(
    ...
    `INSERT INTO zed.platform_user_roles (account_id, role)
     VALUES ($1::uuid, 'super_admin'::zed.platform_role)
  ...
  return async () => {
    await execute(
      env.databaseUrl!,
      open,
      `DELETE FROM zed.platform_user_roles
       WHERE account_id = $1::uuid
         AND role = 'super_admin'::zed.platform_role`,
      accountId,
    );
  };
}
